...
| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.7-1 ✅ | DB 스키마 + 진입 기록 | `scalp_pick` 모델 + 마이그레이션, 신규 진입 감지·기록 로직 | 스켈핑 후보 신규 진입 시 DB에 정확히 1행 기록 — **완료(2026-07-21)** |
| 5.7-2 ✅ | 호라이즌 샘플링 | 5/15/30/60분 + 당일 마감 change_rate 채우기, live_refresh 60초 잡에 배선 | 시간 지나면 해당 컬럼이 채워짐, 새 외부 호출 없음 — **완료(2026-07-21)**, 실서비스에서 5분 호라이즌 자동 채움 확인 |
| 5.7-3 ✅ | 조회 API | `GET /api/markets/scalp-candidates/track-record` | 최근 N일 pick + 호라이즌 값 반환 — **완료(2026-07-21)** |

### Phase 5.8 — 전일 미국장 + 필라델피아반도체지수(SOX) (2026-07-22 사용자 제안)

사용자: "오전에 어제 미국장에서 열린 내용과... 필라델피아반도체 지수 같은 특정
ETF도 알아야 할 것 같고" — 지금 대시보드는 국내장 전용이라 오늘 국내장 움직임의
전제가 되는 전일 미국장 맥락이 빠져 있었다.

**실측(2026-07-22)**: yfinance로 S&P500(`^GSPC`)·나스닥종합(`^IXIC`)·다우(`^DJI`)·
필라델피아반도체지수(`^SOX`) 전부 정상 조회 확인(전일 07-21 종가까지 정상 수신,
rate limit 없음) — 유가(WTI)와 동일한 안전한 패턴(하루 1회 배치)으로 바로 적용
가능. **야간선물은 범위 밖**(사용자와 별도로 조사·합의 완료 — 키움 REST API
공식 문서 527p 전체 검색 결과 선물/옵션 카테고리 자체가 없음 확인, 대신증권/
LS증권/KIS 중 하나의 신규 계좌가 있어야 하는 영역이라 이번 phase에서 제외).

**설계**:
1. **소스**: `clients/us_indices.py` 신규(commodities.py의 yfinance→FRED 폴백
   패턴 그대로 재사용). FRED 대체 시리즈가 있는 건 S&P500(`SP500`)·나스닥
   (`NASDAQCOM`) 정도이고, 다우·SOX는 FRED에 없어 yfinance 단일 소스로 문서화
   (§7 리스크 표에 추가).
2. **저장**: 기존 `macro_series` 테이블 재사용(새 테이블 불필요) — series 키
   `us_sp500`/`us_nasdaq`/`us_dow`/`us_sox` 4개 추가. `collectors/macro.py`의
   `collect_macro`에 이 4개 시리즈 fetch+upsert를 추가(WTI/Brent와 동일한
   자리, 하루 1회 배치 그대로).
3. **API**: 기존 `GET /api/macro/series?ids=...`가 이미 범용이라 프런트에서
   새 id 4개를 그냥 요청하면 된다 — 백엔드 라우터 변경 불필요.
4. **프런트**: "시황·자금" KpiTile 행에 환율/WTI와 같은 자리에 4개 타일 추가
   (라벨: S&P500/나스닥/다우/SOX, 등락 화살표+StaleDate 배지 동일 패턴).
   상세는 기존 `MacroModal`을 확장하거나 새 섹션으로 분리 — 구현 에이전트
   재량(레이아웃 자연스러운 쪽으로).

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.8-1 ✅ | 소스 + 수집 | `clients/us_indices.py` + `collectors/macro.py` 4개 시리즈 추가 | 매크로 배치 실행 시 macro_series에 4개 시리즈 upsert — **완료(2026-07-22)** |
| 5.8-2 ✅ | 프런트 타일 | 시황·자금 행에 4개 KpiTile 추가 + 상세 모달 | 대시보드에 전일 미국장 4개 지수 표시, 날짜 배지 정상 — **완료(2026-07-22)** |

### Phase 5.9 — 장 상태 배너 (2026-07-22 사용자 지적)

사용자가 아침 8:48(정규장 09:00 개장 12분 전)에 "업종·테마 강약이 어제 걸로
보여서 헷갈린다", 이어서 "전체적으로 모두 어제꺼가 대부분이야"라고 지적.
직접 전수 점검해보니 **이 시점엔 실제로 정상**이었다 — 실시간 관심 TOP5만
NXT 프리마켓(08:00~)이라 라이브고, 나머지(지수타일·등락종목수·게이지·
수급·거래대금·업종테마·베이시스·외인선물·환율)는 정규장(09:00)이 열려야
"오늘" 계산이 시작되는 공식 통계들이라 어제 확정치로 있는 게 맞는다.

문제는 데이터가 아니라 **"지금이 장 시작 전이라 그렇다"는 걸 화면이 알려주지
않는다**는 것 — 그래서 매번 "고장났나?" 헷갈림이 반복된다(이번이 벌써
여러 번째 유사 지적). 이미 서버가 알고 있는 신호(`indexTilesLive.market_closed`,
`attentionTop`의 `market_closed` 유무)만으로 3단계 장 상태를 판정해 배너로
보여준다 — 새 API·새 시간 로직 없이 이미 폴링 중인 값만 재사용:

1. **정규장 중**: `indexTilesLive?.market_closed === false`
2. **NXT만 열림**(프리마켓 08:00~09:00 또는 애프터마켓 15:30~20:00):
   `indexTilesLive?.market_closed === true` && attention이 살아있음
   (`attentionTop`에 `market_closed:true`가 없음, 즉 실제 rows가 옴)
3. **완전 마감**(주말/야간 20:00~08:00): 둘 다 마감

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.9-1 ✅ | 장 상태 배너 | 대시보드 상단에 3단계 상태 배너(정규장/NXT만/완전마감) + 짧은 설명 문구 | 09:00 전·15:30~20:00·야간에 각각 올바른 배너, 정규장엔 배너 없음(또는 최소화) — **완료(2026-07-22)** |

### Phase 5.10 — 투자자별 수급 요약 코스피/코스닥 분리 (2026-07-22 사용자 요청)

사용자: "투자자별 수급 요약을 코스피, 코스닥을 유분리 해줘야 해.. 어차피
코스피로 다 몰려 있어서.. 코스닥이 주목 받는 날을 좀 봐야 하거든" — 지금은
3M(일별 히스토리)·1D(장중 누적) 둘 다 코스피+코스닥을 항상 합산해서만
보여준다(프런트 `mergeFlows` 함수, 3M은 `/api/markets/{market}/series` 두
번 호출 후 프런트에서 합산, 1D는 백엔드 `intraday_snapshot.record_flow_snapshot`이
애초에 합산해서 버퍼에 적립). 사용자에게 "합계/코스피/코스닥 토글" 방식으로
확인받음(기존 1D/3M 토글과 같은 UI 패턴, 클릭 시 그 시장만의 차트로 전환,
기본은 지금처럼 합계).

**주의(중요)**: `intraday_snapshot.py`의 "외국인" 버퍼는 "투자자별 수급 요약"
뿐 아니라 **"외인 양손" 모달의 현물(spot) 시리즈도 재사용** 중이다
(`get_foreign_position_series`). 이번 작업으로 코스피/코스닥을 분리해도
"외인 양손" 쪽은 지금처럼 코스피+코스닥 합산 그대로 유지해야 한다(그
모달은 이번 요청 범위 밖) — 회귀시키지 마라.

**설계**:
1. **백엔드 버퍼 구조 변경**: `_buffers`를 `{"kospi": {"개인": [], "외국인": [],
   "기관계": []}, "kosdaq": {...}, "외인선물": [...]}`로 재구성(시장별 3개
   투자자 시리즈를 따로 적립). `record_flow_snapshot`은 더 이상 kospi+kosdaq을
   합산하지 않고 각 시장 값을 각자의 버퍼에 append한다.
2. **`get_flow_series()` 응답 변경**: `series.kospi.{개인,외국인,기관계}` /
   `series.kosdaq.{개인,외국인,기관계}` 두 블록으로 반환(합계는 프런트가
   필요하면 그때그때 더해서 계산 — 백엔드가 굳이 세 번째 "합계" 블록을
   미리 계산해서 얹지 않는다).
3. **`get_foreign_position_series()`는 회귀 없이 그대로**: "외국인" spot
   시리즈는 kospi+kosdaq 버퍼를 합산해서(시간 키 기준으로 매칭 — 같은
   warm 호출에서 항상 같은 시각에 같이 적립되므로 인덱스가 맞는다) 지금과
   동일한 모양으로 응답한다. "외인선물"은 그대로.
4. **프런트 — FlowSummaryModal(투자자별 수급 요약)**: 3M/1D 각 탭 안에
   `marketFilter` 토글(합계/코스피/코스닥, 기본 합계) 추가.
   - 3M: 지금처럼 kospi/kosdaq series를 각각 fetch해두고(이미 그렇게 하고
     있음), `marketFilter`에 따라 `mergeFlows(양쪽)`(합계) 또는 한쪽만
     그대로 쓰도록 분기.
   - 1D: `get_flow_series()`의 새 응답(kospi/kosdaq 분리)을 받아
     `marketFilter`에 따라 두 시장을 더하거나(합계) 한쪽만 쓰도록 분기
     (시간 축이 같은 warm 틱에서 나온 거라 그대로 맞춰 더하면 된다).
5. **ForeignPositionModal은 건드리지 않는다** — 이번 phase 범위 밖, 회귀
   테스트로 기존 동작 유지를 확인한다.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.10-1 ✅ | 백엔드 버퍼 분리 | `intraday_snapshot.py` 코스피/코스닥 버퍼 분리, `get_flow_series` 응답 변경, `get_foreign_position_series`는 합산 유지(회귀 없음) | 유닛테스트로 분리·합산 둘 다 검증 — **완료(2026-07-22)**, 외인 양손 회귀 테스트 포함 |
| 5.10-2 ✅ | 프런트 토글 | FlowSummaryModal 3M/1D 양쪽에 합계/코스피/코스닥 토글 | 코스닥만 선택 시 코스닥 수급만 표시, 외인 양손 모달은 그대로 — **완료(2026-07-22)**, Playwright로 Y축 스케일 재조정 확인(코스닥 단독 시 ±12억 vs 합계 ±160억) |

### Phase 5.11 — 평일 18:00 배치 misfire 버그 (2026-07-22 발견) ✅

사용자가 "개인 방향성(파생ETF) 차트가 07-15/07-20 이틀치뿐이라 이상하다"고
지적해 추적하다 발견한 심각한 운영 버그. `collectors/scheduler.py`의
`add_job`에 `misfire_grace_time`을 지정하지 않아 APScheduler 기본값(사실상
초 단위)이 적용됐다 — 이벤트 루프가 몇 분만 늦어도(도커 컨테이너 CPU 경합
등) 그날 18:00 배치 전체(11개 잡)가 예외/로그 없이 조용히 스킵된다.

**실증**: collect_log 전수 조회 결과 지난 일주일 중 11개 잡이 전부 완주한
날은 07-17·07-21 단 둘뿐이었는데, 둘 다 ran_at이 18:00대가 아니라 오후
3시대 — 전부 이 세션에서 사람(나)이 수동으로 `_run_all_jobs`를 직접
실행한 시각과 정확히 일치했다(§5.6-4 등). worker 컨테이너 로그를 타임스탬프까지
까보니 07-21 18:03:56 KST에 `"Run time of job ... was missed by 0:03:56"`
경고만 있고, `_run_all_jobs` 진입 시 무조건 찍히는 "scheduled batch
starting..." INFO 로그가 전혀 없었다 — 트리거는 맞았지만 실행 자체가
스킵된 것.

**수정**: `misfire_grace_time=3600`(1시간) 추가. 정시 실행보다 "그날 안에
한 번은 확실히 도는 것"이 하루 1회뿐인 배치에는 훨씬 중요하다. 회귀 방지
테스트(`test_daily_batch_scheduler.py`) 추가 — `misfire_grace_time`이
최소 30분 이상인지 고정.

**영향 범위**: 이번 세션 내내 반복됐던 "어제/그제 정보로 멈춰있다"류
지적(§5.6-4 지수/등락종목수, 오늘 §5의 ETF 파생 방향성 등) 중 상당수가
사실 이 버그가 근본 원인이었을 가능성이 높다 — 각 사례를 그때그때 수동
배치로 땜질했지만, 이 misfire_grace_time 수정이 향후 재발을 막는
근본 조치다. `net_inflow` 계열처럼 소급 불가능한 소스(§4.5-1)는 이미
지나간 날짜(07-16/07-18/07-19)를 복구할 수 없다 — 앞으로의 재발만 막는다.

**후속 실수 + 교훈(2026-07-23)**: 사용자가 "ETF 경유 수급 상위가 2일째
같은 정보 같다"고 재차 지적 — 확인해보니 **07-22 18:00 배치도 여전히
안 돌았다**. 원인은 이 수정 자체가 아니라 **배포 방식**이었다:
`worker` 컨테이너는 의도적으로 `--reload`가 없다(스케줄러 상태를
지키려고, §Phase 1 참고) — 그래서 `scheduler.py` 코드 파일은 고쳐져도
**이미 떠 있던 worker 프로세스는 재시작 전까지 옛 코드를 메모리에 그대로
쓴다**. 이 misfire 수정을 커밋한 게 07-22 09:19 KST였는데 worker는
07-21 09:41부터 계속 떠 있던 채였다 — 즉 수정이 파일로는 존재했지만
**단 한 번도 실행된 적이 없었다**(07-22 18:08 로그에 똑같은 "8분 25초
missed" 패턴이 재현돼 확인됨). `docker compose restart worker`로 재시작한
뒤에야 실제로 적용됐다(재시작 로그에 새 job 목록 12개 — Phase 5.14의
`intraday_compaction`도 이때 같이 뒤늦게 등록됨, 그것도 같은 이유로
미적용 상태였다).

**교훈**: **`worker` 서비스에 영향을 주는 코드(collectors/*, scheduler.py,
worker.py, live_refresh.py 등)를 고친 뒤에는 `docker compose restart
worker`를 반드시 해야 한다** — `backend`는 `--reload`가 있어 자동
반영되지만 `worker`는 아니다. 앞으로 이 카테고리 파일을 수정하면 세션
끝에 자동으로 반영됐다고 가정하지 말고 명시적으로 재시작을 확인한다.
(`backend`를 재생성/재시작하면 프런트-백엔드 도커 네트워크가 끊기는
문제와는 별개 — `worker`는 재시작해도 그 문제가 없다, HTTP를 안 서빙하기
때문.)

### Phase 5.12 — 업종·테마 트리맵 클릭 → 대장 종목 TOP10 (2026-07-22 사용자 제안)

사용자: "트리차트에서 해당 항목을 누르면 대장 종목을 찾을수 있게.. 해당
분류에서 탑10을 찾아서 넣는건 어때?" — 실측 결과 이미 이 프로젝트가 쓰는
`clients/naver_group.py`의 그룹 상세 페이지(`sise_group_detail.naver`)에
구성 종목별 데이터가 전부 있음을 확인(2026-07-22): 종목코드
(`<a href="/item/main.naver?code=131400">`)·종목명·등락률·거래대금까지
한 행에 다 나온다. 지금은 `fetch_group_value`가 거래대금 **합계**만
뽑고 개별 종목 정보는 버리고 있다 — 새 외부 소스 없이 파싱 함수만
추가하면 된다. 시가총액 컬럼은 이 소스에 없어(§4.6 3.6-3 기존 한계와
동일) "대장 종목" 기준은 **거래대금 내림차순**으로 한다.

**주의**: 그룹의 상세 페이지 조회에는 `no`(그룹 내부 ID)가 필요한데,
이건 목록 페이지(`fetch_group_snapshot`) 호출로만 얻을 수 있고 현재
`GroupSnapshot` DB 모델에도 저장되지 않는다 — 이름→no 매핑을 캐싱하거나
매 요청마다 목록을 다시 조회해야 한다.

**설계**:
1. `clients/naver_group.py`에 `fetch_group_constituents(group_type, no,
   limit=10) -> list[{"code","name","change_rate","value"}]` 신규 —
   기존 `_DETAIL_ROW_RE`/`_NUMBER_TD_RE` 재사용, 종목코드·종목명 추출
   정규식만 추가(`<a href="/item/main\.naver\?code=(\d+)">([^<]*)</a>`).
   거래대금 내림차순 정렬 후 상위 `limit`만 반환.
2. 새 라우터 `GET /api/groups/top-stocks?type=upjong&name=...&limit=10`
   — 이름→no 매핑은 인메모리 캐시(그룹 목록은 자주 안 바뀌므로 TTL
   길게, 예: 1일 또는 앱 수명 동안 1회) + 캐시 미스 시 목록 재조회.
   상세 페이지 결과는 몇 분(예: 5분) TTL 캐시 — 클릭할 때마다 새로
   불러올 필요 없고, 반복 클릭으로 소스에 과호출하지 않기 위함.
3. 프런트 — `GroupTreemap.jsx`에 박스 클릭 핸들러 추가(현재 없음, 순수
   시각화 컴포넌트) → 클릭 시 새 모달(`groupTopStocks` 타입)을 열어
   위 API를 호출, TOP10 리스트를 기존 `Top5RowTile`/`ValueRankTable`류
   패턴으로 렌더(종목명·등락률·거래대금). 행 클릭 시 기존 종목 상세
   모달로 이어지게(다른 랭킹 카드들과 동일한 관례).

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.12-1 ✅ | 백엔드 | `fetch_group_constituents` + `GET /api/groups/top-stocks` + 이름→no 캐싱 | 실제 업종/테마 이름으로 TOP10(코드·이름·등락률·거래대금) 반환 확인 — **완료(2026-07-22)** |
| 5.12-2 ✅ | 프런트 | 트리맵 박스 클릭 → TOP10 모달, 행 클릭 → 종목 상세 | 트리맵 아무 박스나 클릭 시 그 분류의 실제 대장주 상위 10개 표시 — **완료(2026-07-22)**, Playwright로 반도체와반도체장비 클릭·종목 상세 연결까지 확인 |

### Phase 5.13 — 등락 종목수 1D 상승비율 추이 그래프 (2026-07-22 사용자 요청)

사용자: "등락 종목수를 50%를 기준으로 그래프로 만들수 없나? 오늘 오르는
종목이 많은지 적은지를 라인그래프로 보는거야 시간순으로.. 그래야 투심을
정확하게 보지.. 그냥 현재 오른종목 숫자만 보면 시간흐름을 놓쳐" — 지금
"등락 종목수" 타일은 순간 스냅샷(현재 상승/하락 개수)만 보여준다. §5.4-2/
§5.7에서 이미 만든 "장중 자체 샘플링 누적" 패턴(`intraday_snapshot.py`)을
그대로 재사용한다 — 새 외부 호출 없이 이미 60초 잡이 워밍하는
`_warm_breadth_live` 결과를 그대로 적립만 하면 된다.

**지표 정의**: 상승비율 = adv / (adv + dec) × 100(코스피+코스닥 합산,
보합(flat)은 분모에서 제외 — 사용자가 "50% 기준"이라고 명시했으니 상승
대 하락만의 비율이 중립점 50%에서 자연스럽게 시작하는 게 맞다). 60초마다
한 점씩 적립.

**설계**:
1. `intraday_snapshot.py`에 `record_breadth_snapshot(payload)` 신규 —
   `_warm_breadth_live`가 반환한 값(이미 60초 잡이 fetch 완료)을 받아
   상승비율을 계산해 새 시리즈("등락비율" 등 1개 키)에 append. 장 마감이면
   스킵(`record_flow_snapshot`과 동일 원칙). `collectors/live_refresh.py`의
   60초 잡에서 `_warm_breadth_live` 호출 직후 배선(새 외부 호출 없음).
2. `get_breadth_series()` 신규 — `{"date", "series": [{"time","value"}...],
   "market_closed"}` 반환(플로우 시리즈와 동일한 모양).
3. 새 라우트 `GET /api/markets/breadth/intraday-accumulated`.
4. 프런트 — "등락 종목수" KpiTile의 상세 모달(BreadthModal)에 1D 탭 추가
   (기존 FlowSummaryModal의 1D/3M 토글 패턴 재사용). 1D 탭은 라인차트 +
   **y=50 기준선**(ReferenceLine, IntradayFlowChart의 y=0 기준선 패턴과
   동일) — 50 위/아래로 상승 우세/하락 우세가 한눈에 보이게. 값 포맷은
   억원이 아니라 "%"(기존 `eokLabel` 재사용 불가, 새 포맷터 필요).

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.13-1 ✅ | 백엔드 | `record_breadth_snapshot`/`get_breadth_series` + 라우트, live_refresh 60초 잡 배선 | 장중 60초마다 상승비율 한 점씩 실제로 쌓임 — **완료(2026-07-22)** |
| 5.13-2 ✅ | 프런트 | BreadthModal에 1D 라인차트 + 50% 기준선 | 시간순 상승비율 추이가 50% 선 기준으로 보임 — **완료(2026-07-22)**, mocked 데이터로 차트 렌더 확인(장마감 시점이라 실데이터는 빈 상태) |

### Phase 5.14 — 장중 누적 데이터 DB 영속화 + 다운샘플링 (2026-07-22 사용자 요청)

사용자: "정보를 흘려 보내는게 문제야.. 이전 정보는 볼 수 있어야 하자나...
정보에 대해서 실시간 처리 하고.. 압축해서 저장해 놓는 방법을 고안해야
할 것 같아.. 왜 아까운 정보를 흘려 보내지? 무턱대고 정보를 추가만 하면..
비대해 지니 차후 압축으로 정리해 놔야 할 것 같아.. 다른 서비스들은 어떻게
처리를 하고 있지?"

`intraday_snapshot.py`가 지금까지 순수 메모리 버퍼(재배포마다 소실, §5.6
사고 원인)였던 세 기능(투자자별 수급 1D/외인 양손 1D/등락비율 1D, §5.4-2·
§5.9-2·§5.13)을 전부 **DB 영속화 + 다운샘플링**으로 전환한다. 사용자
확인: 최근 7일은 원본(60초 틱) 그대로, 8일 전부터는 15분 단위로 압축.

**참고(다른 서비스 사례, 사용자 질문에 대한 답)**: TradingView/증권사
HTS는 최근 며칠만 1분봉을 주고 그 이상 과거는 5분봉·일봉만 준다.
Prometheus/Grafana도 최근 데이터는 원본, 오래된 건 시간당 평균으로
룰업(rollup)한다 — "최근 고해상도, 과거 저해상도" 다운샘플링이 업계
표준이다. 이 프로젝트가 이미 index_ohlcv/stock_ohlcv에 쓰는 "일봉" 개념도
근본적으로 같은 압축 원리다. docker-compose의 db 이미지가 TimescaleDB지만
이 프로젝트는 아직 hypertable 등 Timescale 전용 기능을 쓴 적이 없다 —
이번에도 일반 Postgres 테이블 + 배치 압축(plain SQL)으로 간다(더 단순하고
기존 패턴과 일관됨, Timescale 전용 기능은 필요해지면 나중에 고려).

**설계**:
1. **신규 테이블** `intraday_sample(series_key, time, value, resolution_seconds)`
   — PK `(series_key, time)`. `series_key`는 8종 고정 문자열: 투자자별 수급
   6개(`flow_kospi_개인`/`flow_kospi_외국인`/`flow_kospi_기관계`/
   `flow_kosdaq_개인`/`flow_kosdaq_외국인`/`flow_kosdaq_기관계`) + 외인선물
   1개(`futures_외국인`) + 등락비율 1개(`breadth_ratio`). "외인 양손"의
   현물(spot) 시리즈는 저장하지 않는다 — 지금처럼 `flow_kospi_외국인`+
   `flow_kosdaq_외국인`을 시간 매칭으로 합산해 조회 시점에 계산한다(§5.10
   에서 이미 검증된 방식 그대로, 중복 저장 안 함). `resolution_seconds`는
   원본 0, 압축 후 900(15분).
2. **기록부 재작성** — `intraday_snapshot.py`의 `record_flow_snapshot`/
   `record_futures_flow_snapshot`/`record_breadth_snapshot`이 이제 메모리
   append 대신 `session`을 받아 `intraday_sample`에 INSERT(+즉시 commit,
   `collectors/base.py`의 커밋 관례 참고). `_buffers`/`_buffer_date`/
   `_reset_if_new_day`/`MAX_POINTS_PER_SERIES`는 전부 제거 — 더는 메모리에
   아무 것도 안 든다.
3. **조회부 재작성** — `get_flow_series`/`get_foreign_position_series`/
   `get_breadth_series`가 이제 `session`과 `days`(기본 1 = 오늘만, 과거
   조회 시 늘림, 상한 30 정도)를 받아 DB를 쿼리한다. 반환 모양(`series`
   구조, 필드명)은 최대한 지금과 동일하게 유지해 프런트 변경을 최소화한다.
4. **다운샘플링 배치 잡** — 새 collector `collectors/intraday_compaction.py`,
   REGISTRY에 등록(18:00 배치에 같이 돎). `resolution_seconds=0`이고
   `time < now() - 7일`인 행을 `series_key`+15분 버킷으로 묶어 평균값을
   `resolution_seconds=900`로 upsert한 뒤, 방금 압축한 원본 행을 삭제한다.
   이미 압축된 구간을 다시 건드리지 않아 매일 돌아도 안전(idempotent) —
   압축 대상은 항상 `resolution_seconds=0`인 것만이므로 재실행해도 같은
   행을 두 번 압축하지 않는다.
5. **API**: 기존 3개 라우트(`/api/markets/flow/intraday-accumulated` 등)에
   `days` 쿼리 파라미터 추가(기본 1, 기존 동작 그대로 유지). `GET
   /api/markets/breadth/intraday-accumulated?days=14` 처럼 과거 조회 가능.
6. **프런트**: 세 모달(FlowSummaryModal/ForeignPositionModal/BreadthModal)의
   1D 탭에 기간 선택(예: 1일/7일/30일 토글 또는 기존 `PeriodPicker` 재사용)
   추가 — 7일 넘어가면 자동으로 15분 단위 점이 섞여 나오지만 차트는
   똑같이 그리면 된다(해상도가 달라도 문제없이 렌더되게).
...
### Phase 5.15 — 검증 기반 시장 우세 판정 + 종목 연결 (2026-07-23 사용자 전략)

**배경**: 사용자가 "지금까지 만든 게 쓸모가 없는 게 문제, 매몰비용 신경 쓰지 말고
쌓은 걸 활용하자"고 지적 — 뉴스·공시(정성적) 대신 이미 쌓인 숫자로 먼저 검증하기로
합의. `index_ohlcv`/`market_flow`에 이미 ~3년치(2023-07-18~) 일별 데이터가 있어
실제 백테스트가 가능했다. 메인 세션이 직접 쿼리로 4개 가설을 검증했고, 결과를
아래에 남긴다 — **이 수치들은 일회성이 아니라 §5.15-1의 정식 백테스트 함수로
승격해 데이터가 쌓일 때마다 재계산되게 한다.**

#### 검증 결과 (2026-07-23, n은 표본 거래일수)

1. **백워데이션(선물<현물) → 다음날 지수 하락? — 근거 없음.**
   백워데이션 162일: 다음날 평균 +0.266%, 하락확률 43.8%. 콘탱고 567일: 평균
   +0.119%, 하락확률 43.0%. 거의 차이 없음(오히려 백워데이션 쪽이 평균은 더
   높음) — §4.5-5에서 "차익 매도 유의" 배지를 근거 없다며 이미 문구를
   순화했는데(중립 계기판 원칙, 2026-07-22), 이번 실측으로 **배지 자체를
   제거해도 된다는 근거**가 생겼다.
2. **외인 선물(K200) 연속 순매수/매도일수 → 다음날 코스피 방향 — 약한 신호.**
   베이스라인(평균 +0.151%, 상승확률 56.8%, n=731) 대비 "2일 연속매도"만
   뚜렷하게 약세(평균 -0.403%, 상승확률 50.6%, n=85) — 3일+ 매도는 다시
   베이스라인 근처로 돌아옴(평균 0.001%, n=108), 매수 쪽은 베이스라인과
   큰 차이 없음. 일관성이 약해 "약한 신호"로만 채택.
3. **코스피/코스닥 × 외국인/기관 연속 순매수·매도일수 → 다음날 그 시장 방향
   — 코스닥·외국인만 뚜렷, 나머지는 노이즈.**
   - **코스닥 외국인**(베이스라인 평균 -0.006%, 상승확률 51.6%, n=731):
     3일+매수 평균 **+0.522%**(상승확률 65.2%, n=69), 1일매수 +0.225%(59.3%,
     n=182), 1일매도 -0.181%(47.0%, n=183), **2일매도 -0.463%**(41.8%,
     n=98) — 방향이 바뀔 때마다 일관되게 부호가 맞음. **가장 신뢰도 높은
     패턴**, §5.10(코스피/코스닥 분리)이 정확히 맞는 방향이었음을 실증.
   - 코스피·외국인, 코스피·기관, 코스닥·기관은 버킷 간 부호가 들쭉날쭉해
     신호로 채택하지 않는다(예: 코스피·기관 "2일매도"는 오히려 평균
     +0.554%인데 "3일+매도"는 다시 0 근처로 — 일관성 없음).
4. **스켈핑 스코어 → 실제 수익률(§5.7), 등락비율 극단값 → 되돌림(§5.13) —
   표본 부족(각각 2일치·3일치)으로 아직 결론 불가.** §5.14로 DB 영속화를
   고쳐뒀으니 앞으로 자동으로 쌓인다 — 몇 주 뒤 재검증.

#### 사용자 제안(실시간 활용 전략, 2026-07-23)

"코스피, 코스닥 현물 상황 + 선물 동향 + 수급을 보고, 지금 유입이 잘 되는
시장에서 유입이 잘 되는 종목을 찾아 거래한다" — 위 3번 검증 결과(코스닥
외국인 연속매수/매도)를 1차 필터로 삼아 "지금 어느 시장이 유리한지" 판정하고,
그 시장의 기존 종목 랭킹 카드(거래대금 상위/실시간 관심 TOP5/스켈핑 후보)로
바로 연결한다. **정직성 원칙**: 코스닥·외국인만 근거가 있고 나머지(코스피
전반)는 신호가 약하다는 걸 판정 결과에 그대로 노출한다 — 억지로 코스피도
판정해서 "있어 보이게" 만들지 않는다.

**설계**:
1. **백테스트 함수 승격** — `app/quant/regime_backtest.py`(신규): 오늘
   메인 세션이 애드혹 스크립트로 돌린 로직(스트릭 계산 + 버킷별 평균수익률/
   상승확률/표본수)을 정식 함수로 만든다. `compute_streak_buckets(session,
   market, investor) -> list[{streak_bucket, n, avg_return_pct,
   positive_rate_pct}]` 같은 시그니처. 순수 계산부는 DB 세션 인자로 받아
   언제든 재계산 가능해야 한다(§5.7 track-record와 같은 "쌓일수록 갱신"
   철학).
2. **오늘의 스트릭 계산** — 어제까지 확정된 `market_flow`(코스피/코스닥 ×
   외국인/기관) 스트릭 + 오늘 `flow/live`의 잠정 방향을 반영해 "지금 몇 일째
   연속인지" 계산.
3. **API** `GET /api/markets/regime` — 코스피/코스닥 각각의 현재 스트릭 +
   그 스트릭 구간의 백테스트 통계(1번에서 계산) + 종합 판정(코스닥우세/
   코스피우세/중립 — **코스닥·외국인 신호만 판정 근거로 쓰고 나머지는
   참고 수치로만 노출**, 판정 이유를 문장으로 설명).
4. **프런트** — 대시보드에 "지금 유입 우세" 배너/카드 신설: 판정 결과 +
   근거(스트릭 일수, 과거 상승확률·표본수) 표시. "종목 랭킹 요약" 섹션에
   판정된 시장으로 필터하는 버튼(예: "코스닥만 보기") 추가 — 거래대금
   상위/실시간 관심 TOP5/스켈핑 후보 3개 카드가 그 시장 종목만 보이게.
   **문구는 항상 관찰+확률로 서술**("코스닥 외국인 3일 연속 매수 중 —
   과거 이 구간 다음날 상승확률 65%(n=69)"), "사라", "지금이 기회" 같은
   명령형 금지(§5 전체 원칙).

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.15-1 ✅ | 백테스트 함수 | `regime_backtest.py` — 스트릭·버킷 통계 계산을 재사용 가능한 함수로 | 오늘 애드혹 결과와 동일한 수치가 함수 호출로 재현됨 — **완료(2026-07-23)**, 코스닥·외국인 baseline/버킷 수치 전부 일치 확인 |
| 5.15-2 ✅ | 오늘의 판정 API | `GET /api/markets/regime` — 현재 스트릭 + 백테스트 통계 + 판정 | 코스닥 외국인 스트릭이 실시간 반영, 코스피는 "신호 약함" 명시 — **완료(2026-07-23)**, 코스피 3일 연속매수(53.9%)로도 "코스피우세" 안 뜸 확인 |
| 5.15-3 ✅ | 프런트 배너 + 필터 연결 | "지금 유입 우세" 카드 + 종목 랭킹 3종 시장 필터 버튼 | 판정된 시장 클릭 시 3개 카드가 그 시장으로 필터링 — **완료(2026-07-23)**, 백워데이션 배지 제거 포함 |

**후속 발견(2026-07-23, 구현 검증 중)**: 테스트 스위트 자체가 실제 운영
데이터를 훼손하는 버그를 발견·수정했다 — `test_intraday_compaction.py`가
"실 데이터와 안 겹치게" 2099년을 TEST_DAY로 썼는데, `compact_intraday_samples`
가 series_key로 스코프하지 않는 전역 함수라 "2099년 기준 8일 이전"이 사실상
"모든 실제 2026년 데이터"에 걸려버렸다 — 테스트를 돌릴 때마다 그날 막 쌓인
운영 데이터(원본 60초 틱)가 조기에 15분 평균으로 압축돼버리는 사고였다(실측:
오늘 새벽 원본 32건이 이미 조기 압축됨, 되돌릴 수 없음 — 손실은 해상도
저하뿐이라 치명적이진 않으나 반복되면 안 됨). `compact_intraday_samples`에
`series_keys` 필터 파라미터를 추가해(기본 None=전체, 운영 동작 그대로) 테스트가
자기 series_key만 스코프하도록 고쳤다 — 재실행해도 실데이터 압축 건수가 더
안 늘어나는 것 확인.

### Phase 5.16 — 종목 상세: 개인 수급 "집계 중" 라벨 + 회전율 추가 (2026-07-23)

사용자 제보 2건. **① 개인 0원**: 실측(ka10059 직접 호출) 결과 오늘(2026-07-23)
날짜의 `ind_invsr`(개인)만 소스 자체가 0을 반환 — 외국인(`frgnr_invsr`)·
기관계(`orgn`)는 오늘도 실제 값이 들어온다. 과거 날짜(07-21/07-22)는 개인도
정상적으로 채워져 있어(-940,343/-109,576) 파싱 버그가 아니라 **개인 수급이
당일 중에는 소스에서 아직 집계되지 않는 구조적 특성**으로 보인다(정확한 집계
시점은 미확인 — 장마감 후인지 D+1인지). 코드로 억지로 채우지 않고, 화면에서
"진짜 0"과 "아직 집계 안 됨"을 구분되게 라벨링한다(§ 전체 원칙 "정직한 표시").
**② 회전율 부재**: 종목 상세 모달에 회전율이 아예 없다 — `value_rank` 테이블에
이미 있는 회전율(거래대금 상위 등에서 쓰는 것과 동일 계산)을 그대로 가져다
쓰면 된다(새 소스 불필요, `value_rank`는 상위 종목만 있어 없는 종목도 있을 수
있음 — 그 경우 정직하게 "데이터 없음").

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.16-1 ✅ | 개인 "집계 중" 라벨 | 오늘 날짜 + 개인 net_value=0 + 다른 투자자군은 0 아님 → "집계 중" 표시(진짜 0과 구분) | 실제 0원인 날/종목과 구분되게 렌더 — **완료(2026-07-23)**, 삼성전자로 실측 확인(개인 "집계 중" vs 외국인 1,390억/기관계 -325억) |
| 5.16-2 ✅ | 회전율 추가 | `/api/stocks/{code}/series`에 `value_rank` 최신 회전율 조인, 종목 상세 모달에 표시 | 값 있으면 %, 없으면 생략 — **완료(2026-07-23)**, 삼성전자 0.38%(07-22) 확인, value_rank 미등재 종목은 turnover:null 확인 |

### Phase 5.17 — 수급 가속도(실시간 반응성 지표) (2026-07-23 사용자 제안)

사용자: "그냥 평균값으로 좋다 나쁘다가 아니라.. 기준점에서 변동에 대한 반응성을
지표로 실시간으로 계속 뭔가 대응해 줘야 하지 않을까?" — §5.15의 "며칠 연속
매수/매도" 스트릭은 하루 단위 이산값이라 오늘 하루 안에서 매수/매도 속도가
빨라지는지 느려지는지는 전혀 못 본다. §5.14로 어제부터 쌓기 시작한 장중 60초
틱(`intraday_sample`)이 정확히 이걸 계산할 재료다. 사용자 확인: 스트릭(느리지만
검증된 신호)과 가속도(빠르지만 아직 검증 안 된 신호)를 합치지 않고 **따로**
보여준다 — 가속도는 처음엔 그냥 숫자만 보여주고(과거 기준 없음), 데이터
쌓이면 나중에 정규화(§5.15처럼 검증)를 고려한다.

**지표 정의**: `intraday_sample`의 값은 "오늘 누적 순매수"(계속 오르내리는
절대 수준, `_warm_flow_live`가 ka10051의 당일 누적치를 그대로 담아온다) —
그래서 시간 구간의 차분이 곧 그 구간의 순매수 "속도"다.
- `recent_velocity` = 지금 값 − 30분 전 값(그 구간 순매수 속도)
- `prior_velocity` = 30분 전 값 − 60분 전 값(그 이전 구간 속도)
- `acceleration` = `recent_velocity` − `prior_velocity` — **양수면 순매수 속도가
  빨라지는 중(더 강하게 사거나 덜 팔거나), 음수면 느려지는 중**(부호 일관된
  단일 스케일이라 해석이 간단하다)
- 오늘 아직 60분치 틱이 안 쌓였으면(장 시작 직후 등) "데이터 부족"으로 응답,
  억지로 계산하지 않는다.

**설계**:
1. `app/quant/regime_backtest.py`(같은 파일 재사용, 성격이 비슷함) 또는
   신규 `app/quant/flow_acceleration.py`에 `compute_flow_acceleration(session,
   series_key, now, window_minutes=30) -> dict | None` 추가 — `intraday_sample`
   에서 지금/30분전/60분전 시점에 **가장 가까운 이전 틱**(정확히 그 시각의
   틱이 없을 수 있음, `time <= target` 중 최댓값)을 찾아 위 계산. 데이터
   부족이면 None.
2. `GET /api/markets/regime` 응답에 각 조합(코스피/코스닥 × 외국인/기관계)마다
   `acceleration` 필드 추가(위 4개 재사용 — series_key는 `flow_kospi_외국인`
   등, §5.14/§5.15에서 이미 쓰는 것과 동일). 새 외부 호출 없음(DB 조회만),
   기존 60초 캐시에 그대로 얹는다.
3. 프런트 — 기존 "지금 유입 우세" 카드의 콤보 타일(4개)에 가속도 줄 추가:
   "최근 30분 순매수 +12억(직전 30분 대비 가속)" / "…(감속)" / "데이터 부족"
   중 하나. 색은 §5 원칙대로 중립 톤(등락 색과 안 겹치는 계열, 기존
   `.regime-combo-reversal` 스타일 참고) — "좋다/나쁘다" 판단 문구 금지,
   관찰 서술만.
...
### Phase 5.18 — 코스피/코스닥 쏠림 비율 추이 (2026-07-23 사용자 요청)

사용자: "외인, 기관이 적극적으로 매수를 해야 코스피, 코스닥이 오르는거 같아..
코스피로 쏠리는지 코스닥으로 쏠리는지 분포되는지도 알아야 해... 오늘은
대체로 골고루 수급이 퍼지는거 같고" + "이게 추이 분석으로 가야해 아까까진
골고루 펼쳐졌는데 점점 쏠리거나 하락추세로 변경될 수 있으니.. 그당시의
수치만 보여주면 안 된다는거야.. 추이 그래프가 있어야 해."

**실측 확인(2026-07-23 12:52 KST)**: 사용자 관찰이 맞았다 — 코스피 상승796:
하락104(87%), 코스닥 상승1314:하락367(78%)로 양쪽 다 광범위한 상승. 다만
외국인 순매수는 코스피(187.9억)에 코스닥(4.6억)보다 절대금액으로 압도적으로
쏠려있었다 — "골고루 오르지만 돈은 코스피에 더 쏠린다"는 게 동시에 사실.

**확산도(얼마나 골고루 오르는지) 추이는 이미 있다** — §5.13 "등락 종목수"
모달의 1D 추이(상승비율 50% 기준선)가 정확히 이 지표다, 재작업 불필요.
**이번엔 "코스피/코스닥 쏠림 비율" 추이만 신규**로 만든다.

**지표 정의**: 쏠림% = |코스피 활동량| / (|코스피 활동량| + |코스닥 활동량|)
× 100, 활동량 = |외국인 순매수| + |기관계 순매수|(방향과 무관하게 "어디에
돈이 몰리는지"를 보는 것이라 절댓값 사용 — 사용자가 "외인/기관이 적극
매수해야 오른다"고 했으니 이 둘을 "스마트 머니"로 묶는다). 50%=균등 분산,
100%=코스피 완전 쏠림, 0%=코스닥 완전 쏠림 — §5.13 등락비율과 동일하게
50% 기준선을 쓰는 게 자연스럽다.

**설계**: 새 저장소 불필요 — `intraday_sample`에 이미 있는
`flow_kospi_외국인`/`flow_kospi_기관계`/`flow_kosdaq_외국인`/
`flow_kosdaq_기관계` 4개 시리즈를 시간 매칭으로 조합해 조회 시점에
비율을 계산한다(§5.10의 "외인 양손 spot = kospi+kosdaq 외국인 시간매칭
합산"과 같은 패턴).

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.18-1 ✅ | 쏠림 비율 조회 함수 | `intraday_snapshot.py`에 4개 시리즈를 시간매칭해 쏠림%를 계산하는 함수 추가 | 오늘 실데이터로 정확한 쏠림% 시계열 산출 — **완료(2026-07-23)**, 228개 포인트로 95%대 코스피 쏠림 확인(외국인 수급 절대금액 비교와 방향 일치) |
| 5.18-2 ✅ | API + 프런트 | `GET /api/markets/flow-concentration/intraday-accumulated` + 1D 추이 차트(50% 기준선, `BreadthRatioChart` 패턴 재사용) | 대시보드에서 쏠림 추이가 시간순 그래프로 보임 — **완료(2026-07-23)**, "코스피/코스닥 쏠림" 카드+모달(현재/1D 추이) Playwright로 확인, 오전 내내 95~100% 쏠림 유지 추이 확인 |

### Phase 5.19 — 쏠림 비율 시총 편향 보정 (2026-07-23 사용자 지적, 5.18 직후)

사용자: "생각해보니 코스피, 코스닥 단순 쏠림 비교는 시총 차이가 너무 생겨서
코스피로만 발생할거야.. 전일 비례 코스피에서 코스피, 코스닥에서 코스닥..
쏠림 가중 차이를 알아야 해."

**문제**: §5.18 지표(`쏠림% = 코스피_활동량/(코스피+코스닥_활동량)×100`)는
절대 금액을 직접 비교한다. 코스피 시장 규모(시총·거래대금)가 코스닥보다
구조적으로 훨씬 커서, "오늘 특별히 코스피에 쏠렸다"와 "그냥 평소에도
코스피가 원래 더 크다"를 구분하지 못한다 — 평범한 날에도 90%대가 나올
가능성이 높아 변별력이 없다(실측: 오전 내내 95~100% 고정, 이게 "이례적
쏠림"인지 "원래 그런 것"인지 이 지표만으로는 알 수 없었다).

**해결**: 각 시장을 절대금액이 아니라 **"그 시장 자신의 평소 활동량 대비
오늘 몇 배인지"**로 먼저 정규화한 뒤 비교한다.

- `평소_활동량(시장)` = `market_flow`에서 최근 20거래일(확정치)의
  일별(|외국인 순매수|+|기관계 순매수|) 평균 — `regime_backtest.py`에
  `compute_activity_baseline(session, market, days=20)` 신규 추가, 기존
  `compute_baseline`(다음날 수익률용)과 나란히 같은 파일에 둔다.
- `오늘_배율(시장)` = `오늘_활동량(시장) / 평소_활동량(시장)` — 오늘_활동량은
  §5.18과 동일한 계산(intraday_sample 4개 series_key 시간매칭).
- `가중쏠림% = 코스피_배율 / (코스피_배율 + 코스닥_배율) × 100` — 50%=양쪽
  다 "평소 대비" 똑같이 늘거나 줄었다, 코스피_배율이 상대적으로 더 크면
  "코스피가 평소보다 유독 더 활발"이라는 뜻.
- **단위가 달라도 무관**: `intraday_sample`(오늘 활동량, 단위 A)과
  `market_flow`(평소 활동량, 단위 B)가 다른 단위여도 상관없다 — 단위 배율은
  두 시장에 동일하게 곱해지는 공통 상수라 비율(분자/분모) 계산에서 그대로
  약분된다. 정확한 단위 일치를 신경 쓸 필요가 없다는 뜻(수학적으로 증명
  가능 — 구현 시 주석으로 남긴다).
- 베이스라인이 없으면(market_flow에 해당 시장 데이터가 전혀 없음, 실전에선
  거의 발생 안 함 — 3년치 있음) 그 틱은 건너뛴다(§5.18과 동일하게 "억지로
  채우지 않는다" 원칙 유지).
- 7일/30일 조회에서도 베이스라인은 "지금 시점 최근 20거래일" 하나로
  통일해서 전체 구간에 적용한다(과거 시점마다 그때의 20일 베이스라인을
  다시 계산하는 것보다 단순하고, 베이스라인이 몇 주 사이 급변하지 않는다는
  전제하에 실용적으로 충분하다 — 정밀도보다 단순성 우선).

**중복 계산 방지**: `GET /api/markets/regime`이 이미 60초 TTL로 시장별
루프를 돌며 `compute_baseline`을 호출하고 있으니(routers/markets.py
`_warm_regime`), 같은 루프에서 `compute_activity_baseline`도 같이 호출해
`kospi`/`kosdaq` 응답 안에 `activity_baseline` 필드로 얹는다 — 프런트가
이미 1분 티어에서 폴링 중인 `regime`을 그대로 재사용해 KPI 타일/모달
"현재" 탭 계산에 쓸 수 있다(새 API 호출 없음, §5.18의 "이미 fetch한 값
재사용" 관례 유지). `flow-concentration/intraday-accumulated` 엔드포인트는
자체적으로 `compute_activity_baseline`을 한 번 호출해 1D 시계열 계산에
쓴다.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.19-1 | 활동량 베이스라인 함수 | `regime_backtest.py`에 `compute_activity_baseline(session, market, days=20)` 추가 — `market_flow`에서 최근 20거래일 (|외국인|+|기관계|) 일별 합의 평균 | 실 DB로 코스피/코스닥 각각 평소 활동량(억원 단위)이 산출되고, 코스피 평소값이 코스닥 평소값보다 몇 배 큰지(=시총 편향의 크기) 확인 |
| 5.19-2 | regime 응답에 얹기 | `_warm_regime`의 market 루프에서 `compute_activity_baseline` 같이 호출, `kospi`/`kosdaq` 딕셔너리에 `activity_baseline: {"n", "avg_daily_activity"}` 필드 추가 | `GET /api/markets/regime` curl로 두 시장 모두 activity_baseline 필드 확인 |
...
### Phase 5.21 — K200 선물 일봉에 오늘 잠정치 봉 추가 (2026-07-23 사용자 지적)

사용자: "선물 (코스피200) · 캔들차트 이건 실시간 1D는 안되는건가? 그리고
몇일째 후일 차트로 보이는데..?"

**조사 결과(재조사 아님, 이미 있는 결론 재확인)**: K200 선물은 분봉 소스가
없다(§5.1에서 이미 네이버 여러 엔드포인트를 탐색했지만 실패 — 프런트
`CandleModal`이 이미 `market==='futures'`면 분봉 토글을 비활성화하고
"K200 선물은 분봉 데이터 소스가 없습니다" 툴팁을 보여주는 것도 그 결론의
반영이다, 재작업 대상 아님).

**실제 증상 진단**: DB(`index_ohlcv`)를 직접 확인한 결과 코스피/코스닥/선물
세 시장 모두 최신 날짜가 동일(어제, 정상 — 일봉 배치가 18시에 돈다)했다 —
데이터 파이프라인 자체는 멈춰있지 않다. 사용자가 "며칠째 멈춘 것처럼
보인다"고 느낀 진짜 원인은 **두 증상이 하나의 원인에서 나온다는 것**이다:
코스피/코스닥은 일봉이 어제까지만 있어도 분봉 탭으로 전환하면 오늘 실시간
움직임이 보이지만, **선물은 분봉이 없어 일봉이 유일한 뷰라서 매번 볼 때마다
"어제 그대로"**로 보인다 — 실제로 안 멈췄지만 유독 정체돼 보이는 구조적
문제.

**해결 방향(사용자 확인)**: 완전한 실시간 분봉은 여전히 불가능하지만(소스
자체가 없음), **일봉 차트 맨 끝에 "오늘 잠정치" 봉 하나를 추가로 보여줘서
멈춘 느낌을 없앤다**. 이미 `routers/basis.py::_warm_basis_live`가 베이시스
계산을 위해 `clients/naver_index.py::fetch_index_series(FUTURES_MARKET, ...)`
로 "오늘" 행(`futures_row`)을 이미 가져오고 있다 — 이게 2026-07-20 장중
실측으로 **체결마다 갱신되는 진짜 장중 캔들**임을 이미 확인해 뒀다(basis.py
모듈 docstring "GET /api/markets/basis/live" 절 참고, live_refresh.py 60초
잡이 이미 이 캐시를 선제적으로 채우는 중이라 새 외부 호출 없음). 지금은
`futures_row`에서 `close`만 꺼내 베이시스 계산에 쓰고 나머지(open/high/low/
volume)는 버리고 있었다 — 이걸 그대로 노출하면 새 소스 없이 끝난다.

**설계**:
1. `basis.py::_warm_basis_live` payload에 `futures_today`
   `{date, open, high, low, close, volume}`(이미 fetch한 `futures_row`
   그대로) 필드 추가 — 새 외부 호출 없음.
2. `routers/markets.py::_build_prices`(또는 `market_series` 핸들러)가
   `market == "futures"`이고 장 마감이 아니면 `basis._warm_basis_live()`를
   호출해 `futures_today`를 받는다. 그 날짜가 DB `prices`의 마지막 행
   날짜보다 **더 최신**일 때만(EOD 배치가 아직 그 날짜를 안 채웠다는 뜻)
   `prices` 끝에 합성 행을 추가한다 — `changeRate`는 기존 방식(직전 종가
   대비)과 동일하게 계산, `"provisional": true` 플래그를 얹어 확정치와
   구분한다. 이미 DB에 그 날짜가 있으면(배치 완료 후) 추가하지 않는다(중복
   방지).
3. 코스피/코스닥은 건드리지 않는다 — 이 문제는 선물에만 있다(분봉 대안이
   있어 필요 없음).
4. 프런트 `CandleModal`/`CandleChart`: 마지막 행이 `provisional`이면 차트
   아래 "마지막 봉은 장중 잠정치 · 체결마다 갱신, 확정 아님" 안내 문구를
   추가한다(§5 "관찰 사실만" 원칙 — 확정치인 것처럼 보이면 안 됨). 캔들 자체
   색상 구분까지는 과설계이므로 하지 않는다(텍스트 안내로 충분).

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.21-1 ✅ | basis/live에 오늘 선물 OHLCV 노출 | `_warm_basis_live` payload에 `futures_today` 추가(새 외부 호출 없음, 이미 fetch한 값 재사용) | **완료(2026-07-23)** — 단위테스트로 3개 분기(라이브 성공/마감-캐시없음/마감-캐시재사용 carry-over) 전부 확인. 검증 시점이 마침 15:30 장마감 직후라 실시간 curl 시연은 다음 거래일 장중에 가능(로직은 테스트로 검증 완료) |
| 5.21-2 ✅ | 선물 일봉에 잠정치 합성 행 추가 | `market_series`가 `market=="futures"`일 때 `futures_today`를 조회해 DB 마지막 행보다 최신이면 `provisional:true` 행을 append | **완료(2026-07-23)** — 단위테스트로 확인(신규 행 추가/중복 스킵/장마감 스킵/라이브 실패시 우아한 축소/코스피코스닥 영향없음 5종), 462 passed |
| 5.21-3 ✅ | 프런트 안내 문구 | `CandleModal`이 마지막 가격 행의 `provisional`을 보고 안내 문구 표시 | **완료(2026-07-23)** — 코드 리뷰 + `vite build` 클린 확인(브라우저 자동화 미가용) |

### Phase 5.22 — 미국장 지표 아침 조기 수집 (2026-07-24 사용자 지적)

사용자: "나스닥, 다우존스 같은 어제자 정보가 안맞는거 같아.. 정보 갱신에서
오전에 8시 이전에 필수로 해야 할 게 있을거 같은데.. 뭔가 안맞음."

**진단(실측)**: `macro_series` DB를 직접 조회한 결과, 2026-07-24(오늘, 오전
7시대) 시점 기준 나스닥/다우/S&P500/SOX 전부 최신 날짜가 **07-22**에 멈춰
있었다 — 07-23(엊그제 KST 새벽에 이미 마감된 미국장) 데이터가 아직 없었다.
원인은 타임존 계산: 미국 정규장은 한국시간 기준 밤 22:30~23:30에 열려
다음날 새벽 05:00~06:00에 마감한다(서머타임에 따라 ±1시간) — 즉 "어젯밤
미국장"은 한국시간 새벽에 이미 완전히 끝나 있다. 그런데 이 프로젝트의 유일한
배치 스케줄은 평일 **18:00** 한 번뿐이라(`collectors/scheduler.py`
`_run_all_jobs`), 이미 새벽에 끝난 미국장 데이터를 그날 저녁 18시에야 처음
수집한다 — 사용자가 아침에 대시보드를 열면, "어젯밤 미국장이 어땠는지"를
정작 그날 저녁까지 알 수 없는 구조였다. 사용자의 "8시 이전에 뭔가 해야
한다"는 직관이 정확했다.

**해결**: 미국장 데이터(`collectors/macro.py::collect_macro` — 환율/유가/
미국지수 4종/KOFIA 전부 한 잡으로 묶여 있음, `REGISTRY["macro"]`)는 이미
`LOOKBACK_DAYS=10` 창 + upsert로 **하루 두 번 실행해도 안전**(idempotent,
모듈 docstring 그대로) — 새 잡을 만들 필요 없이 **같은 `collect_macro`를
아침에 한 번 더 돌리는 두 번째 cron만 추가**하면 된다. 시각은 미국장 마감
(새벽 05~06시, 서머타임 변동 감안)보다 넉넉히 늦고 NXT 프리마켓 시작(08:00)
보다는 이르게 **07:30 KST**로 잡는다 — 그러면 사용자가 아침에 대시보드를 볼
때 이미 전날 미국장 확정치가 반영돼 있다. `misfire_grace_time`은 §5.11에서
이미 겪은 사고(그레이스 없으면 조용히 스킵)를 반복하지 않도록 기존 18시
잡과 동일하게 넉넉히(3600초) 준다.

**KOFIA(예탁금/신용융자/대차잔고)는 같은 잡 안에 있지만 문제 없음** —
한국 시장 통계라 07:30에 다시 불러도 최신 확정치가 아직 그대로일 뿐(전날
18시에 이미 받은 값과 동일), 중복 조회가 무해하다(idempotent upsert).
굳이 이 잡을 쪼개서 US지수만 분리할 필요는 없다 — 지금 이대로도 안전하고,
분리하면 오히려 관리할 잡이 하나 더 늘 뿐이다.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.22-1 ✅ | 아침 07:30 macro 재실행 cron 추가 | `collectors/scheduler.py`에 `_run_macro_job()`(REGISTRY["macro"]만 `run_job`으로 실행) 신규 함수 + `CronTrigger(day_of_week="mon-fri", hour=7, minute=30, timezone="Asia/Seoul")` + `misfire_grace_time=3600`으로 두 번째 `add_job` 등록 | **완료(2026-07-24)** — `docker compose restart worker` 후 실 로그로 "weekday 18:00... + weekday 07:30 Asia/Seoul macro-only catch-up" 두 스케줄 모두 등록 확인, 465 passed(신규 3개: 07:30 잡 등록/misfire_grace_time, macro만 단독 실행 확인, REGISTRY 누락시 안전 스킵) |

### Phase 5.23 — 코스피/코스닥 쏠림 비율 백테스트 (2026-07-24 사용자 요청)

사용자: "그러면 50% 기준으로 코스닥이 좋은지 코스피가 좋은지 표시 해주면
되는거 아닐까?" → 내가 "이 지표는 검증 안 됐다(§5.15 백워데이션처럼 근거
없이 '좋다'라고 하면 안 됨)"고 답하자 → "그거 백테스트 해보자."

**가설**: §5.18/§5.19의 "쏠림 비율"(코스피_배율/(코스피_배율+코스닥_배율),
배율=오늘활동량/최근20거래일 평소활동량, 활동량=|외국인|+|기관계| 순매수)이
50%를 넘겨 코스피 쪽으로 쏠린 날, 다음날 실제로 코스피가 코스닥보다 더 잘
가는지(또는 코스피 자체가 상승하는지) — 반대로 코스닥 쪽 쏠림일 때도 대칭.
검증되면 §5.15 방식 그대로 "우세" 판정에 반영, 안 되면(백워데이션과 같은
결말이면) 지금처럼 관찰 서술("코스피 쪽 활동이 더 많다")만 유지.

**방법론(§5.15와 동일한 엄밀함)**: `market_flow`(3년치)로 날짜별 쏠림%를
과거 전체 기간 재구성한다 — **미래참조(lookahead) 없이** 각 날짜 D의
베이스라인은 D 이전(D 미포함) 20거래일 평균만 사용한다(라이브 버전은
"현재" 기준 20일을 쓰지만 매 순간 다시 계산되므로 이 롤링 방식이 라이브
동작을 그대로 재현하는 것과 같다). 계산된 쏠림%를 4구간(코스피강쏠림
≥60%/코스피약쏠림 50~60%/코스닥약쏠림 40~50%/코스닥강쏠림 <40%)으로 나눠,
각 구간의 "다음날" 코스피 수익률·코스닥 수익률·상대수익률(코스피-코스닥)
평균과 표본수를 `regime_backtest.py`의 `_daily_returns`를 그대로 재사용해
계산한다(§5.15와 같은 "다음날 수익률" 정의 공유, 로직 이원화 방지).

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.23-1 ✅ | 백테스트 함수 구현 | `app/quant/concentration_backtest.py` 신규(regime_backtest.py 패턴) — 날짜별 쏠림% 재구성(롤링 20일 베이스라인, 미래참조 없음) + 4구간 버킷별 다음날 코스피/코스닥/상대 수익률 통계 | **완료(2026-07-24)** — 단위테스트 7개(롤링 베이스라인 미래참조 없음 포함) + 472 passed, 실 DB(2023-08~2026-07, 766일)로 4구간 전부 n=118~236 실제 수치 산출 |
| 5.23-2 ✅ | 결과 해석 + 반영 여부 결정 | 메인 세션이 실제 백테스트 수치를 검토해 신호가 있는지 판단 | **완료(2026-07-24) — 신호 없음, 반영 안 함(백워데이션과 같은 결말)**. 아래 실측 결과·판단 근거 참고 |

**실측 결과(2026-07-24, `compute_concentration_buckets` 실 DB 실행)**:

| 버킷 | n | 코스피 다음날 평균수익률 | 코스닥 다음날 평균수익률 | 상대수익률(코스피-코스닥) | 코스피가 다음날 우세할 확률 |
|---|---|---|---|---|---|
| 코스피강쏠림(≥60%) | 225 | +0.016% | -0.208% | +0.224%p | 58.7% |
| 코스피약쏠림(50~60%) | 118 | +0.144% | -0.049% | +0.194%p | 61.9% |
| 코스닥약쏠림(40~50%) | 132 | -0.017% | -0.268% | +0.251%p | 57.6% |
| 코스닥강쏠림(<40%) | 236 | +0.406% | +0.389% | +0.017%p | 49.6% |

**판단 — 신호 없음, 채택 안 함**: 가설대로라면 코스피 쪽으로 쏠릴수록(왼쪽 버킷)
"코스피가 다음날 우세할 확률"이 단조 증가하고, 코스닥 쪽으로 갈수록(오른쪽 버킷)
50% 밑으로 떨어져야 한다. 실제로는:
- **단조성이 없다** — "코스피약쏠림"(61.9%)이 오히려 "코스피강쏠림"(58.7%)보다
  높다. 쏠림이 강할수록 신호도 강해져야 하는데 거꾸로다.
- **정반대 구간이 있다** — "코스닥약쏠림"(코스닥 쪽으로 쏠린 날)인데도 코스피가
  다음날 우세할 확률이 57.6%로 오히려 "코스피강쏠림" 구간과 비슷하다. 가설이
  맞다면 이 구간은 50% 밑으로 가야 한다.
- **상대수익률도 같은 패턴** — 코스피 우위(+0.017%p~+0.251%p)가 4구간 모두에서
  나타나고 쏠림 방향과 무관하게 비슷한 크기다. 이건 "쏠림 신호"가 아니라
  그냥 코스피가 평균적으로 코스닥보다 변동성이 낮아 생기는 배경 편향으로 보인다
  (코스닥강쏠림 구간의 코스피 평균수익률 +0.406%가 4구간 중 가장 높다는 것도
  같은 정황 — 가설대로면 이 구간에서 코스피가 가장 부진해야 하는데 반대다).

§5.15의 백워데이션(43.8% vs 43.0%)과 같은 결론 — **겉보기엔 그럴듯해도 실제
검증하면 예측력이 없다**. 그래서 이 쏠림 비율 지표는 §5.18/§5.19의 현재 표현
그대로(모달의 "코스피 쪽 활동이 더 많다"처럼 관찰 서술만, "우세"/"좋다" 판정
언어 추가 안 함) 유지한다. `concentration_backtest.py`는 삭제하지 않고 남겨둔다
— 나중에 다른 조합(예: 방향성 있는 순매수로 재정의, 더 긴/짧은 베이스라인
...
### Phase 5.25 — ETF 비중 변화 감지 (2026-07-24 사용자 요청, 알테오젠 사례)

사용자: "알테오젠이 수급을 받는거 같아... 외인, 기관은 팔고 있는데.. 개인은
사고 있어.. 개별종목을 사는게 아니라 ETF로 사는거 같아.. 아무래도 ETF
비중이 바뀐거 같아.. active도 있잖아.. 이거 확인 좀 해줘봐. 그리고 이렇게
바뀐게 있는지 감별도 하자."

**알테오젠(196170) 직접 확인 결과(2026-07-24, 실 DB 조회)**:
- **투자자별 직접 수급은 사용자 설명과 다르다** — `stock_flow`(ka10059)로
  보면 07-23 확정치는 외국인 +39,769(매수) · 기관계 +20,531(매수) · 개인
  -60,818(매도)로 **오히려 외인·기관이 사고 개인이 판** 날이었다. 오늘(07-24
  장중)은 외국인 +3,732(매수) · 기관계 -1,246(소폭 매도) · 개인은 항상
  0(§5.16에서 이미 확인한 대로 장중 개인 수치는 실시간 집계가 안 되는
  소스 특성 — 실제 매수/매도 여부를 이 수치로 알 수 없음). "외인·기관
  팔고 개인 산다"는 설명과는 안 맞는다 — 아마 다른 화면(가격/거래량
  패턴)에서 받은 인상일 수 있다.
- **ETF 비중 변화는 사용자 관찰이 실제로 맞다** — `etf_holdings`(일별 top10
  구성 스냅샷, 07-18/21/22/23 4개 날짜 보유) 조회 결과, 알테오젠을 담은
  **액티브 펀드 다수가 최근 며칠 사이 비중을 꾸준히 늘리고 있었다**:
  - RISE 바이오TOP10액티브: 8.75%→9.09%→9.09%→**9.40%**
  - TIGER 기술이전바이오액티브: 8.00%→8.34%→8.34%→**8.60%**
  - KoAct 바이오헬스케어액티브: 7.56%→7.85%→7.85%→**8.61%**(07-23 하루에
    +0.76%p 급등)
  - TIME K바이오액티브: 9.74%→10.04%→10.04%→**10.20%**
  - **ACE K바이오코스닥액티브: 07-23에 처음 등장(9.07%)** — 이전 3개
    스냅샷엔 알테오젠이 이 펀드 top10에 없었다(신규 편입 가능성, 다만
    top10 스냅샷이라 "이전에도 보유했지만 top10 밖이었다가 올라온 것"과
    "진짜 신규 매수"를 이 데이터만으로 완전히 구분할 순 없음).
  - 반대로 KODEX 헬스케어(266420)는 07-21(9.84%) 이후 스냅샷에서 사라짐
    (top10 이탈 — 매도인지 다른 종목이 더 커져 순위 밀림인지는 불명).
  - `via_etf_net`(§4.5 flow_path)도 07-23 +3,360으로 양수 — 다만 그날
    외국인+기관 직접 매수(+60,300)에 비하면 작은 비중이라, "그날의
    주된 매수 주체"는 ETF 경유보다는 직접 매수 쪽이었다(과대해석 주의).

**"감별" 기능 설계**: `etf_holdings`가 이미 매일 스냅샷을 쌓고 있으니(새
수집 불필요), 최근 2개 스냅샷 날짜를 비교해 종목별·ETF별 비중 변화를
분류하는 스크리너를 만든다.

- **이벤트 분류**: 신규편입(이전 top10에 없었다가 등장) / 비중확대(있었고
  비중 증가, 임계값 이상만) / 비중축소(있었고 비중 감소) / 편출(있었다가
  사라짐). **"top10 스냅샷"이라는 한계를 항상 명시**한다 — 신규편입/편출이
  "매수/매도 시작"이 아니라 "top10 안/밖으로 들고남"일 수 있다는 걸
  숨기지 않는다(§5 "관찰 사실만 서술" 원칙, flow_path.py가 이미 쓰는
  것과 동일한 정직한 한계 고지 관례).
- **액티브 펀드 식별**: `stocks.name`에 "액티브" 포함 여부로 판정(전체
  ETF 358개 중 65개, 실측 확인 — 한국 ETF 네이밍 규정상 액티브 펀드는
  이름에 "액티브"를 반드시 포함해야 해서 신뢰할 수 있는 판별 기준). 액티브
  펀드의 변화는 펀드매니저 재량(종목 선택)을 반영할 가능성이 커서 패시브
  (지수 추종) ETF의 기계적 리밸런싱과 구분해 표시한다.
- **범위**: 시장 전체 스크리닝(어떤 종목이 최근 ETF 비중 변화를 겪었는지
  찾기, 사용자의 "감별" 요청 그대로) + 종목 지정 조회(특정 종목, 예:
  196170을 담은 ETF들의 변화만) 둘 다 하나의 엔드포인트에서 옵션 파라미터로
  처리한다(엔드포인트 두 개로 나누지 않음, 과설계 방지).

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.25-1 ✅ | 비중 변화 계산 함수 | `app/quant/etf_weight_changes.py` 신규 — 최근 2개 `etf_holdings` 스냅샷 날짜를 찾아 종목×ETF별 비중 변화를 신규편입/비중확대/비중축소/편출로 분류, `is_active`(이름에 "액티브" 포함) 플래그, 임계값 미만 변화는 제외 | **완료(2026-07-24)** — 실 DB로 재검증: 196170(알테오젠) 조회 결과 RISE 바이오TOP10액티브 비중확대 +0.31%p, KoAct 바이오헬스케어액티브 +0.76%p, ACE K바이오코스닥액티브 신규편입(9.07%) 등 손계산과 정확히 일치, 497 passed |
| 5.25-2 ✅ | API 엔드포인트 | `GET /api/markets/etf-weight-changes`(옵션: `code`=특정 종목 필터, `active_only`=액티브 펀드만, `event`=이벤트 타입 필터, `limit`) | **완료(2026-07-24)** — curl로 `code=196170` 조회 시 위 손계산 결과와 일치, 시장 전체 스크리닝(레버리지 ETF들의 큰 비중 변화 등 실데이터) 확인, 잘못된 `event` 값 400 확인 |
| 5.25-3 ✅ | 프런트 반영 | 종목상세 모달에 "이 종목 담은 ETF 중 최근 비중 변화" 섹션 추가(해당 종목 조회 시), 대시보드에 "ETF 비중 변화 상위" 카드/모달 신규(시장 전체 스크리닝) | **완료(2026-07-24)** — 코드 리뷰(top10 스냅샷 한계 문구가 화면에 노출됨, 상승=빨강/하락=파랑 색 규칙 적용 확인) + `vite build` 클린 확인 |

### Phase 5.26 — ETF 비중 변화 ↔ 실제 수급/가격 연관 분석 (2026-07-24 사용자 요청)

사용자: "ETF 비중 변화 상위가 의미가 있으려면 단순 변화만 비교하는게
아니라 수급에 의해서 비중이 변화한게 결국 가격 변동에 효과가 있는지
알아야해.. 하이닉스의 경우 어떤건 비중이 있고 어떤건 내리고 하잖아..
비중에 의해서 수급이 붙고 외인,기관과 같은 수급에 의해서 움직이는지
관계를 따져야 해.. 더 의미 있는 상관·연관 관계를 통해서 분석기를
만들어야 해." → SK하이닉스(000660) 실측 확인 결과, "KODEX
반도체레버리지"의 비중이 하루 만에 46.87%→53.41%(+6.54%p)로 급등한 사례가
있었는데, 레버리지 ETF는 매일 목표 배율(2배)을 유지하려고 기계적으로
리밸런싱하므로 이 변화가 "의미 있는 자금 유입"이 아니라 **레버리지
구조 자체의 기계적 효과**일 가능성이 크다는 게 사용자 우려의 핵심 사례로
확인됨(추적 358개 ETF 중 48개가 레버리지/인버스).

**중대한 데이터 제약(정직하게 인정)**: `etf_holdings`(ETF 구성 스냅샷)는
현재 **4일치(07-18/21/22/23)뿐**이다 — §5.15/§5.23처럼 "비중 변화 →
다음날 수익률" 관계를 통계적으로 검증하려면 최소 수십~수백 일치가
필요한데, 지금은 비교 가능한 날짜 쌍이 3개뿐이라 **지금 당장 통계적으로
의미 있는 백테스트는 불가능하다**. 여기서 억지로 "상관관계 있음/없음"을
결론 내리면 §5.23(쏠림 백테스트)이 경계했던 것과 같은 실수 — 표본이
너무 적어 신호와 노이즈를 구분할 수 없는 상태에서 판정하는 것 —
반복이다. 사용자에게 이 제약을 명시하고 "둘 다" 진행하기로 확인:
(A) 지금 당장 가능한 것(레버리지/인버스 구분 + 종목별 실제 수급·가격 변동
나란히 보여주기 — 통계적 검증은 아니지만 대시보드에서 바로 판단 가능한
근거 자료), (B) `etf_holdings`가 몇 주~몇 달 쌓인 뒤 재실행할 수 있는
백테스트 인프라를 지금 미리 만들어 둔다(regime_backtest.py/
concentration_backtest.py와 같은 위치·패턴).

**(A) 스크리너 보강**: `etf_weight_changes.py`의 각 변화 행에 두 가지를
추가한다 — ① `is_leveraged`(이름에 "레버리지" 또는 "인버스" 포함 — §5.25의
`is_active` 판정과 동일한 이름 기반 휴리스틱 방식, 358개 중 48개 확인),
② 그 종목의 같은 기간(prev_date~curr_date) 실제 수급(`stock_flow`의
외국인+기관계 net_value 합, 65일치 축적됨 — §5.20이 이미 쌓아 둔 데이터
재사용)과 가격 변동률(`stock_ohlcv`의 종가 기준, 107일치 축적됨). 새
엔드포인트를 만들지 않고 기존 `GET /api/markets/etf-weight-changes`
응답에 필드를 추가한다(엔드포인트 증식 방지). `exclude_leveraged` 쿼리
파라미터(기본 False — 숨기지 않고 플래그로 노출, `active_only`와 동일한
관례)로 필터링 옵션만 추가한다.

**(B) 백테스트 인프라(결과는 아직 신뢰 구간 밖임을 명시)**: 신규
`app/quant/etf_weight_backtest.py` — `etf_holdings`에 존재하는 **모든**
연속 날짜 쌍(현재는 3쌍뿐)에 대해 종목별 비중변화 합계(레버리지 제외
옵션) vs 그 종목의 실제 수급 변화·가격 변동률의 상관계수를 계산하는
함수. 표본수(n)를 항상 결과에 포함하고, 호출부(관리자 전용, 대시보드
카드 아님 — 표본 부족 상태에서 일반 사용자에게 "상관관계"를 보여주면
과신을 유발하므로 이번 단계에서는 노출하지 않는다)가 "n이 통계적으로
의미 있는 최소치(예: 30) 미만이면 참고용 이하로도 취급하지 말 것"을
docstring에 명시한다. `etf_holdings`가 충분히 쌓인 뒤 재실행해 §5.15/
§5.23처럼 정식 결과 판정을 내리는 건 별도 후속 작업으로 남긴다.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.26-1 ✅ | 스크리너에 레버리지 플래그 + 실제 수급/가격 컨텍스트 추가 | `etf_weight_changes.py`에 `is_leveraged` 판정, `stock_flow`/`stock_ohlcv` 조회로 각 변화 행에 `stock_flow_delta`·`price_change_pct` 추가, `exclude_leveraged` 파라미터 | **완료(2026-07-24)** — 실 DB 재검증: 하이닉스(000660) "KODEX 반도체레버리지" 행 `is_leveraged: true`, `stock_flow_delta`/`price_change_pct` 값을 raw SQL로 교차검증해 정확히 일치(1,242,934원, +5.41%) 확인, 510 passed |
| 5.26-2 ✅ | 백테스트 인프라 함수 | `app/quant/etf_weight_backtest.py` 신규 — 모든 연속 스냅샷 쌍에 대해 종목별 비중변화 vs 실제 수급/가격 변동 상관계수 계산, n 항상 노출, 표본 부족 경고 docstring 명시 | **완료(2026-07-24)** — 실 DB 재실행 결과 `n_date_pairs=3, n_stock_observations=995, correlation_weight_vs_flow=0.197, correlation_weight_vs_price=0.109, reliable=true`. **중요 발견**: `n_stock_observations`가 995로 30을 넘어 `reliable=true`가 나오지만, 실제 독립 표본 단위는 `n_date_pairs`(3)이고 995는 같은 3개 날짜쌍을 종목 단위로 쪼갠 횡단면 유사상관(cross-sectional pseudo-replication)이라 신뢰할 수 없음을 모듈 docstring에 명시 — 라우터/UI 미배선(의도) |

### Phase 5.27 — 스켈핑 후보 가격제한폭 근접/과도한 하락 처리 (2026-07-24 사용자 지적)

사용자: "스켈핑 후보에서 하한가, 상한가에 근접한 놈도 있는데 이게 추천대상에
맞을까? 1. 움직임에 대한 Room이 없음 2. 하한가 혹은 마이너스 폭이 큰 녀석은
위험한 상태로 볼 수 있음. 기타.. 근거 이유가 부족한 상태로 추천하는거 같음."

**실측 확인**: `/api/markets/scalp-candidates` 실 데이터에서 정확히 이 문제가
보였다 — 1위 "에이치엘지노믹스"가 등락률 **-24.60%**(회전율 279.66%, 사실상
거래 폭주하며 무너지는 종목)로 스코어 1위, "일승"은 **+29.86%**로 이미
사실상 상한가(KRX 가격제한폭 ±30%, 틱 단위 반올림 때문에 정확히 30.00%가
아니라 29.86%처럼 찍힘)에 도달해 있었다.

**문제 진단**: `quant/screener.py`의 `change` 요소가 `abs(change_rate)`를 써서
"변동성 크기 자체가 기회"로 취급한다(§5.2 원 설계 의도 — 방향 무관하게 스켈핑
기회로 본다는 전제). 이 전제가 두 가지 경우에 깨진다:
1. **가격제한폭(±30%) 근접/도달**: 그 방향으로는 오늘 더 움직일 "room"이
   물리적으로 없다(상한가 도달 시 매수 체결 자체가 막히고, 하한가는 매수
   호가가 사라져 유동성이 증발하는 경우가 흔함) — 스켈핑(당일 변동성 포착)의
   전제 자체가 성립하지 않는다.
2. **큰 폭 하락**은 상승과 "변동성 크기"는 같아도 리스크 성격이 다르다 —
   악재/유동성 위기로 인한 붕괴일 가능성이 높아 "기회"보다 "위험"에 가깝다.
   `abs()` 하나로 뭉뚱그리면 이 구분이 사라진다.

**해결**:
1. **가격제한폭 근접 종목은 후보에서 아예 제외**(스코어 조정이 아니라 구조적
   배제 — ETF 제외와 같은 카테고리의 "애초에 후보 자격이 없음" 처리, §5.2
   "ETF는 제외" 원칙과 동일선상). 임계값은 실측 데이터(29.86%가 실질적으로
   상한가)에 맞춰 `abs(change_rate) >= 29.5`로 잡는다(정확히 30.00%만
   걸러내면 틱 반올림으로 실질 제한폭 종목을 놓친다).
2. **큰 폭 하락(제한폭까지는 아님)은 배제하지 않되 별도 플래그로 노출** —
   `change_rate <= -15.0`이면 `at_risk`(가칭) 플래그를 응답에 추가해 프런트가
   "하락폭 과다 · 참고" 배지로 보여준다. 스코어 산식 자체(`abs(change_rate)`
   유지)는 건드리지 않는다 — 과설계 방지, 이미 검증 안 된 가중치(§5.20 comment
   그대로)를 또 손대는 대신 투명한 플래그로 사용자가 직접 판단하게 한다.
3. 두 임계값 모두 모듈 상수로 빼서 나중에 조정 가능하게 한다.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.27-1 ✅ | 가격제한폭 근접 종목 후보 제외 | `routers/scalp.py`의 후보 구성 단계(`_scored_candidates`)에서 `abs(change_rate) >= PRICE_LIMIT_THRESHOLD_PCT`(29.5) 종목을 후보 리스트에서 제외(ETF 제외와 동일한 필터링 단계) | **완료(2026-07-24)** — 실시간 데이터 재검증: value-rank/live엔 "일승"(+29.86%)·"코오롱티슈진"(-29.93%)이 있지만 scalp-candidates 응답에는 0건, 정상 제외 확인 |
...
### Phase 5.30 — K200 선물 미결제약정(open interest) 조사 (2026-07-26, 외부 강의 검토 두 번째)

외부 강의 검토 아이디어 두 번째: "외국인 선물 순매도가 3천 계약인데
미결제약정은 5천 계약 늘었다면, 단순 매도가 아니라 저가 매수(신규
포지션)도 섞여 있다"는 식으로 순매수/매도 수치와 미결제약정 증감을
같이 봐야 신규 진입인지 청산인지 구분할 수 있다는 조언. 우리 시스템은
지금 외국인 선물 "순매수 금액"만 보고 미결제약정 자체는 전혀 수집하지
않는다 — 완전히 새로운 데이터 축.

**선행 리스크(이미 알고 있는 사실, 재확인 불필요)**: 이 프로젝트는 이미
"키움 REST에는 애초에 선물 도메인이 없다"는 걸 K200 선물 분봉 조사에서
확인한 바 있다(§5.1, `/api/markets/futures/intraday`가 501을 반환하는
근거). 그래서 미결제약정도 키움에 없을 가능성이 있다 — 이번 작업은
**조사 우선**이다. 조사 결과 데이터가 없으면(키움도 네이버도) 그 자체가
유효한 결론이고, 억지로 구현하지 않는다(§5.1의 "K200 선물 분봉 소스
없음" 결론과 동일한 정직한 dead-end 문서화 원칙).

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.30-1 ✅ | 미결제약정 소스 조사 | 키움(GitHub `younghwan91/kiwoom-rest-api` 참고 + 실호출)과 네이버(basis.py가 이미 쓰는 fchart 계열, m.stock.naver.com 등) 양쪽에서 K200 선물 미결제약정(또는 전체/투자자별) 데이터 소스를 탐색 | 소스를 찾으면 실호출로 URL/파라미터/응답 확정, 못 찾으면 시도한 것·근거를 명확히 문서화(§5.1과 동일한 dead-end 기록 원칙) — **완료(2026-07-26), 소스 없음으로 결론**. 키움: GitHub 저장소 전체 클론해 grep한 결과 `domestic/` 14개 파일 전부 국내 주식 관련이고 선물/옵션/파생 전용 파일·TR·메서드가 0건(공식 가이드 페이지도 재확인 — "국내주식"/"미국주식" 두 카테고리뿐). 네이버: `naver_index.py`(fchart FUT)의 원본 헤더가 `[날짜,시가,고가,저가,종가,거래량,외국인소진율]` 7개뿐이고, `naver_futures_flow.py`(trend API)도 개인/외국인/기관 3필드가 전부 — 신규 발견한 `/api/index/FUT/integration`, `/basic`, `/price` 및 `openInterest`류 추측 경로 다수 실호출·페이지 grep까지 전부 미결제약정 필드 없음. 상세 근거는 `backend/app/routers/basis.py` 모듈 docstring "K200 선물 미결제약정 조사 — 소스 없음, dead end" 절 참고 |
| 5.30-2 (조건부, 스킵) | 수집기 + 노출 | 5.30-1에서 소스를 찾은 경우만 진행 — 일별 시계열로 수집(신규 테이블 또는 macro_series 재사용), 외국인 선물 순매수와 나란히 보여줘서 "신규/청산" 해석에 참고할 수 있게 함 | 실 DB로 데이터 적재 확인, 프런트에 노출 — **5.30-1이 소스를 찾지 못해 조건 미충족, 진행 안 함**(억지 구현 금지 원칙) |

### Phase 5.31 — 선물 만기 롤오버 검토 (2026-07-26, 외부 강의 검토 세 번째) — 결론: 추가 조치 불필요

외부 강의 검토 세 번째 아이디어: "만기 임박 시 월물 롤오버(예: 9월물
매도 + 12월물 매수)로 인해 그날 순매수/매도 누적치만 보면 방향을
오판할 수 있다"는 지적.

**분석(메인 세션, 기존 코드 재검토 — 새 조사/구현 불필요)**: 우리
시스템이 쓰는 외국인 선물 데이터(`clients/naver_futures_flow.py`,
`m.stock.naver.com/api/index/FUT/trend`, itemCode="FUT"="코스피 200
선물")는 **애초에 계약월 구분 없는 시장 전체(전 종목월 합산) 순매수
집계치**다 — 비디오가 예로 든 "9월물 팔고 12월물 산 것"처럼 특정
월물 하나만 떼어서 보는 경우에만 롤오버가 착시를 일으키는데, 우리는
그 특정 월물 데이터 자체를 수집하지도, 보여주지도 않는다. 전체
집계치는 롤오버(같은 크기로 매도+매수)가 있으면 두 다리가 서로
상쇄돼 자동으로 "순수한 신규 방향성 변화"만 남는다 — 즉 **우리가 이미
쓰고 있는 지표 자체가 롤오버에 영향받지 않는 형태**라서 비디오가
경고하는 오판 함정이 원천적으로 발생하지 않는다.

만기와 관련해 실제로 남는 잔여 위험(프로그램매매 만기효과, 네 마녀의
날의 변동성 급증)은 §4.5-5에서 **이미 "만기 임박"(D-3 이내) 배지 +
"네 마녀의 날" 배지로 처리돼 있다**(`derivatives.py`의
`next_futures_expiry`/`is_quadruple_witching`/`days_to_expiry`,
`DashboardPage.jsx`의 `EXPIRY_SOON_D_DAY`). 새로 만들 게 없다.

**결론**: 새 데이터 수집도, 새 UI도 필요 없음 — 조사만으로 완료.
"세 가지 아이디어 모두 적용" 요청을 이렇게 마무리한다: (1) 프로그램매매
비차익 노출 + 종목별 수집 완료(§5.29), (2) 미결제약정 조사 결과 소스
없음(§5.30), (3) 만기 롤오버는 우리 데이터 구조상 애초에 문제가 안 되고
잔여 위험은 이미 기존 배지로 커버됨(§5.31, 이번 절).

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.31-1 | 롤오버 영향 분석 | `naver_futures_flow.py`/`futures_flow.py`가 쓰는 데이터가 계약월 구분 없는 전체 집계치임을 확인, 롤오버가 이 집계치에 미치는 영향(자동 상쇄) 분석 | **완료(2026-07-26)** — 위 분석 참고, 추가 구현 불필요로 결론 |

### Phase 5.32 — 공매도(short selling) 데이터 추가 (2026-07-27, 벤치마킹 결과 1순위)

경쟁 서비스 벤치마킹(조사 에이전트 위임) 결과: 키움 영웅문·삼성증권·
네이버페이 증권 PC·KRX 오픈데이터까지 조사한 거의 모든 곳에 공매도
거래/잔고 데이터가 있는데 우리만 없다는 게 확인됨. 우리가 가진
"대차잔고"(KOFIA, 시장 전체 합계, 종목별/시장별 분리 없음)는 공매도의
전조 지표일 뿐 실제 공매도 거래량/잔고와는 다른 데이터다. 조사 에이전트
소견: 새 유료 데이터가 아니라 이미 쓰는 소스(네이버) 또는 KRX 무료
오픈데이터로 채울 수 있는 손쉬운 공백 — 다만 이건 추정이라 이 프로젝트
관례대로 **먼저 실제 소스를 실호출로 확인**한 뒤 구현한다.

**참고(교차 검증 완료, 재조사 불필요)**: 벤치마킹 에이전트가 "종목별
프로그램매매 차익/비차익 분리는 재확인해볼 만하다"고 했지만, 이건 이미
§5.29에서 ka90013을 실제 파라미터 여러 조합으로 실호출해 분리 필드
자체가 없음을 확정했다 — 이 항목은 이번 스코프에 포함하지 않는다.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.32-1 ✅ | 공매도 소스 조사 | 네이버(종목 상세 PC 페이지 "공매도현황" 탭 또는 그 내부 API — `naver_index.py`/`naver_value_rank.py`가 이미 쓰는 것과 같은 방식으로 페이지 뒤 API 탐색)와 KRX 정보데이터시스템(data.krx.co.kr, 로그인 불필요한 통계 다운로드 포털 — Open API(승인 필요, 이미 403 확인된 것)와는 다른 서비스인지 확인) 양쪽 실호출로 시장 전체(코스피/코스닥)와 종목별 공매도 거래량/거래대금/비중·잔고 데이터 확보 가능 여부 확인 | 실호출로 URL/파라미터/응답 스키마 확정, 못 찾으면 §5.30과 동일한 원칙으로 정직하게 dead-end 문서화(억지 구현 금지) — **완료(2026-07-27)**. 네이버 자체엔 소스가 없고(`finance.naver.com/item/short_trade.naver?code=...`가 KRX 화면을 그대로 iframe으로 삽입할 뿐, 시장 전체 페이지도 없음), 그 iframe이 가리키는 **KRX 정보데이터시스템의 공매도 통계 서브포털(`data.krx.co.kr/comm/srt/srtLoader/...`)이 §7이 이미 기록한 로그인 필수 일반 통계 포털과는 별개의 무인증 서비스**임을 실호출로 확정 — 종목별(MDCSTAT30001_OUT)·시장 전체(MDCSTAT30201_OUT, "업종별" 화면을 시장 자신(idxIndCd=001)으로 재활용)를 모두 찾음. 상세 근거는 `backend/app/clients/krx_short_selling.py` 모듈 docstring 참고 |
| 5.32-2 ✅ | 시장 전체 공매도 수집기 | 5.32-1에서 찾은 소스로 코스피/코스닥 일별 공매도 거래대금·비중 시계열 수집(`macro_series` 재사용 우선 검토, 안 맞으면 신규 테이블) | 실 DB 적재 확인 — **완료(2026-07-27)**. 시장당 6개 필드(공매도 거래량/거래대금, 시장 전체 거래량/거래대금, 두 비중)라 macro_series(단일 값)에 안 맞아 신규 테이블 `short_selling_market` 채택(`collectors/short_selling_market.py`, REGISTRY 등록) — 실 DB에 162행 적재 확인 |
| 5.32-3 ✅ | 종목별 공매도 수집기 | ka10059/ka90013과 동일한 온디맨드+DB캐시 패턴(신규 테이블 또는 기존 확장) — 종목상세 조회 시 캐싱 | 실 DB로 실제 종목 데이터 확인 — **완료(2026-07-27)**. `ProgramTrade`와 동일한 (code, date) PK 모양의 신규 테이블 `short_selling_stock` + `routers/stocks.py`의 `_ensure_short_selling_cached`/`_read_short_selling`(캐시 계약은 `_ensure_program_trade_cached`와 동일, meta.short_selling_error로 부분 실패 흡수) — 005930 실 데이터로 `/api/stocks/005930/series` 응답에 `short_selling` 확인 |
| 5.32-4 ✅ | 프런트 반영 | 대시보드에 공매도 시장 전체 타일/차트, 종목상세 모달에 공매도 섹션 추가 | 코드 리뷰 + `vite build` 클린 확인, curl로 실데이터 확인 — **완료(2026-07-27)**. 대시보드 "시황·자금" 섹션에 "공매도 비중" 타일(대차잔고와 분리된 별개 타일) + `ShortSellingModal`(코스피/코스닥 거래대금 비중 라인차트, `MacroChart` 재사용), 종목상세 모달에 `ShortSellingSection`(`ProgramTradeSection`과 동일한 row-list 패턴) 추가 — `vite build` 클린, 실데이터로 확인 |

**요약**: 네이버는 자체 공매도 데이터가 없고 KRX 정보데이터시스템 화면을 iframe으로 보여줄 뿐이었지만, 그 KRX 서브포털(`/comm/srt/srtLoader/...` + `/comm/bldAttendant/getJsonData.cmd`) 자체가 로그인/OTP 없이 열리는 별개 서비스임을 실호출로 확인해 시장 전체(코스피/코스닥)와 종목별 공매도 거래량/거래대금/비중·순보유잔고를 모두 확보했다 — 대차잔고(KOFIA)는 그대로 두고 별개 지표로 신규 테이블 2개(`short_selling_market`/`short_selling_stock`)를 추가했다.

### Phase 5.33 — 업종 로테이션 감지 (2026-07-28, 뉴스 영상 검토)

사용자가 JTBC 뉴스 클립(2026-07-27, "'7000 넘으면 판다'... '로테이션' 안
나타나는 이유")을 검토해달라고 요청. 요지: 코스피가 7,000선을 넘을 때마다
외국인이 차익실현 매도로 돌아서는 패턴이 반복되고, 개인은 삼성전자·
SK하이닉스만 순매수하고 나머지는 순매도, 기관도 반도체 비중은 줄였지만
그 자금이 다른 업종으로 유의미하게 옮겨가지 않았다는 진단 — 대신 국내
증시에서 빠진 자금이 미국 주식·은행 예금으로 이탈하고 있다는 것. 즉
"로테이션 없음" = 업종 간 자금 순환이 아니라 시장 자체에서 자금이
빠져나가는 상태. 사용자 요청: 이 로테이션이 실제로 있는지 확인하고,
지금은 없는 기능이지만 매번 체크해서 로테이션이 나타나는 순간을 감지하는
기능을 만들자.

**핵심 발견(코드 조사, 재조사 불필요)**: `collectors/market_flow.py`가
이미 매일 호출하는 ka10051(업종별투자자순매수요청) 응답의 `inds_netprps`
배열은 **업종별 전체 행**을 담고 있는데, 지금은 "종합"(시장 전체 합계)
행 1개만 골라 쓰고 나머지 업종별 행은 전부 버리고 있었다(`clients/
kiwoom.py` "ka10051 추가 검증" 절 — 투자자 13분류 전부 업종별로도
이미 오고 있음, 필드: `frgnr_netprps`(외국인)/`orgn_netprps`(기관계) 등).
**새 외부 호출 없이** 이미 받는 응답에서 버려지던 부분을 살리면 업종별
수급 데이터를 얻을 수 있다.

**설계 원칙(§5 그대로)**: "로테이션 있다/없다"를 단정하는 블랙박스
판정을 만들지 않는다 — §5.15/§5.23/§5.26이 이미 겪은 것처럼, 백테스트
없이 "신호"라고 부르면 근거 없는 판단이 된다. 대신 §5.18/§5.19(쏠림
비율)와 같은 **관찰 지표**로 설계한다: 업종별 자금 유입/유출 배율을
투명하게 보여주고, "자금이 업종 간에 순환하는지(유입 업종 총합 ≈ 유출
업종 총합) vs 시장에서 이탈하는지(전체 순매수 총합 자체가 감소)"를
사용자가 직접 판단할 수 있는 근거 자료를 제공한다.

**검증 계획**: 구현 후 실제 최근 데이터로 돌려서 이 뉴스가 진단한
"로테이션 없음"(반도체 비중 축소가 다른 업종 유입으로 이어지지 않음)
상황과 일치하는지 확인 — 일치하면 이 지표가 실제 현상을 제대로 포착함을
실증하는 좋은 근거가 된다(§5.32의 공매도 전면금지 기간 교차검증과 같은
성격의 신뢰성 확인).

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.33-1 ✅ | 업종별 투자자 순매수 수집 확장 | `market_flow.py`가 이미 호출하는 ka10051 응답에서 버려지던 업종별(`inds_netprps` 전체) 행을 파싱해 신규 테이블(예: `SectorFlow`: sector_code, sector_name, market, date, 외국인/기관계/개인 net_value)에 upsert — 새 외부 호출 없음 | 실 DB로 여러 업종의 일별 수급 데이터 적재 확인, `group_snapshot.py`가 쓰는 기존 업종 분류(79개)와 코드 체계가 다르면 매핑 관계 문서화 — **완료(2026-07-28)**. 실호출로 `inds_netprps` 실제 shape 확정: KOSPI 28행("001_AL"=종합 + "002_AL"~"004_AL" 대형/중형/소형주 + "005_AL"~"030_AL" 24개 산업 업종), KOSDAQ 32행("101_AL"=종합 + "103_AL"~"129_AL"/"141_AL" 22개 산업 업종 + "138_AL"~"151_AL" 9개 지수구성 분류 KOSDAQ 100/MID 300/SMALL 등) — 종합 행과 완전히 동일한 13개 투자자 필드가 업종별 행에도 그대로 존재함을 확인. 신규 테이블 `SectorFlow`(PK `market`+`sector_code`+`date`, 컬럼 `frgnr_net_value`/`orgn_net_value`/`ind_net_value` 3종만 — 13종 전부는 이번 범위 밖) 추가, 마이그레이션 `ca96115c3353`. `collectors/market_flow.py`의 `collect()`가 raw `inds_netprps`를 1회만 받아 종합 행(`_summary_flows_from_rows`, 기존 로직 그대로 분리)과 업종별 행(`_parse_sector_rows`, 신규) 양쪽에 공유 — 새 REGISTRY 항목 없이 같은 세션·같은 잡에 편승(§5.33 문서 참고). **코드 체계 매핑 결론**: `group_snapshot.py`(네이버, 업종 79개/테마 266개, GICS 계열 세분류)와는 완전히 다른 taxonomy라 매핑 테이블을 만들지 않고 독립 체계로 취급 — 실측 근거로 KOSPI "전기/전자"(013_AL) 하나가 네이버의 "반도체와반도체장비"/"전자장비와기기"/"전자제품"/"디스플레이패널" 등 여러 세부 업종으로 쪼개져 있음을 확인(`app/models.py` SectorFlow docstring, `app/quant/sector_rotation.py` 모듈 docstring에 상세 문서화) |
| 5.33-2 ✅ | 로테이션 관찰 지표 설계 | 업종별 최근 활동량을 자기 자신의 평소(20일) 대비 배율로 정규화(§5.19/§5.26 패턴 재사용) — 유입 상위/유출 상위 업종 랭킹 + 전체 순매수 총합 추이를 나란히 제공(순환 vs 이탈 판별 근거) | 단위테스트로 배율 계산 정확성 확인, 실 DB로 현재 반도체 업종 유출·다른 업종 무유입 여부가 뉴스 진단과 일치하는지 검증 — **완료(2026-07-28)**. `app/quant/sector_rotation.py::compute_sector_rotation` 구현 — 외국인+기관계 **signed**(부호 있는) net_value 합을 "업종 자금 흐름"으로 쓰고(방향이 핵심이라 §5.18/§5.19의 절댓값 활동량과 다름), `multiple = 오늘_signed / 평소_activity`(분모는 절댓값 평균, `volume_surge.py`의 recent/baseline 비중첩 패턴 재사용)로 정의 — signed÷signed 배율은 baseline이 0 근처거나 부호가 같이 뒤집히면 무의미해지는 수학적 결함이 있어 분모만 unsigned로 뒀다(모듈 docstring에 근거 상세). 시가총액 규모별/지수구성 분류(`NON_INDUSTRY_SECTOR_CODES`)는 랭킹에서 제외. "로테이션 있다/없다" 판정 없이 유입/유출 랭킹 + 보조 통계(`gaining_sum`/`losing_sum`/`aggregate_delta`)만 반환(단위테스트 8개, 가짜 market 문자열로 격리 — 실데이터와 같은 sector_code를 실제 market명으로 테스트하면 실 이력이 베이스라인에 섞여 들어가는 문제를 실측으로 발견해 우회). 실 DB(43거래일 백필) 검증: 2026-07-28 기준 KOSPI 집계 signed 합 -406.34억(평소 대비 -182.61억 추가 하락), 유입합 18.07억 vs 유출합 -424.41억(오프셋비 ~4%) — 유출을 상쇄할 유입이 거의 없어 뉴스의 "로테이션 없음" 진단과 정성적으로 일치. "전기/전자"(013_AL, 반도체 프록시)는 today=-167.55억, baseline_signed_avg=-134.705억으로 **평소에도 이미 순매도 기조**였고 오늘은 그 평소보다 오히려 작은 배율(-0.56배)이라 "오늘 하루의 극단적 이벤트"라기보다 "지속적 순매도 추세"에 가까움 — 12일치 비중복 관측 전부가 극단적으로 한쪽으로 쏠려(오프셋비가 항상 0에 가깝거나 매우 큼, 1.0 근처인 날이 하루도 없음) 개별 업종 간 균형 잡힌 로테이션보다는 시장 전체가 동시에 위험선호/회피로 움직이는 패턴에 가까워 보임 — 다만 3주 표본(12개 유효일)은 매우 작아 단정할 수 없음, 앞으로 이 엔드포인트를 계속 체크하며 축적하는 것이 이 기능의 원래 목적 |
| 5.33-3 ✅ | API + 프런트 반영 | 업종 유입/유출 랭킹 엔드포인트 + 대시보드 카드(업종·테마 강약 트리맵 인근에 배치, 관찰 서술만·매매 판단 문구 없음) | curl로 실데이터 확인, 코드 리뷰 + `vite build` 클린 확인 — **완료(2026-07-28)**. `GET /api/markets/{market}/sector-rotation`(`routers/markets.py`, kospi/kosdaq만 허용) 추가, `compute_sector_rotation`을 그대로 노출. 프런트 `SectorRotationCard`(`DashboardPage.jsx`)를 업종·테마 트리맵 바로 아래 배치 — 코스피/코스닥 토글 + 유입 상위/유출 상위 2열 + "평소 대비 N배" 텍스트 + 유입합/유출합/전체 합계 보조 통계 한 줄, "매수"/"매도"/"로테이션 있음" 등 판정 문구 없음. `vite build` 클린, curl로 실데이터 확인 완료 |

**요약**: `market_flow.py`가 매일 호출하던 ka10051 응답에서 버려지던 업종별 행(KOSPI 27개/KOSDAQ 31개, 종합 행 제외)을 새 외부 호출 없이 살려 `sector_flow` 테이블에 적재하고, 외국인+기관계 signed net_value를 업종 자기 자신의 20일 평소 대비 배율로 정규화한 관찰 지표(`quant/sector_rotation.py`)를 만들었다. 43거래일 백필 실데이터로 확인한 결과 KOSPI는 유출을 상쇄할 유입이 거의 없고(오프셋비 ~4%), "전기/전자"(반도체 프록시)는 평소부터 이미 순매도 기조라는 점에서 뉴스의 "로테이션 없음" 진단과 정성적으로 일치했다 — 다만 관측일이 12일뿐이라 확정적 결론은 아니고, 이 지표를 계속 체크하며 데이터를 쌓아가는 것이 원래 목적이다. 판정(로테이션 있다/없다)은 어디에도 반환·표시하지 않는다(§5.15/§5.23/§5.26 house rule 유지).

### Phase 5.34 — 지지선/저항선(거래량 프로파일) 감지 (2026-07-28 사용자 요청)

사용자: "특정 지점에서 매수, 매도가 많은 것도 알아내야 해.. 지지선 혹은
저항선 말이야." — 특정 가격대에 과거 거래량이 몰려 있으면 그 가격대가
다시 오면 매수/매도 압력이 몰려 지지·저항으로 작용한다는 고전적
기술적분석 개념(거래량 프로파일/Volume Profile, POC)을 우리 데이터로
감지하는 기능.

**데이터 현실 점검(설계 전 확인)**: 진짜 "가격대별 체결 거래량"(틱 단위
volume-at-price)은 우리에게 없다 — 갖고 있는 건 일봉(open/high/low/
close/volume)과 온디맨드 분봉(1분 단위 OHLCV)뿐이다. 그래서 이 지표는
**근사치**다 — 하루(또는 1분봉 한 개)의 거래량을 그날의 저가~고가
구간에 분배해 가격 구간별로 누적하는 방식(일봉/분봉 기반 거래량
프로파일 근사, 실전 HTS들도 틱 데이터가 없을 때 흔히 쓰는 방식). 이걸
숨기지 않고 "근사치"임을 명시한다 — §5 "정직한 표시" 원칙, 백테스트
없이 "이 가격대가 진짜 지지선이다"라고 단정하지 않는다(§5.15/§5.23/
§5.33과 동일한 house rule — 관찰 지표로만 제공, 판정 없음).

**설계**:
1. 순수 계산부(`quant/volume_profile.py`) — OHLCV 리스트(일봉 또는
   분봉, 이미 있는 `_read_candles`/`get_market_series_from_db`/온디맨드
   분봉 응답 재사용, 새 데이터 수집 없음)를 받아 가격 구간(예: N=50
   bin)으로 나누고, 각 봉의 거래량을 그 봉의 저가~고가 구간에 걸쳐
   분배(균등분배가 가장 단순 — 정교한 삼각분포 등은 과설계, 근사치라는
   전제하에 단순함 우선)해 구간별 누적 거래량(프로파일)을 만든다.
   그 프로파일에서 국소 최댓값(피크)들을 지지/저항 후보로 추출하고,
   전체 최댓값(POC, Point of Control)을 별도로 표시한다.
2. 종목/지수 아무 대상이나 적용 가능한 범용 함수로 만든다(이미 있는
   `_read_candles`(종목)/`get_market_series_from_db`(지수) 응답 형태를
   그대로 입력으로 받음).
3. API: 종목상세/지수 시리즈 조회에 옵션으로 얹거나 별도 엔드포인트로
   제공(구현 시 더 간단한 쪽으로 판단). 프런트: `CandleChart.jsx`에
   수평 참조선(지지/저항 가격대) 오버레이 추가 — lightweight-charts의
   `createPriceLine` 활용, 기존 `overlay` prop(시계열 라인, VWAP용)과는
   다른 새 prop 필요.
4. "매매 신호"가 아니라 "과거 이 가격대에서 거래가 많이 몰렸다"는 관찰
   서술만 — 색상도 매매 신호로 안 보이게 중립 톤 유지.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.34-1 | 거래량 프로파일 계산 함수 | `quant/volume_profile.py` 신규 — OHLCV 리스트를 받아 가격 구간별 누적 거래량 프로파일 + 국소 피크(지지/저항 후보) + POC 추출 | 단위테스트로 합성 OHLCV 데이터에 대한 프로파일/피크 계산 정확성 확인 |
...
### Phase 5.36 — 시장 위험 경보 상단 배너(서킷브레이커/사이드카/거래량 급증) (2026-07-28 사용자 요청)

사용자: 오늘 코스피 서킷브레이커 발동 수준(-8%) 급락을 실측으로 확인해 준
직후, "하락폭 혹은 볼륨이 크면 뭔가 이슈를 알려줘야 하지 않을까? 위험하다고
경고 발동을 해줘야 할 것 같아.. 상단 배너로 말야.."

**설계 전 조사 결과**:
- 서킷브레이커(코스피/코스닥 각각 전일 종가 대비 8%/15%/20% 하락, 통상 1분
  지속 시 발동)와 사이드카(코스피 — K200 선물가격이 기준가 대비 ±5% 변동,
  통상 1분 지속 시 프로그램매매 호가효력 5분 정지)는 KRX의 실제 공식
  제도다 — 임의 임계값이 아니라 이미 정해진 규정 수치를 그대로 쓴다. 다만
  이 기능은 우리가 가진 1분봉 종가로 "그 임계값에 도달했는지"를 근사
  판정할 뿐, KRX의 실제 발동 선언(정확한 1분 지속 판정, 정확한 발동
  시각)을 직접 수신하는 공식 채널이 아니다 — "발동되었다"고 단정하지 않고
  "발동 기준에 해당하는 변동폭"이라고 근사·관찰로만 표현한다(§5 house
  rule과 동일한 정직한 표시 원칙).
- 코스닥 사이드카는 코스닥150 선물 기준인데 이 프로젝트엔 그 데이터가
  아예 없다(조사 완료) — 지원 불가로 명시하고 조용히 생략하지 않는다.
- 거래량 급증은 이미 §5.24(`quant/volume_surge.py::compute_volume_surge`)가
  계산해 `GET /api/markets/index-tiles/live`의 kospi/kosdaq
  `volume_surge.multiple` 필드로 이미 노출되고 있다 — 이건 KRX 공식 제도가
  아니라 우리가 정하는 자체 휴리스틱 임계값이므로, 서킷브레이커/사이드카와
  섞어 "공식 제도"처럼 보이지 않게 구분해서 표시한다.
- `_index_tile_prev_close`(전일 확정 종가, 오늘 제외)와 `_index_tile_from_
  intraday`/`_index_tile_futures_live`(실시간 현재가)가 이미
  `index-tiles/live`에 다 있다 — 이 배너는 새 외부 호출·새 엔드포인트 없이
  그 응답에 판정 필드만 얹는다.
- 프런트에 이미 `.banner` 클래스가 있지만(NXT/장마감 안내용) 의도적으로
  "경고색을 넣지 않는다"는 기존 관례를 따른다(`DashboardPage.jsx` 주석).
  이번 배너는 관찰용 오버레이가 아니라 실제 규정 임계값 도달이라는 사실을
  알리는 것이라 그 관례의 대상이 아니다 — 이 배너만 예외적으로 경고 톤
  (심각도별 색상)을 쓴다.

**설계**:
1. `quant/risk_alert.py`(순수 계산, DB/세션 무관) —
   `classify_market_risk(kospi, kosdaq, futures)`: `index-tiles/live`가 이미
   반환하는 시장별 dict(`close`/`change_rate`/`volume_surge` 등)를 그대로
   입력받아 서킷브레이커 단계(0~3)/사이드카 여부(코스피만, 코스닥은
   `supported: false`)/거래량 급증 여부를 판정해 반환. 임계값은 이름 붙은
   상수로(8/15/20/±5/거래량 배수 자체 기준) 문서화.
2. `routers/markets.py::index-tiles/live` 응답에 `"risk"` 필드로 얹는다(새
   외부 호출 없음, 기존 60초 캐시 재사용). 장 마감(`market_closed: true`)
   이면 위험 배너는 "지금 당장의 위험"이 아니므로 판정을 생략하거나 프런트가
   숨긴다.
3. 프런트: 새 배너 컴포넌트 — 심각도(서킷브레이커 3>2>1 > 사이드카 > 거래량
   급증) 중 가장 높은 걸 헤드라인으로, 나머지 활성 조건은 보조 텍스트로.
   "발동되었습니다" 단정 대신 "발동 기준(-8%)에 해당하는 변동폭이 감지됨"
   같은 관찰형 문구. 세션 내 닫기 가능하되, 더 심각한 단계로 악화되면 다시
   노출.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.36-1 ✅ | 위험 판정 함수 | `quant/risk_alert.py` 신규 — CB/사이드카/거래량 급증 판정 | 단위테스트(23개)로 각 단계 경계값 확인 |
| 5.36-2 ✅ | API 반영 | `index-tiles/live` 응답에 `risk` 필드 추가 | curl로 오늘 실데이터 기준 코스피 CB 도달(-9.3%) 확인 |
| 5.36-3 ✅ | 프런트 배너 | 심각도별 경고 톤 상단 배너, 관찰형 문구, 세션 닫기+악화 시 재노출 | 코드 리뷰 + vite build 클린 + 브라우저 확인(오늘 실데이터로 실제 렌더 확인) |

**추가 요청(2026-07-28, 같은 대화 중)**: 사용자 — "평소보다 외인 혹은 기관의
기울기가 낮으면 위험 경고를 보내주는것도 추가해봐.. 투자자별 수급 요약을
활용하면 되지 않을까해.." → "기울기"는 이미 §5.17(`quant/flow_acceleration.py`)이
계산해 둔 `recent_velocity`/`prior_velocity`(각 30분 구간 순매수 변화량, intraday_
sample 재사용, 새 수집 불필요)와 정확히 같은 개념이다 — 이미 `/api/markets/regime`
엔드포인트가 코스피/코스닥 × 외국인/기관계 4개 조합 전부에 대해 이 값을 계산해
두고 있다(`routers/markets.py::_compute_regime_combo`).

**주의(§5.17 기존 원칙과의 관계)**: `flow_acceleration.py` 모듈 docstring은
"가속도는 아직 정규화/검증 안 된 신호라 `_judge_regime`(코스닥우세/코스피우세
같은 방향성 종합판정)에는 절대 섞지 않는다"고 명시했다 — 이 원칙은 그대로
유지한다. 이번에 추가하는 것은 그 종합판정이 아니라 §5.36의 위험 배너(순수
관찰 알림, "매매 신호" 판정이 아님)이므로 이미 배너에 들어가 있는 거래량
급증(§5.24, 이 역시 백테스트로 검증된 적 없는 자체 휴리스틱)과 같은 급으로
취급한다 — "공식 제도 아님, 검증 안 됨, 참고용" 문구를 그대로 계승.

**설계 추가**:
- `recent_velocity < 0`이고 `prior_velocity < 0`(둘 다 매도 중)이며
  `abs(recent_velocity) >= FLOW_SLOPE_ALERT_MULTIPLE * abs(prior_velocity)`
  (직전 30분 대비 매도 속도가 그 배수 이상 빨라짐)면 경보. 방향 전환(매수→매도)
  케이스는 배수 비교가 무의미해지므로 이번 스코프에서 제외(단순함 우선, 필요하면
  나중에 별도 추가).
- `index-tiles/live`의 `_warm_index_tiles_live`가 이미 session을 들고 있으니
  `flow_acceleration.compute_flow_acceleration`을 4개 series_key
  (`flow_kospi_외국인`/`flow_kospi_기관계`/`flow_kosdaq_외국인`/
  `flow_kosdaq_기관계`)에 대해 직접 호출해(새 엔드포인트·새 프런트 fetch 없음)
  `risk` 안에 통합한다 — `/api/markets/regime`과 별개로 한 번 더 계산되는
  중복이 생기지만, 각각 가벼운 인덱스 조회 3회뿐이고 두 엔드포인트가 서로의
  캐시에 의존하지 않는 편이 더 단순하다(이 코드베이스의 기존 라이브 엔드포인트들도
  서로 캐시를 공유하지 않고 각자 계산하는 방식을 이미 쓰고 있음).
- 심각도는 사이드카/서킷브레이커(공식 제도)보다 낮고, 거래량 급증과 같은
  "자체 관찰" 급으로 — 정확한 상대 순서는 구현 시 판단.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.36-4 ✅ | 수급 가속도(기울기) 경보 추가 | `risk_alert.py`에 판정 함수 추가, `index-tiles/live`에서 4개 series_key 가속도 계산해 통합, 프런트 배너에 새 alert kind 반영 | curl로 실데이터 확인(오늘은 조건 미충족 확인, 강제 모킹으로 프런트 렌더 검증) + 단위테스트(10개 추가, 총 625 passed) + vite build 클린 |

## 5.37 — 경쟁 서비스 벤치마킹 조사 (2026-07-30, 조사 에이전트 위임)

사용자: "타 서비스와 비교해서 우리에서 부족한 부분을 찾아봐.. 1. 없는거.. 2.
있지만 로직부분에서 부족한거.." — 국내 HTS/MTS(키움 영웅문, 삼성증권 mPOP,
토스증권, 네이버페이 증권, 카카오페이증권, 미래에셋 m.Stock), 국내 퀀트/
스크리닝 툴(뉴지스탁 GenPort, 퀀터스), 해외 참고(TradingView, Finviz,
Stock Rover)까지 웹 조사(뉴스/공시는 기존 결정대로 계속 스코프 밖).

### A. 없는 기능 (조사 결과, 우선순위 순)

1. **알림/notification 시스템 자체가 없음** — 조사한 모든 서비스가 최소
   가격 알림은 기본 제공(키움/삼성/토스는 커스텀 조건 알림까지). 테이블
   스테이크스급 공백.
2. **다중 조건 검색식(AND/OR, 저장·상시감시)** — 키움[0150]/삼성 mPOP
   [1503]만 보유, 네이버는 고정 프리셋 5개뿐.
3. **사용자 정의 백테스트/전략 빌더** — 뉴지스탁 GenPort/퀀터스급 프로슈머
   영역(연 20만~120만원대 구독) — 지금 스코프엔 과함, 참고만.
4. **종목별 VI 근접 스크리닝** — 미래에셋 KAIROS에만 있음. 우리 서킷
   브레이커/사이드카는 지수·선물 레벨만.
5. **상장주식수 대비 수급 비중(%) 정규화** — 씽크풀이 이렇게 함(원화
   절대값 대신 %). 우리가 이미 시장 전체 쏠림비율에 적용한 "시총 편향
   보정" 원칙을 종목 단위 수급에도 적용하는 자연스러운 확장.
6. 커뮤니티 공유 스크린/전략 마켓플레이스, 사용자 조정 가능한 스코어
   가중치 노출(Stock Rover), 생존편향 보정 백테스트 유니버스(Finviz
   Elite) — 참고 수준, 우선순위 낮음.
7. (뉴스/공시 기반 AI 시그널 — 토스/미래에셋이 최근 추가 중이라는데 스코프
   밖 유지)

### B. 있지만 로직이 약한 것

1. **수급이 절대값/자기 과거 대비 위주, 유니버스 내 상대순위(percentile)가
   없음** — 뉴지랭크는 시총 그룹 내 상대 순위를 매김. 우리는 "이 종목이
   비슷한 규모 종목들 대비 얼마나 특이한지"를 못 봄.
2. **스마트머니 vs 노이즈 필터링(헤지/차익 매매 vs 순수 방향성 매매 분리)**
   — 하나증권 리서치 수준의 정교한 분해인데 어디에도 퍼블릭 제품으로
   없음(만들면 선도지만 데이터 난이도·불확실성 높음 — 이번 스코프에서는
   보류, 나중에 별도로 재검토).
3. **서킷브레이커/사이드카 외 KRX 공식 경보 등급 누락** — 투자주의/경고/
   위험 종목 지정, 단기과열 단일가매매 지정도 KRX 공식 제도인데 우리
   위험 배너(§5.36)가 안 잡고 있음. 기존 "공식 임계값 + 관찰형 문구"
   원칙에 그대로 확장 가능.
4. **스켈핑 스크리너 자체의 적중률 검증 없음** — 퀀터스/GenPort는 스크리닝
   결과의 과거 승률을 보여줌. 우리도 이미 `scalp_pick`/`scalp_tracker`
   (§5.7 — 진입 시점·이후 5/15/30/60분·EOD change_rate 기록)로 재료는
   쌓고 있으니, 이 데이터를 실제로 집계해 노출하는 게 자연스러운 다음
   단계.

**우리가 이미 앞서는 것(참고, 작업 대상 아님)**: 연속매수매도 스트릭 +
익일수익률 백테스트 조합(경쟁사는 순위만 보여주고 백테스트 검증 없음),
수급 가속도(기울기) 지표, 자기 과거 분포 대비 백분위 수급 랭킹 — 조사한
어디에도 없었음.

**다음 단계**: 사용자가 "2번(있지만 로직 부족한 것)부터 보완하자"고 확정
— B-1(수급 유니버스 상대순위)/B-3(KRX 경보 등급 확장)/B-4(스크리너
적중률 검증) 3개를 우선 스코프로 하고, B-2(스마트머니 분해)는 데이터
난이도가 높아 이번엔 보류. 아래 Phase 5.38/5.39/5.40에서 각각 상세 설계.

### Phase 5.38 — 수급 유니버스 상대순위(percentile) (2026-07-30, §5.37 B-1)

**설계 전 확인**: `value_rank`(거래대금 상위, 시장당 top100) 수집기가 이미
소스(naver_value_rank) 응답의 `marketValueRaw`(시가총액/AUM)를 받아 turnover
계산에만 쓰고 버리고 있었다(`collectors/value_rank.py`) — Phase 5.35의
"이미 받아오고 있었는데 저장 안 했다" 패턴과 동일. 저장만 하면 새 외부
호출 없이 시총 데이터가 생긴다. 그리고 그 동일 종목군(코스피+코스닥
거래대금 상위 최대 200개)에 대해 `stock_flow`(외국인+기관 순매수)도
이미 10분마다 수집 중이다(`collectors/live_refresh.py::_run_stock_flow_scan`).
즉 시총·수급 둘 다 이미 있는 데이터라 조인만 하면 된다.

**설계**: 그날의 value_rank 종목군을 시장별(코스피/코스닥 분리 — 안 그러면
쏠림 비율과 같은 시총 편향 재발)로 시가총액 4분위(또는 구현 시 적절한
tier 수)로 나누고, 각 tier 안에서 그날 순매수(`stock_flow`)의 percentile을
계산한다. "이 종목이 비슷한 규모 종목들 대비 수급이 얼마나 몰렸는지"를
보여주는 관찰 지표 — 매매 신호 아님, 이 지표 자체의 예측력도 검증된 적
없음을 명시.

**스코프 한계(정직하게 명시)**: 전체 시장(~4천 종목)이 아니라 value_rank
top100×2(코스피/코스닥)만 대상 — "그날 거래대금 상위권 안에서의 상대
순위"이지 전 시장 대비가 아니다.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.38-1 | 시가총액 저장 | `ValueRank`에 `market_value_million` 컬럼 추가(마이그레이션) + `collectors/value_rank.py`가 이미 파싱하는 값을 upsert에 포함 | 실 DB에 값 적재 확인 |
| 5.38-2 | percentile 계산 함수 | 신규 `quant/flow_percentile.py` — value_rank+stock_flow 조인, 시장별 시총 tier 내 순매수 percentile 산출(순수 함수) | 단위테스트(합성 데이터로 tier 분리·percentile 정확성 확인) |
...
— 서킷브레이커/사이드카와 마찬가지로 KRX가 실제로 운영하는 공식 제도다.
지금 위험 배너(§5.36)는 이 등급들을 전혀 안 본다. **다만 이 프로젝트엔
아직 이 데이터의 실제 소스(키움 TR 또는 data.krx.co.kr)를 확인한 적이
없다** — §5.32(공매도)·§5.30(미결제약정)과 동일한 원칙대로, 구현 전에
반드시 실호출로 소스를 먼저 확정한다. 못 찾으면 §5.30처럼 정직하게
dead-end로 문서화하고 억지 구현하지 않는다.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.39-1 ✅ | 소스 조사 | 키움 API(관리종목/투자유의·경고·위험 종목, 단기과열 지정 관련 TR 탐색) + data.krx.co.kr 무인증 서브포털(§5.32와 같은 방식) 실호출로 확인 | 실호출로 URL/파라미터/응답 스키마 확정 또는 정직한 dead-end 문서화 — **완료(2026-07-30)**. 키움: GitHub `younghwan91/kiwoom-rest-api` 전체 재클론(§5.30/§5.32와 동일 방법)해 `domestic/` 16개 파일 전부를 "관리종목/투자유의/경고/위험/단기과열/정리매매"로 grep — 매칭 0건, 가장 근접한 `ka10054`(변동성완화장치, VI)도 완전히 다른 제도. 이미 호출 중인 `ka10001` 실호출 응답(46필드 전수 확인)에도 상태 플래그 없음 — **키움 쪽 소스 없음 확정**. data.krx.co.kr: 메뉴 트리에 "단기과열종목 현황"(MDC02021001)이 있지만 실호출하면 바로 로그인 페이지로 리다이렉트(§7 기존 로그인 장벽과 동일) — §5.32 같은 무인증 서브포털은 이 메뉴엔 없음. 대신 **KRX 공시채널 KIND(kind.krx.co.kr)에서 투자주의/경고/위험 3단계 전용 무인증 화면을 새로 발견**: `POST kind.krx.co.kr/investwarn/investattentwarnrisky.do`(로그인/쿠키 불필요 콜드 POST, jQuery `ajaxSubmit`이 같은 URL에 요청하는 것을 실측). tier별 응답 스키마가 다름(경고/위험=번호·종목명·공시일·지정일·해제일, 해제일 "-"=현재 지정 중; 주의=번호·종목명·유형·공시일·지정일로 해제일 컬럼 자체가 없음 — "그날 하루"짜리 통보라 해제 개념이 없기 때문). 2026-07-30 실측: 투자경고 활성 17종목(비비안·모나미·원풍물산 등), 투자위험 활성 0종목. 단기과열은 KIND에 전용 조회 화면이 없어 일반 공시 전문검색(`POST kind.krx.co.kr/disclosure/details.do`, `reportNm=단기과열` 텍스트 검색)으로만 real 데이터를 찾음(계양전기우·SK증권우 등 우선주 위주 "지정연장" 공시가 실제로 나옴) — 상세 근거는 `backend/app/clients/kind_investor_warning.py` 모듈 docstring 참고 |
| 5.39-2 ✅ | 수집/조회 구현 (투자주의/경고/위험만 — 단기과열은 보류, 아래 설명) | 등급 조회 로직 + 종목 단위 배지(시장 배너가 아닌 별도 위치, 아래 설명) | **완료(2026-07-30)**. `models.InvestorWarningEvent`(PK: tier/raw_name/designated_date) + 마이그레이션 `cfce0c8583f9`, `clients/kind_investor_warning.py`(tier별 정규식 파싱, HTML 엔티티 unescape 포함), `collectors/investor_warning.py`(경고·위험 90일/주의 10일 lookback, `stocks.name` 정확 일치로 code 매핑, REGISTRY 등록), `quant/investor_warning_status.py`(순수 판정 함수 — risk>warning>caution 우선순위, caution은 "전역 최신 caution 데이터 날짜와 지정일이 같을 때만 활성"이라는 별도 규칙, 단위테스트 8개). `routers/stocks.py::stock_series`에 `investor_warning` 필드 추가(하루 1회 배치라 온디맨드 캐시 불필요, DB 조회만). 실데이터 확인: `GET /api/stocks/002070/series`(비비안) → `{"active_tier":"warning","label":"투자경고종목","designated_date":"20260720"}`, 정상 종목(005930)은 `active_tier: null`. **설계 판단(문서화)**: `risk_alert.py`의 `classify_market_risk`는 지수 3개(코스피/코스닥/선물) 단위 시장 전체 배너라 종목 단위 지정을 끼워 넣을 자리가 없어 확장하지 않았다 — 대신 별도 순수 모듈(`quant/investor_warning_status.py`)로 분리하고 소비처도 종목 상세로 잡았다(근거는 그 모듈 docstring 상단 절 참고). **단기과열은 이번 스코프에서 구현 보류** — 5.39-1에서 찾은 소스가 KIND의 구조화된 지정현황 표(투자주의/경고/위험처럼 종목명/지정일/해제일 고정 컬럼)가 아니라 공시 제목 전문검색(`reportNm=단기과열`)이라, "지정/지정연장/지정해제"를 자유 텍스트 제목에서 정규식으로 추론하고 괄호 안 우선주 종목명("계양전기우")까지 파싱해야 하는 등 다른 모든 소스보다 스키마 안정성이 훨씬 낮다(제목 문구가 KRX 재량으로 바뀌면 조용히 깨질 위험) — §5 "억지 구현 금지" 원칙에 따라 이번엔 투자주의/경고/위험만 구현하고 단기과열은 소스만 문서화해 둔다(클라이언트 모듈 docstring 참고, 향후 필요해지면 재검토) |
| 5.39-3 | (보류 — 단기과열 소스가 위 이유로 구현되지 않음) | 위험 배너에 새 등급 alert kind 추가 | 원래 지시는 "위험 배너에 alert kind 추가"였으나, 5.39-2에서 투자주의/경고/위험 3단계는 시장 배너가 아니라 **종목 상세 배지**(`StockDetailModal.jsx`, `Badge kind="investor-caution/warning/risk"`, §5.36 위험 배너와 동일한 info/warn/danger 색 언어 재사용)로 반영했다 — 서킷브레이커/사이드카/거래량급증/수급기울기는 전부 "시장 전체" 조건이라 상단 배너가 맞지만, 이건 "이 종목이 지금 지정 상태인가"라는 종목 단위 사실이라 배너에 넣으면 "지금 보고 있는 시장과 무관한 개별 종목 목록"이 배너에 섞이는 부자연스러운 결과가 된다는 판단(5.39-2 항목의 "설계 판단" 참고). vite build 클린 확인 완료, 실데이터(비비안 종목상세에서 "투자경고종목" 배지 노출) 확인 완료 |

### Phase 5.40 — 스켈핑 스크리너 적중률 검증 (2026-07-30, §5.37 B-4)

`scalp_pick`(§5.7)이 이미 진입 시점 스코어/순위와 이후 5/15/30/60분·EOD
change_rate를 기록 중이다(2026-07-30 기준 실 DB 328건, EOD까지 채워진 건
221건, 2026-07-21부터 누적). 이 데이터를 집계해 "이 스크리너가 실제로
맞았는지"를 승률/평균수익률로 노출한다 — 새 예측이 아니라 기존 스코어링의
**사후 검증 투명성**을 높이는 작업.

**정직하게 명시할 것**: 표본이 아직 9거래일뿐이고 이 기간이 극심한 하락장
(§5.36 서킷브레이커 발동 수준의 이례적 기간)이라 평상시를 대표한다고 볼
수 없다 — 표본 수와 기간을 항상 함께 노출하고, "검증됨"이 아니라 "지금까지
관찰된 결과"로만 표현한다.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.40-1 | 집계 함수 | 신규 `quant/scalp_hitrate.py` — scalp_pick에서 호라이즌별 승률(양수 비율)·평균 change_rate·표본수 집계(순수 함수 또는 session 기반, 구현 시 판단) | 단위테스트 + 실 DB 값으로 검증 |
| 5.40-2 | API 반영 | 신규 엔드포인트(예: `GET /api/markets/scalp-candidates/hitrate`) | curl로 실데이터 확인 |
| 5.40-3 | 프런트 반영 | 스켈핑 후보 카드 근처에 표본수·기간 명시한 통계 노출 | vite build 클린 + 코드리뷰 |

### Phase 5.41 — 선물(K200) 외국인/기관 순매매 가격대별 프로파일 (2026-07-30 사용자 요청)

사용자: "외인 양손 · 현선물 > 다음만기는 그냥 날짜만 보여주는데.. 외인, 기관의
포지션을 알려줄 수 있어? 즉 제일 많이 거래된 부분 말야.. 상방 하방 포지션
모두 알려줘야 할 것 같은데?" — 이어서 "혹시 이런거 매매기법이 있다면 그것도
적용해 보는건 어때?"

**설계 전 확인**:
- 미결제약정(open interest) 데이터는 §5.30에서 이미 키움·네이버 양쪽 dead-end
  확정 — 지금도 유효, 재조사 불필요.
- "가격대별 투자자 유형 태깅 매매"라는 공개 데이터 자체가 전 세계 어디에도
  없다(주문 단계 투자자 분류는 거래소/증권사 내부 정보) — 웹 검색으로도
  이걸 정확히 하는 "매매기법"은 확인 안 됨(TradingView의 "Institutional
  Volume Flow"류도 실제 투자자 태깅이 아니라 거래량 패턴으로 추정하는
  방식). **사용자가 말한 "매매기법"은 존재하지 않는다 — 자체 구성한
  근사치임을 명시하고 간다.**
- 대신 이미 가진 두 데이터로 근사 가능: K200 선물 일봉(고가/저가,
  `index_ohlcv` market='k200_futures') + 하루 단위 외국인/기관계 선물
  순매매 금액(`market_flow` market='k200_futures' source='naver',
  `collectors/futures_flow.py`가 이미 수집 중, 새 외부 호출 불필요).

**설계**: §5.34(`quant/volume_profile.py`)와 완전히 같은 binning 기법을
재사용하되, 각 봉의 "거래량"(항상 양수) 대신 그날의 **부호 있는** 외국인
(또는 기관계) 순매매 금액을 그 봉의 저가~고가 구간에 분배해 누적한다.
양수 쪽 최댓값 구간("상방"=순매수 집중대)과 음수 쪽 최댓값 구간("하방"=
순매도 집중대)을 각각 추출 — 방향이 있는 지표라 §5.34의 중립색 원칙과
달리 이번엔 한국 등락 관행(상승=빨강/매수 계열, 하락=파랑/매도 계열,
memory `chart-color-korean-convention` 참고)을 그대로 적용하되, 문구는
철저히 관찰형("이 구간에서 과거 순매수가 몰렸었다")으로 — "사라/팔아라"로
읽히면 안 된다.

**정직하게 명시할 것**: 이건 하루 단위로 뭉뚱그린 근사치다 — 그날 실제로
어느 가격에서 체결됐는지(틱 단위)는 모른다. 미결제약정 기반 "포지션"이
아니라 "순매매 흐름이 가격대별로 어디 몰렸는지"의 근사이므로 이 차이를
UI 문구에 명시한다.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.41-1 ✅ | 부호 있는 프로파일 계산 함수 | 신규 `quant/futures_flow_profile.py` — `quant/volume_profile.py`의 binning 로직을 부호 있는 값에 맞게 재사용/변형, 양/음 각각 극값(레벨) 추출 | 단위테스트 13개(합성 데이터로 상방/하방 구간 분리 검증) — **완료(2026-07-30)** |
| 5.41-2 ✅ | API 반영 | `GET /api/markets/futures/series`에 `include_flow_profile` 옵트인 필드로 외국인/기관계 각각의 프로파일 추가(코스피/코스닥은 스코프 밖이라 무시) — 새 외부 호출 없음(이미 있는 index_ohlcv+market_flow 조인) | curl로 실데이터 확인 — **완료(2026-07-30)**. 외국인 buy_levels 889.0/790.8, sell_levels 758.1/823.6/1134.3/954.4, 기관계는 반대 방향(823.6 매수 vs 외국인 매도 등) — 실측 가격범위(538.2~1535.0) 안에 전부 위치 확인. kospi/kosdaq은 `include_flow_profile=true`여도 필드 자체 미노출 확인 |
| 5.41-3 ✅ | 프런트 반영 | `CandleChart.jsx`에 방향성 있는 레벨 오버레이 추가(§5.34 `levels` prop과는 별개 색상 로직 — 매수구간 빨강계열/매도구간 파랑계열), 관찰형 문구+근사치 명시, 선물 캔들 모달에 적용 | 코드리뷰 + vite build 클린 확인 — **완료(2026-07-30)**. 외국인/기관계 토글(동시 렌더 시 선 최대 16개라 하나씩), 선물 캔들은 §5.34 중립 levels 대신 §5.41 방향성 flowLevels만 사용. pytest 661→674(13개 신규) |

**요약**: 미결제약정은 여전히 소스가 없어(§5.30) 진짜 "포지션"은 못 만든다 — 대신 이미 수집 중인 K200 선물 일봉 + 일별 외국인/기관 순매매 금액을 §5.34와 같은 binning으로 부호 있게 결합해 "순매수/순매도가 가격대별로 어디 몰렸는지"의 근사치를 만들었다. 실데이터에서 외국인과 기관계가 서로 반대 방향의 가격대에서 집중된 패턴이 나타나 최소한의 현실성은 확인했지만, 이 프로파일 자체의 예측력은 검증된 적 없다 — 관찰 참고선일 뿐이다.

### Phase 5.42 — 만기 수렴 패턴 분석 (2026-07-30 사용자 요청)

사용자: "외인 양손 · 현선물 > 다음만기 이부분에 뭔가 디테일 정보로 따로
분석해서 그래프를 그려주는건 어때?" — 지금 "다음 만기" 타일은 날짜+D-day+
네 마녀의 날 배지가 전부다(`derivatives.py`, `DashboardPage.jsx` 3370행
근처).

**설계 전 확인**: `index_ohlcv`에 K200 선물(`k200_futures`)·현물지수
(`kospi200`) 둘 다 **2023-07부터 3년치**(각 735~737행) 이미 쌓여 있다 —
약 36회의 과거 만기 사이클을 커버할 만큼 충분하다. 새 데이터 수집 없이
이걸로 "만기가 다가올수록 베이시스가 역사적으로 어떻게 움직였는지"를
분석할 수 있다.

**설계**: `routers/basis.py`가 이미 계산하는 베이시스(선물종가-현물종가)를
과거 각 만기일 기준 "만기까지 남은 거래일수"(D-day, 달력일이 아니라
거래일 기준 — 공휴일/주말 때문에 사이클마다 어긋나지 않게)로 정렬해,
동일 D-day끼리 모아 평균/중앙값을 계산한다. 예: "D-10부터 D-0까지 각
시점의 베이시스 평균이 과거 N회 사이클에서 어땠는지"를 한 그래프로.

**정직하게 명시할 것**: 이건 과거 관찰 통계이지 "이번에도 이렇게 수렴할
것"이라는 예측이 아니다 — §5.40(스켈핑 적중률)과 동일한 원칙, 표본
수(사이클 개수)를 항상 그래프에 함께 노출한다.
...
| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.45-1 ✅ | 모달 필터 확장 | `FlowSummaryModal`에 "선물" marketFilter 옵션 추가(3M은 `fetchMarketSeries('futures', days)` 그대로 재사용해 전체 3투자자 지원, 1D는 `foreign-position/intraday-accumulated`의 futures 필드를 재사용해 외국인만) | 코드리뷰 + vite build 클린 + 실데이터 확인 — **완료(2026-07-30)**. curl로 K200 선물 개인/외국인/기관계 최근 9거래일 실데이터 확인(2026-07-29: 외국인 +17,126억, 기관계 -15,297억, 개인 -1,390억) |
| 5.45-2 ✅ | 문구 정정 | "선물 제외 — 투자자별 수급 미수집"(더는 사실이 아니었음) 제거, 1D+선물 선택 시에만 "1D는 외국인만 제공" 조건부 문구로 교체 | 코드리뷰 — **완료(2026-07-30)** |
| 5.45-3 ✅ | 컴팩트 타일 명확화 | "투자자별 수급 요약" → "투자자별 수급 요약 (현물)"로 섹션 제목 수정 | vite build 클린 — **완료(2026-07-30)**. 백엔드 변경 없음(FLOW_MARKETS에 이미 futures 포함돼 있던 걸 프런트가 안 쓰고 있었을 뿐). 브라우저 자동화 도구 이번 세션도 없어 코드리뷰+curl+빌드로 대체 |

### Phase 5.46 — 변동성 스트레스 점검에서 발견된 실버그 2건 수정 (2026-07-31)

사용자: "요즘 변동성이 장난 아니다.. 우리의 로직들이 제대로 작동하는지도
점검하기 좋은 자료가 될 수도 있어.. 지금 정보들 보면서 검토 해봐" — 서브에이전트
2개로 위험/경보 계열·스크리닝/랭킹 계열을 실데이터로 점검한 결과, 실제로
고칠 가치가 있는 버그 2건을 발견했다(나머지는 정상 작동 확인 또는 버그가
아닌 실제 시장 관찰로 판정).

**버그 1 — 키움 토큰 8005(무효) 자동 재발급 없음**: 실시간 라이브 기능
다수(`stock-flow-scan`, `attention`, `scalp-tracker`, `scalp-candidates`)가
`[3] 인증에 실패했습니다[8005:Token이 유효하지 않습니다]`로 계속 실패
중인데, `clients/kiwoom.py::call_tr`의 재시도 로직이 `return_code==5`
(rate limit)만 재시도하고 `return_code==3`(인증 실패)는 재시도 없이 바로
예외를 던진다 — 게다가 클라이언트는 로컬에서 계산한 `expires_at`만 믿고
서버가 실제로 그 토큰을 무효화했는지는 확인할 방법이 없어, 한 번 무효화되면
로컬 만료 시각(최대 24시간 뒤)까지 죽은 토큰을 계속 재사용한다. `backend`
(코드 수정마다 재시작)와 `worker`(상시 구동)가 각각 독립적으로 토큰을
발급/캐시할 수 있는 구조라 한쪽이 새로 발급하면 다른 쪽의 캐시가 (로컬
계산상으론 멀쩡해 보여도) 서버 쪽에서 무효화됐을 가능성이 유력하다.

**버그 2 — 정정(최초 진단보다 정확한 원인 파악됨)**: 처음엔 "flow_rank
수집기가 실패를 성공으로 거짓 기록한다"고 봤으나, 코드를 다시 읽어보니
**수집기 자체는 이미 정직하게 동작 중**이다(`collectors/flow_rank.py::
collect_flow_rank`) — 소스(네이버, 날짜 쿼리 불가·"최근 2거래일 고정"
응답만 줌)가 반환한 날짜가 target_date와 다르면 `message`에
"실제 적재된 날짜: ... (요청한 target_date=...는 무시됨)"라고 이미 정확히
남기고 있다(`collect_log` 직접 확인: 07-30 실행분 message에 실제 적재
날짜가 07-28/07-29로 찍혀 있음 — 07-30 저녁 18시 배치 시점엔 네이버가
아직 그날 최종 랭킹을 안 올렸을 가능성). **진짜 문제는 두 가지**: (a) 이
정보가 `message` 텍스트에만 있고 `status`는 그대로 `'ok'`라 아무도(이번
점검 포함) 걸러내지 못했다, (b) 더 근본적으로 **이 소스 특성상 18:00
정기 배치 시점엔 그날 데이터가 아직 없을 수 있다는 걸 알면서도 재시도
메커니즘이 없다** — §5.22(미국장 조기 수집)가 이미 "정기 배치 시점에
소스가 아직 준비 안 됐을 수 있다"는 똑같은 클래스의 문제를 "같은 잡을
다른 시각에 한 번 더 실행"으로 풀었던 선례가 있다 — flow_rank도 그 패턴을
그대로 적용한다.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.46-1 | 키움 토큰 8005 자동 재발급 | `clients/kiwoom.py::call_tr`에 `return_code==3`(인증 실패) 감지 시 로컬 캐시(메모리+파일) 모두 무시하고 강제 재발급 후 1회 재시도하는 로직 추가(무한루프 방지 — 기존 `max_retries` 상한 그대로 적용) | ✅ 단위테스트(모킹으로 8005 응답 → 재발급 → 성공 시나리오, `return_code==3`이 무관한 오류에도 재사용되는 범용 코드임을 확인해 메시지 키워드(8005/인증+실패)로 오탐 방지) — 실운영 로그에서도 8005 반복 발생 후 강제 재발급으로 정상화 확인 |
| 5.46-2 | flow_rank 저녁 재수집 크론 | §5.22와 동일한 패턴으로 `collectors/scheduler.py`에 18:00 정기 배치와 별개로 flow_rank만 다시 도는 저녁 늦은 시각(19:30 KST) 크론 추가 — 소스가 idempotent(재실행 안전)이므로 새 로직 없이 같은 collect_fn 재사용 | ✅ 수동 트리거 후 collect_log의 flow_rank message가 날짜 불일치 없이 최신 거래일을 반영함을 확인 |

### Phase 5.47 — 만기 수렴 패턴 그래프 해석 가이드 추가 (2026-07-31 사용자 지적)

사용자: "지수가 8000~7500 사이 뭐 이런식으로 예측을 못하나?"라는 질문에, §5.41
매수/매도 밀집 구간(절대 가격, 캔들차트 축)과 §5.42 베이시스 D-day 패턴(만기까지
남은 거래일 축)은 축 자체가 달라 겹쳐 그릴 수 없고, 둘을 합쳐 지수 도달 범위를
예측하는 건 검증 없는 예측 주장이라 house rule에 어긋난다고 설명하고 만들지
않기로 합의. 이어서 사용자가 "만기 수렴 패턴 그래프가 뭘 의미하는지, 간격
차이가 뭘 말하는지, 추세가 좋은지 나쁜지 모르겠다 — 있어도 도움이 안 된다"고
지적 — 그래프 자체를 새로 만들지 않고, 기존 §5.42 그래프에 해석 가이드 패널을
추가했다.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.47-1 | 만기 수렴 패턴 해석 가이드 패널 | `DashboardPage.jsx::ExpiryPatternModal`에 기본 펼침 `<details>` 패널 추가 — 베이시스 부호(콘탱고/백워데이션) 의미, D-0 수렴이 예측이 아니라 만기 정산 규칙상 당연한 구조적 결과라는 점, 평균-중앙값 간격의 의미, 추세 자체엔 좋고 나쁨이 없다는 점, pt/% 차이, 표본수(n) 신뢰도를 설명 | ✅ `npx vite build` 통과, 실 컨테이너 확인 |

### Phase 5.48 — 키움 주문 TR(kt10000~kt10003) + 계좌 조회 클라이언트 메서드 추가 (2026-07-31)

이 프로젝트 최초의 실거래(매수/매도) 기능. `KiwoomClient`에 매수(`kt10000`)/매도(`kt10001`)/정정
(`kt10002`)/취소(`kt10003`) 주문과 예수금상세(`kt00001`, 읽기전용)/미체결(`ka10075`, 읽기전용)
조회 메서드를 추가했다. 사용자가 합의한 이 기능 전체의 총 예산 5만원을 `MAX_ORDER_NOTIONAL_KRW`
하드 캡으로 `place_buy_order`/`place_sell_order` 내부에 박아 넣어, HTTP 호출 전에
notional(수량×단가)이 초과하면 무조건 `ValueError`로 막는다(우회 불가 — 테스트로 HTTP 호출
자체가 0건임을 확인). TR 스펙(요청 body 필드/URL)은 웹서치·GitHub 조사로만 확보했고 아직
실호출로 검증하지 않았다. 대시보드는 로그인이 없는 설계라 주문 관련 라우터는 의도적으로
추가하지 않았다 — 터미널에서만 호출 가능.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.48-1 | 주문/계좌 TR 클라이언트 메서드 | `clients/kiwoom.py`에 `place_buy_order`/`place_sell_order`/`cancel_order`/`get_deposit_detail`/`get_unfilled_orders` 추가, notional 하드캡을 HTTP 호출 전에 검사 | ✅ 단위테스트(하드캡 초과 시 HTTP 호출 0건 확인 포함) — kt10000~kt10003 전부 실호출 미검증 상태로 남음 |

### Phase 5.49 — 키움 주식호가(ka10004) 클라이언트 메서드 추가 (2026-08-02)

사용자: "호가 창을 알아야 가격을 매도, 매수 할 것 같은데.. 아무 생각 없이 현재가로 할 수 없잖아..
호가창에 지금 가격에 매물대를 확인해 봐야 할 것 같아." — 주문 직전 터미널에서 매도/매수 10단계
호가와 잔량(매물대)을 확인할 수 있는 `KiwoomClient.stock_quote()`를 추가했다(`ka10004`,
`/api/dostk/mrkcond` — 기존 `ka10063`/`ka10066`/`ka90013`과 같은 카테고리). 읽기 전용 조회라
`place_buy_order`류의 notional 캡은 불필요. TR 스펙 출처는 GitHub `Kiwoom-Securities/
Kiwoom-REST-API`(homepage가 openapi.kiwoom.com과 일치해 공식으로 보이는 조직) `kiwoom_docs/
시세.md` — kt10000류와 동일하게 실호출 미검증 상태.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.49-1 | 호가 조회 클라이언트 메서드 | `stock_quote(code)` 추가, `TR_RESOURCE_URL`에 `ka10004` 등록, 매도/매수 각 10단계 호가·잔량·총잔량·시간외잔량 필드 구조를 모듈 docstring에 기록 | ✅ 단위테스트(요청 shape 검증) — 실호출 미검증 |

### Phase 5.50 — 삼성전자/SK하이닉스 포지셔닝 통합 뷰 설계: 호가·ETF 괴리율·개별주식선물·SOX/나스닥선물 (2026-08-03 사용자 요청)

사용자: "매수매도를 해보려고 하는데, 실제 금액을 넣고 하기 이전에 충분히 모의 테스트를 우리
스스로 할 수 있게 만들어 보는 건 어때? 일단 매수 매도가 되는지 확인하는 건 차후로 검증하면
되고. 호가를 보고 매수 매도를 보고 어느 시점에 진입하는 게 좋을지, 하이닉스 혹은 삼성전자,
ETF 레버리지 2X, 인버스 2X를 가지고 해보는 거야. 본주 ETF 모두 봐야 할 것 같고, 필라델피아
반도체지수, 나스닥 선물 이런 걸로 지금 롱, 숏 어떤 걸 살지 고민도 하고." 이어서: "ETF 괴리율,
선물까지 다 봐야 할 거야.. 수급을 제일 영향받는 종목이 삼성전자, 하이닉스 이 두 개이니.. 이걸로
포지셔닝해보자."

**확인된 설계 원칙(사용자 확정, 2026-08-03 질의응답)**:
1. **관찰 데이터만 — 매매 신호/추천 절대 금지**: 이 도구는 §5 전체 원칙(house rule)을 그대로
   따른다. 호가·괴리율·선물가·SOX 값은 숫자 그대로 보여줄 뿐 "지금 진입"·"롱/숏 유리" 같은
   판단 문구를 생성하지 않는다 — 이 프로젝트의 다른 모든 관찰 지표(§5.15 백워데이션, §5.33
   업종 로테이션, §5.42 만기 수렴 등)와 동일한 태도. 판단은 전적으로 사용자가 한다.
2. **이번 Phase 범위 = 통합 뷰까지만**: 진입가/수량/청산가를 기록해 손익을 추적하는 "가상
   매매 장부"는 이번 설계에서 제외하고 별도 후속 Phase로 미룬다. 이번엔 "호가 + ETF 괴리율 +
   개별주식선물 + SOX/나스닥선물을 한 화면에서 보는" 통합 뷰까지만 만든다.
3. **실제 주문 API는 이 Phase에서 호출하지 않는다**: kt10000/kt10001(§5.48)은 여전히 미검증
   상태로 그대로 둔다 — 이번 작업은 순수 조회(읽기 전용) TR/데이터소스만 다룬다.
4. **모의투자 서버(mockapi.kiwoom.com)도 이번 범위 아님**: 사용자가 "우리 스스로 만들자"고
   명시했으므로 키움 공식 모의투자 신청 없이, 실전 키로 호출 가능한 시세 조회 TR만으로 통합
   뷰를 구성한다.

**대상 종목/상품(2026-08-03 조사)**: 2026-05-27 삼성전자/SK하이닉스 단일종목 레버리지·인버스
ETF/ETN 18종(ETF 16 + ETN 2, KODEX/TIGER/ACE/RISE/PLUS/KIWOOM/1Q/SOL 8개 운용사)이 이미
상장돼 있음을 확인했다. 그중 KIWOOM/1Q의 레버리지 상품과 PLUS/SOL의 인버스 상품은 현물이
아니라 **개별주식선물(주식선물)로 복제**한다 — 이 프로젝트가 지금까지 다룬 K200 지수선물과
별개로 삼성전자/SK하이닉스 개별 종목 대상 KRX 파생상품이 실제로 존재함을 이번 조사로
확인했다(실제 시세 조회 방법은 미확인, 5.50-1 조사 대상).

- 본주: 삼성전자(005930), SK하이닉스(000660)
- 레버리지 2X ETF(후보 — 유동성 큰 KODEX 브랜드 채택, 확정 전 재검토 가능): KODEX
  삼성전자레버리지(0193W0), KODEX SK하이닉스레버리지(0193T0)
- 인버스 2X(두 종목 다 인버스는 이 한 상품씩만 존재 — 나머지 16종 중 14종은 전부 레버리지):
  PLUS 삼성전자선물인버스2X(0193L0), SOL SK하이닉스선물인버스2X(0197X0)
- 참고 지수: 필라델피아반도체지수(SOX, 기존 `us_sox` 시리즈 — 현재 전일 종가 EOD만 수집,
  장중 참고용으로는 부족), 나스닥(선물 참고, 신규)

**5.50-1 데이터소스 조사 결과(2026-08-03 완료)**:
- **ETF NAV 실시간 괴리율 — 확정, 신규 TR 불필요**: 이미 쓰고 있는
  `clients/naver_etf.py::fetch_etf_list()`(무키, EOD 배치 `etf_stats`의 기존
  소스)가 종목마다 `nav`(NAV)와 `now_value`(현재가)를 **이미 같이** 준다 —
  즉 괴리율 = `(now_value - nav) / nav * 100`을 새 TR 조사 없이 바로 계산할
  수 있다. 실호출로 4개 타깃 티커 전부 확인(2026-08-03,
  단위: 원): KODEX 삼성전자레버리지(0193W0) nav 10900/now 10840, KODEX
  SK하이닉스레버리지(0193T0) nav 9616/now 9570, PLUS 삼성전자선물인버스2X
  (0193L0) nav 12246/now 12255, SOL SK하이닉스선물인버스2X(0197X0) nav
  9351/now 9305 — 4종 모두 정상 응답. 이 함수는 EOD 배치용으로만 쓰였지만
  라이브 호출 자체는 막혀 있지 않으므로, `attention`/`flow/live`와 동일한
  온디맨드+60초 캐시 패턴으로 재사용하면 된다(전체 1,155종목 중 4종만
  필터링해서 쓴다).
- **나스닥 선물(NQ=F) 준실시간 — 확정**: `yfinance.Ticker("NQ=F").history
  (period="1d", interval="5m")`로 5분봉이 정상 수신됨을 확인(2026-08-03,
  마지막 봉 21:10 EDT ≈ 같은 시각 KST 오전 — CME Globex가 KST 주간에도 계속
  열려 있어 "지금" 값에 가깝다). 기존 `clients/us_indices.py`(EOD 전용)와는
  별도로 인트라데이 조회 함수를 하나 추가하면 된다.
- **필라델피아반도체지수(SOX) — 추가 작업 불필요**: SOX는 미국 현물 지수라
  KST 주간(미국 장 마감 후)에는 애초에 "실시간"이 존재하지 않는다 — "어젯밤
  미국 반도체 지수가 어떻게 마감했는지"를 보는 지표이므로 기존 `us_sox`
  EOD 시리즈(§5.8)가 이미 정확히 맞는 데이터다. 업그레이드 대상에서 제외.
- **개별주식선물(주식선물) 시세 — 미해결, 보류**: 상품 자체(KIWOOM/1Q
  레버리지, PLUS/SOL 인버스가 이걸로 복제됨)의 존재는 확인했지만, 무료로
  시세를 조회하는 구체적 방법을 이번 조사에서 찾지 못했다. 이 항목은
  후순위로 미루고(아래 5.50-4는 보류), 나머지 확정된 소스로 먼저 진행한다.

### 탑다운(top-down) 프레임: 하이닉스를 베이지표로 쓰는 "오늘의 포지셔닝 재료" (2026-08-03 사용자 확장)

사용자가 이어서 지적: "전략이 단순하게 하나만 있으면 안 될 것 같아.. ①코스피/코스닥 수급이
어디로 쏠려있는가 ②미장 선행지표가 어떤가 ③오전에 외인·기관이 선물·현물을 매수/매도하는가
④개별 종목이 어떻게 반응하는가 ⑤기타.. 이런 걸 보면서 오늘은 어떤 걸 매수해야겠구나 라고
보는 거지.. 수급에 영향을 제일 많이 받는 하이닉스를 기준으로 이걸 하는 거야." 이 구조는
트레이딩에서 이미 이름이 있는 기법(**Top-Down Analysis** — 큰 맥락→중간 맥락→좁은 맥락 순으로
좁혀가며, 특정 종목을 시장 전체의 read-through/bellwether로 쓰는 것도 실무에서 흔한 기법)과
정확히 일치해, 이 표준 틀을 그대로 채택한다. 5개 항목 대부분이 이미 이 프로젝트에 흩어져
있어 **신규 개발보다 기존 엔드포인트 재조합이 핵심**임을 확인했다:

| 항목 | Top-down 단계 | 소스 |
|---|---|---|
| ① 코스피/코스닥 수급 쏠림 | 큰 맥락 | `GET /api/markets/regime`(§5.15, 기존) + `GET /api/markets/flow-concentration/intraday-accumulated`(§5.18/19, 기존) — 재사용, 신규 작업 없음 |
| ② 미장 선행지표 | 중간 맥락(오버나잇) | SOX EOD(기존, 그대로) + 나스닥선물 인트라데이(신규, 5.50-1에서 확정) |
| ③ 오전 외인·기관 선물/현물 매매 | 중간 맥락(장초반) | `GET /api/markets/foreign-position/intraday-accumulated`(기존 — 이미 현물+선물 둘 다 있음) — 재사용, "오전 구간" 라벨링만 프런트에서 추가 |
| ④ 개별 종목(하이닉스) 반응 | 좁은 맥락 | 신규: 하이닉스 등락률 − 코스피 등락률(상대강도, 관찰치일 뿐 판정 아님) + 기존 attention/flow_rank에 000660 노출 여부 |
| ⑤ 기타 | 보조 맥락 | 변동성/리스크 경보(§5.36, 기존) — 재사용 |

이 브리핑도 house rule을 그대로 따른다: ①~⑤ 숫자를 한 화면에 모아 보여줄 뿐 "그러니
오늘은 이거 사라" 같은 결론은 도구가 내지 않는다 — 그 종합 판단은 전적으로 사용자의 몫이다.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.50-1 | 데이터소스 조사 | ETF 괴리율/나스닥선물/SOX/개별주식선물 4가지 확정 — **완료**(위 절 참고) | ✅ 완료(2026-08-03) — 개별주식선물만 미해결로 보류 |
| 5.50-6 ✅ | "오늘의 포지셔닝 재료" 브리핑 패널(하이닉스 중심) | 새 프런트 섹션 — ①regime+쏠림 비율, ③외인/기관 현물·선물 오전 누적(`foreign-position/intraday-accumulated`), ⑤리스크 경보를 기존 엔드포인트 그대로 재사용해 하이닉스 중심 한 화면에 재배치. 신규 백엔드 없음(순수 프런트 조합) | ✅ 완료(2026-08-03) — `HynixPositioningModal`(DashboardPage.jsx), pytest 722개 통과, vite build 클린, 실 컨테이너 curl로 실데이터 확인, 판단어(강함/약함 등) 없음 자가점검 완료 |
| 5.50-7 ✅ | 하이닉스 상대강도(신규) | 000660 등락률 − 코스피 등락률(z-score 아님, 단순 초과수익률 관찰치) 계산 — 백엔드 소스는 이미 있는 stocks 등락률 + market series 재사용, 새 TR 없음 | ✅ 완료(2026-08-03) — `GET /api/markets/hynix-relative-strength`(기존 `_warm_index_tiles_live`/`_warm_stock_intraday`/`get_stock_series_from_db` 3개 재조합, 신규 캐시 없음), 단위테스트 3건(정상 계산/장마감 시 null/하이닉스 조회 실패해도 502 전파 안 함), 실데이터 확인(하이닉스 -7.42%, 코스피 -4.27%, 상대강도 -3.15%p) |
| 5.50-2 ✅ | 종목 페어 통합 뷰 | 본주+레버리지2X+인버스2X 3열, 현재가·등락률·호가 매물대(ka10004 재사용). 삼성전자/SK하이닉스 세트 토글 | ✅ 완료(2026-08-03) — `GET /api/markets/positioning/pair-view?set=samsung\|hynix` + `PairViewModal`(`QuoteLadder` 사다리). **이 작업 중 ka10004(주식호가요청)를 처음 실호출로 확정**(005930/000660/0193W0/0193T0/0193L0/0197X0 6개 코드 전부 정상 응답, 가격 필드에 `-` 부호 접두 실측 확인 — `clients/kiwoom.py` 모듈 docstring "주식호가요청(ka10004) 조사·실측 확정" 절 갱신). pytest 733개 통과, vite build 클린 |
| 5.50-3 ✅ | ETF 괴리율 표시 | 5.50-1에서 확정한 `naver_etf.fetch_etf_list()` 재사용, 레버리지/인버스 ETF 열에 NAV 대비 괴리율(%) 추가 | ✅ 완료(2026-08-03) — 5.50-2와 같은 커밋. 실데이터 확인(예: SK하이닉스 인버스2X 괴리율 -0.69%, 삼성전자 레버리지 -0.32%) |
...
### Phase 5.53 — 지수 타일 "장 마감 직후 며칠 전 데이터로 보이는" 버그 수정 (2026-08-03 사용자 지적)

사용자: "오늘 정보로 나와야 하는데 31일 정보로 나오는건 뭐지? 장 마감 이후 말야."
— 실측(월요일 16:08 KST, 정규장은 15:30에 마감)으로 `GET /api/markets/
index-tiles/live`가 `date: "20260731"`(지난 금요일, 주말 낀 3거래일 전)을
반환하는 걸 재현했다.

**원인**: `_warm_index_tiles_live`가 정규장 마감(`_market_closed_kst`) 여부만으로
라이브 호출을 완전히 건너뛰고 곧장 `index_ohlcv` DB 확정치로 폴백했는데, 그
확정치는 평일 18:00 배치(§ `collectors/scheduler.py`)가 돌아야 그날 날짜로
갱신된다 — 즉 **정규장 마감(15:30)과 배치 실행(18:00) 사이 2.5시간 동안은
항상 "그 이전 거래일" 데이터가 나온다**(주말이 끼면 최대 3거래일 전으로 보임,
이번에 실제로 관찰된 사례). 라이브 소스(`ka20005` 분봉/`naver_index` 일봉)는
정규장이 끝난 뒤에도 그날 마지막 봉을 그대로 갖고 있음을 실측 확인했다(16:08
KST에 재호출해도 15:30 종가 봉이 마지막 — NXT 연장 데이터가 섞이지 않아 "정규장
종가" 의미도 그대로 유지됨).

**수정**: 장 마감이어도 DB 확정치 날짜가 오늘이 아니면(배치 미실행) 라이브를
한 번 더 시도하도록 `_warm_index_tiles_live`(`routers/markets.py`)를 고쳤다 —
확정치가 이미 오늘 날짜면(배치가 이미 돎) 라이브를 다시 부르지 않고, 라이브도
실패하면(정말 휴장일 등) 기존처럼 옛 확정치로 최종 폴백한다. 이 분기는
scheduler의 NXT 게이트(밤/주말엔 이 함수 자체가 안 불림) 안에서만 실행돼
새벽 낭비 호출 걱정은 없다.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.53-1 ✅ | 라이브 재시도 분기 | `_warm_index_tiles_live`에 "장마감+확정치가 오늘 아님" 케이스에서 라이브 1회 재시도 추가 | ✅ 완료(2026-08-03) — 회귀 테스트 2건 추가(재시도로 오늘 데이터 채워짐 / 재시도도 실패하면 옛 확정치로 폴백), 기존 2건은 확정치를 오늘 날짜로 바꿔 "이미 신선하면 라이브 안 부름" 의도를 유지하도록 수정. pytest 773개 통과. 실 컨테이너 curl로 `date: "20260803"`(이전엔 "20260731") 확인 |

### Phase 5.54 — 자동매매 엔진: 트레일링 스탑(SOL AI반도체TOP2플러스, 0167A0) + 전용 탭 (2026-08-04)

사용자가 실제 계좌에 2만5천원을 입금하고 완전자동매매를 요청했다. 이 프로젝트가 지금까지
지켜온 "관찰 데이터만 제공, 판단은 사용자 몫" 원칙(§5)의 **명시적 예외**다 — 사용자가
스스로 이 예외를 요청했고, 전략 규칙 자체(진입/청산/손절 조건)도 사용자가 직접 확정했다.
이 Phase는 그 규칙을 실제로 실행하는 엔진을 만드는 것이지, 새로운 판단 로직을 AI가
발명하는 게 아니다.

**대상 — 2026-08-04 실호출 검증 중 변경됨**: 처음엔 SK하이닉스 본주(1주 159만원대, 예산으로
살 수 없음)/삼성전자(24만원대) 대신 **KODEX SK하이닉스레버리지(0193T0, §5.50 인프라
재사용)** 단독으로 정했으나(2X 롱만, 인버스 0197X0은 범위 밖), 실제로 0193T0에 테스트
주문을 넣어보니 `[2000] 파생상품 ETF 거래신청 등록 후 주문이 가능합니다`로 거부됐다 —
레버리지/인버스 계열은 "파생상품 ETF"로 분류돼 계좌에 별도 적합성 심사(파생상품 이해도
테스트/거래신청)가 돼 있어야 한다(코드 문제 아님, §5.48 절 "2026-08-04 실전 계좌 실호출
검증 결과" 참고). 이 등록이 §5.50 4개 타깃 ETF(0193W0/0193T0/0193L0/0197X0) 전부에
적용될 가능성이 높다. **그래서 대상을 일반(1배수, 비합성) ETF로 변경했다**: 반도체 테마
ETF 중 가격(2만5천원 예산 안)과 유동성(하루 거래대금)을 함께 만족하는 **SOL AI반도체
TOP2플러스(0167A0, 2026-08-04 기준 현재가 16,060원, 하루 거래대금 273억원)**로 확정 —
같은 날 실제로 이 종목에 테스트 주문을 넣어 파생상품 등록 없이 정상 접수됨을 확인했다.

**전략 규칙(사용자 확정, 2026-08-04)**:
- **진입**: `GET /api/stocks/{code}/signals`(§5.3, 이미 있음)의 `ma_cross.state == "golden"`
  **AND** `volume_spike.is_spike == true` — 새 지표 발명 없이 기존 함수 재사용.
- **청산(트레일링 스탑)**: 진입가 대비 +1% 도달 시 "트레일 모드"로 전환(그 전까지는 손절
  조건만 감시하며 보유). 트레일 모드 진입 후에는 추세가 살아있는 동안(신고가 갱신 중)
  계속 보유하고, 추세 반전(`ma_cross.state == "dead"`)이 확인된 뒤 진입가 대비 +0.5%까지
  되돌아오면 매도.
- **손절**: 진입가 대비 -1.5%(제안값, 사용자 이견 없으면 확정) — 트레일 모드 진입 여부와
  무관하게 항상 감시, 도달 즉시 매도.
- **수량**: 1주 고정.

**중대 안전 설계(§5.48에서 이미 예견된 문제를 지금 푼다)**: `clients/kiwoom.py`의
`MAX_ORDER_NOTIONAL_KRW`(5만원) 정의부 주석이 이미 "이 체크는 주문 1건만 보고 누적
노출은 못 막는다 — 자동매매를 만들 때는 별도로 다뤄야 할 후속 작업"이라고 명시해 뒀었다.
지금이 그 시점이다 — 신규 **누적 예산 가드**(`AUTO_TRADE_TOTAL_BUDGET_KRW`, 기본값
25,000원 = 실제 입금액)를 추가해, "이미 보유 중인 포지션 notional + 이번 주문 notional"이
이 값을 넘으면 주문 자체를 내지 않는다. 상태 기계가 한 번에 포지션을 하나만 허용하므로
실질적으로는 "이미 보유 중이면 추가 매수 안 함"과 같지만, 방어적으로 명시적 체크를 둔다.

**선행 조건(반드시 이 순서로)**: kt10000/kt10001(매수/매도)은 **아직 실호출로 단 한 번도
검증되지 않았다**(§5.48). 이 미검증 API 위에 자동화를 얹는 건 위험하므로, 자동매매 엔진을
만들기 전에 먼저 **수동으로** 안전 검증(체결 불가능한 가격으로 1주 매수 주문 → 미체결
조회로 확인 → 취소)을 완료해야 한다. 이 검증은 메인 세션이 사용자와 함께 장중에 직접
수행한다(서브에이전트 위임 없음 — 실제 돈이 걸린 최초 실호출이라 델리게이션하지 않는다).

**전용 탭**: 사용자 지적 — "완전자동매매인데 히스토리 관리해야, 탭 하나 따로 만들어야,
대시보드에서 관리하면 안 된다." 새 최상위 페이지(`AutoTradePage.jsx`, 기존
`DashboardPage.jsx`와 분리)에 킬스위치(즉시 정지 토글), 현재 상태(idle/holding/trailing +
진입가/평가손익), 전체 실행 히스토리(감사 로그)를 표시한다.
...
### Phase 5.54-5 — 진입 신호가 폴링 타이밍에 밀려 누락되는 버그 수정 (2026-08-05 사용자 지적)

사용자: "자동매매를 시도한 적이 없네?" → "너무 조건이 까다로와서 매수가 안 되는거
아니야?" — 킬스위치는 정상적으로 켜져 있었고(24시간 658회 폴링 확인, 백엔드
재시작 없음) 실행 자체는 문제없었지만, "조건이 너무 까다로운가"를 실제 데이터로
검증했다.

**실측**: 2026-08-05 오늘자 0167A0 1분봉을 그대로 재생해 매 시점 골든크로스/
거래량 스파이크를 다시 계산해 보니, 골든크로스 5회·스파이크 9회 중 **실제로
둘이 동시에 충족된 순간이 1번(10:33 KST) 있었다** — "조건이 너무 까다로워서"가
아니라, 그 순간이 실제로 있었는데 못 잡았다는 뜻이었다. 원인 확정: `moving_
average_cross`는 "교차가 일어난 바로 그 1분봉"에서만 "golden"이고 다음 봉부터는
이미 "none"으로 돌아가는 **순간 이벤트**다. 60초 폴링(10:33:11, 10:34:11 …)과
1분봉 경계가 정확히 안 맞아떨어지면, 폴링이 도는 시점에 이미 "최신 봉"이 그
교차 봉을 지나가 버려 신호를 통째로 놓친다 — 실제 로그로 폴링 자체는 정상
실행됐음(10:33:11/10:34:11)을 확인해 이 가설을 확정했다.

**수정**: 조건 자체(골든크로스 AND 거래량 스파이크)는 그대로 두고, "지금 이
순간"만 보던 걸 **"최근 `ENTRY_SIGNAL_LOOKBACK_BARS`(3봉, 약 3분) 이내에 있었는지"**
로 감지 시점만 넓혔다(`collectors/auto_trader.py::_golden_cross_in_lookback`/
`_volume_spike_in_lookback` 신규, `quant/auto_trade_rules.py`의 판정 로직 자체는
불변). 청산(dead cross) 판정은 이번 범위 밖(최신 봉 기준 그대로) — 이번에
실제로 관측된 진입 쪽 문제만 고쳤다.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.54-5 ✅ | 진입 신호 룩백 감지 | 최근 3봉 이내 골든크로스/거래량 스파이크를 재확인하는 헬퍼 2개 추가, `_handle_idle`이 이 값을 쓰도록 변경 | ✅ 완료(2026-08-05) — 2026-08-05 실제 0167A0 1분봉(10:33 골든+스파이크 동시 발생 실사례)을 그대로 테스트 픽스처로 박아 회귀 테스트 7건 작성(정확한 순간/1봉 지연/2봉 지연은 잡힘, 3봉 지연은 룩백 창 밖이라 안 잡힘, naive 방식은 1봉만 늦어도 놓친다는 대조군 포함). 기존 auto_trade 테스트 56건 전부 유지. 실 컨테이너에 핫리로드로 배포 후 라이브 데이터로 재확인, 이 작업 내내 킬스위치는 계속 켜진 상태를 유지했고(테스트 스위트가 같은 dev DB의 실제 싱글턴 행을 건드리지만 snapshot/restore로 안전하게 원복됨을 확인) 실주문은 발생하지 않음 |

**검증 중 발견한 무관한 기존 이슈(추가)**: `tests/test_markets_positioning_hitrate_router.py`의 "빈 결과" 전제 테스트가, §5.52 수집기가 실제로 2일 넘게 돌면서 쌓인 진짜 운영 데이터(dev DB 공유) 때문에 더 이상 "행 0개"를 보장 못 해 실패하기 시작했다 — 테스트가 실제 운영 데이터와 격리돼 있지 않은 기존 설계 결함이며 이번 작업과 무관. 후속 조사 과제로 남겨 둔다.

### Phase 5.54-6 — 보유 중 손절 감시를 실시간 호가 기반 30초 잡으로 분리 (2026-08-05 사용자 요청)

사용자: "매수하고 나서는 polling 간격은 어떻게 되니? 적어도 30초마다 해야 하지
않을까? 1분이니?" — 분석 결과 "폴링만 30초로 당겨도 소용없다"는 걸 먼저
확인했다: 손절 판정에 쓰는 1분봉(`ka10080`) 자체가 60초 캐시로 묶여 있어서,
더 자주 조회해도 어차피 같은 봉을 다시 받을 뿐이다. 반응속도를 실제로
올리려면 데이터 소스 자체를 캐시 없는 실시간 호가(`stock_quote`, ka10004)로
바꿔야 한다는 걸 사용자에게 설명하고 동의를 얻었다.

**설계**: 진입(골든크로스/거래량 스파이크)·트레일 전환·트레일 청산(dead
cross)은 전부 1분봉 기반 기술적 지표라 더 자주 봐도 의미가 없어 그대로 60초
`run_auto_trade`가 전담한다. **손절만** 실시간 호가로 30초 간격 전용 잡
(`collectors/auto_trader.py::watch_stop_loss`, `collectors/live_refresh.py`의
새 `_run_auto_trade_watch`, `IntervalTrigger(seconds=30)`)으로 분리했다 — 진짜
현재가(매수 1호가)로 손절 조건을 확인하고, 충족되면 방금 조회한 그 가격을
그대로 매도 지정가로 써서 재조회 없이 즉시 체결을 시도한다.

**안전장치(신규 잡 추가로 생긴 새 위험 — 중복 매도)**: 60초 `run_auto_trade`와
30초 `watch_stop_loss`가 동시에 같은 포지션을 보고 동시에 매도를 시도할 수
있다 — 모듈 전역 `asyncio.Lock`(`_position_lock`)으로 상태를 "읽는 시점부터"
직렬화해 막는다(두 함수 모두 락 안에서 상태를 읽고 판단하고 쓴다). 회귀
테스트로 `asyncio.gather`로 두 함수를 실제 동시 실행시켜 실제 매도 주문이
**정확히 1건만** 나가는지 확인했다.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.54-6 ✅ | 실시간 호가 기반 30초 손절 감시 | `watch_stop_loss` 신규(손절만, 진입/트레일 전환/청산은 건드리지 않음), `_position_lock`으로 60초 잡과 중복 매도 방지, `live_refresh.py`에 30초 전용 스케줄러 잡 추가(NXT 게이트) | ✅ 완료(2026-08-05) — 회귀 테스트 6건(킬스위치 꺼짐/idle 상태/손절 미충족/손절 충족·실시간가로 매도/trailing 상태 손절/매도 실패 시 상태 보존) + 동시 실행 시 중복 매도 방지 테스트 1건(`asyncio.gather`로 두 함수 동시 실행 → 실제 매도 1건만). 실 컨테이너에 배포 후 30초 간격으로 정상 실행 중임을 로그로 확인 |

**이 작업 도중 실제로 첫 자동매매 진입이 발생했다**: 0167A0 1주를 16,935원에
실제 매수(05:02:48 UTC, `signal_snapshot`에 `golden_cross_recent: true`로
찍힘 — §5.54-5 룩백 수정이 실제로 신호를 잡아낸 최초 사례). 이 사실을
테스트 도중 발견했다 — `tests/test_auto_trader_collector.py`의 로그 검증
헬퍼(`_log_rows`)가 테스트 시작 시점 워터마크 없이 테이블 전체를 읽고
있어서, 실제 운영 로그가 섞여 테스트가 무더기로 실패하기 시작한 게 발견의
계기였다(다른 테스트 파일들처럼 워터마크 필터를 추가해 수정, PLAN.md
"검증 중 발견한 무관한 기존 이슈"와는 별개 — 이건 이번 작업으로 실제
운영 로그가 처음 생기면서 드러난 이 파일만의 결함).

### Phase 5.54-7 — 백엔드 CPU 90~120% 점유 버그 발견·수정: 나스닥선물을 하루 1회만 (2026-08-05)

사용자: "피씨가 버벅거려.. ps 검사도 해줘봐" — `docker stats`로 확인해보니
`stock-backend-1` 컨테이너가 CPU 90~120%(코어 하나 이상)를 지속적으로 쓰고
있었다. `docker top`으로 내부 프로세스를 보니 `multiprocessing.spawn_main`
서브프로세스가 몇 시간째 누적 CPU를 먹고 있었고, 직접 재현 테스트로
**yfinance(나스닥선물 조회, §5.50-5)를 호출할 때마다 서브프로세스가 생기고
제대로 정리되지 않는다**는 걸 확인했다. `docker restart stock-backend-1`로
즉시 정상화(CPU 90~120% → 1% 미만)했다 — 킬스위치·보유 포지션은 DB에
저장돼 있어 재시작해도 유실 없음을 확인 후 진행.

재시작만으로는 같은 문제가 다시 쌓일 수 있어 근본 원인(yfinance 호출
빈도)을 줄였다. 사용자 확인: "나스닥 선물은 자주 확인할 필요 없어.. 코스피,
코스닥을 주로 하기 때문이야.. 한국 장 시작 전에 확인용으로 쓰면 될거야..
오전 8시 이전에 한 번만 하면 될 것 같아."

**수정**: `routers/markets.py`의 나스닥선물 캐시 TTL을 300초(5분)→20시간으로
늘리고, `collectors/live_refresh.py`에 평일 07:50 KST 1회 실행되는 새 잡
(`_run_nasdaq_futures_morning_job`, `CronTrigger`)을 추가해 장 시작(09:00)
전에 하루 한 번만 미리 채워 둔다 — 그날 나머지 시간은 대시보드가 이 캐시를
그대로 재사용해 yfinance를 다시 부르지 않는다. **이 잡을 `collectors/
scheduler.py`(18:00/07:30/19:30 cron, `worker` 컨테이너)가 아니라
`live_refresh.py`(`backend` 컨테이너)에 둔 이유**: 이 캐시는 DB가 아니라
`backend` 프로세스의 인메모리 딕셔너리라, 다른 프로세스(`worker`)에서
워밍해봤자 `backend`의 실제 캐시는 안 채워진다 — 배치 잡들과 다른 종류의
제약이다.

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.54-7 ✅ | 나스닥선물 호출 빈도 축소 | TTL 300초→20시간, 07:50 KST 1회 아침 워밍 잡 신규(`live_refresh.py`, backend 프로세스 내 — 인메모리 캐시라 worker에 두면 무의미) | ✅ 완료(2026-08-05) — `docker restart`로 즉시 CPU 정상화(90~120% → 0.3~1%) 확인, 코드 수정 배포 후에도 정상 유지 확인. pytest 834개 통과(기존 무관 이슈 1건 제외) |

### Phase 5.55 — 자동매매 전략 대대적 개편: 오버나잇 갭 리스크 실사고 이후 (2026-08-06)

**실제 사고**: 2026-08-05 14:02 KST 골든크로스+거래량 스파이크로 0167A0 1주 진입
(16,935원). 장중 손절(-1.5%)에는 한 번도 안 닿았고(최저 -0.74%) 트레일 발동
(+1%)도 없이 장 마감(15:30)까지 그냥 보유 상태로 넘어갔다 — **이 전략에 "장
마감 전 포지션 정리" 규칙이 아예 없었다.** 밤사이 갭하락이 나서 다음날(08-06)
08:50 KST 장 재개 직후 30초 실시간 손절 감시(§5.54-6)가 즉시 반응했지만, 이미
가격이 손절선을 훌쩍 넘어 있어 **16,110원(-4.87%)에 손절 체결**됐다 — 의도한
-1.5%가 아니라 -4.87%. 예산(25,000원) 대비 실현손실 -825원(약 3.3%). 장이
닫혀 있는 동안은 어떤 감시도 팔 수 없다는, 오버나잇 보유 전략의 근본적 한계가
실제 돈으로 증명된 사례.

사용자: "결국 어제 장 막바지에 추세를 보고 결정해서 오버나잇을 관리 했어야
했어" + "특정 숫자만 보면 안 되고 추세를 보면서 대응전략을 보고 매수, 매도를
해야 함." 사고 직후 **킬스위치를 끄고**(포지션은 이미 손절로 청산된 상태)
안전하게 재설계하기로 합의했다.

**사용자가 지적한 4가지, 구체 규칙으로 변환**:

| 지적 | 구체 규칙 | 재사용 재료 |
|---|---|---|
| ①시가/종가 변동성 큼 | 개장 후 10분(09:00~09:10)·마감 전 10분(15:20~15:30) **신규 진입 금지**(손절/청산은 시간 무관 항상 허용 — 안전장치엔 예외 없음) | 신규 시간 필터 |
| ②종가 베팅은 상승장에서만, 오버나잇 무지성 금지 | **15:20 KST 이후 첫 폴링에서 보유 중이면 조건부 강제 청산** — 리스크 경보 활성 시 무조건 청산. 아니면 (a)현재 평가손익 > 0 (b)`status == "trailing"`(장중 한 번이라도 +1% 넘어 트레일 진입한 이력 — 그냥 "holding"인 채로 마감을 맞으면 그 자체가 "이 진입이 안 먹혔다"는 신호라 오버나잇 자격 없음) (c)코스닥 외국인 `confirmed_streak >= 0`(연속 매도 중 아님) **셋 다** 충족해야 오버나잇 허용, 하나라도 실패하면 무조건 청산 | `/api/markets/regime`(§5.15)의 `kosdaq.외국인.confirmed_streak`, 자체 status/평가손익 |
| ③장중 변동성 급등 대응 | 리스크 경보(서킷브레이커/사이드카/거래량급증/수급가속도, §5.36) 활성 중엔 **신규 진입 금지** + 보유 중이면 손절선을 일시적으로 -1.5%→-0.8%로 타이트하게 조정 | `index-tiles/live`의 `risk` 필드(이미 있음) |
| ④수급 변동 따라 대응 변경 | 보유 중, 진입 시점 대비 외인 현물 누적 순매수 **부호가 반전**(매수→매도)되면 가격 조건과 무관하게 조기 청산 검토(로그에 근거 명시) | `foreign-position/intraday-accumulated`(§5.50-6) |

**설계 원칙**: 이번에도 새 판단을 AI가 지어내지 않는다 — 위 4개 규칙은 전부
사용자가 제시한 방향을 이미 있는 지표(regime/risk_alert/foreign-position)에
그대로 매핑한 것이다. house rule 그대로, 매매일지 reason에는 "무엇을 봤고 왜
그렇게 했는지"만 정직하게 서술한다.

**우선순위**: ②(오버나잇 조건부 청산)가 이번 사고를 직접 막았을 규칙이라
최우선 구현. ①③④는 같은 Phase 안에서 이어서 구현.
...
### Phase 5.60 — 전체종목 스크리너 첫 실행 실측: 예상 90분 → 실제 3.5시간, 원인은 N+1 upsert (2026-08-08)

**계기**: "PC 성능 다시 점검해봐" — 이번엔 Docker 자체는 깨끗했다(backend
0.35%, worker 0%). 대신 어젯밤(8/7) 처음 실행된 §5.57 전체종목 스크리너
배치 로그를 보니 20:35→00:10 KST, **3시간 35분**이 걸려 있었다(설계 시
예상 70~90분의 2.5배 이상). 결과 자체는 정상(3,907/3,907종목 스캔, 14건
매칭)이라 그날 밤엔 못 느꼈지만 다음 배치(18:00 본배치)와 안전 마진이
줄어드는 문제라 원인을 팠다.

**원인**: `routers/stocks.py::_upsert_flow_rows`가 ka10059 응답 행 하나마다
`session.execute()`를 개별 호출했다 — 종목 하나당 최대
`FLOW_BACKFILL_DAYS`(90)일 x 투자자 13종 ≈ 최대 780개의 **개별 DB 왕복**.
이 함수는 원래 종목 상세를 온디맨드로 열 때(§5.50-2) 딱 한 종목만 처리하는
용도라 무해했는데, §5.57의 전체종목 스윕이 이 함수를 3,907번 반복 호출하며
비용이 그대로 곱해졌다. 벤치마크로 직접 재현·확인: 동일한 780행을 개별
execute로 upsert하면 1.557초, 다중값 `INSERT .. ON CONFLICT` 하나로 배치하면
0.172초(**약 9배**) — 3,907종목에 곱하면 이 한 함수만으로 최소 90분 이상
차이가 난다. (참고로 `run_job`이 배치 전체를 커밋 없이 한 트랜잭션으로
묶어 두는 것도 있어, 실제로는 트랜잭션이 커질수록 개별 execute 비용이 더
늘었을 가능성이 있다 — 정확한 기여도는 다음 실행 결과로 확인.)

**조치**: `_upsert_flow_rows`를 다중값 `pg_insert(...).values([...])` 배치
upsert 하나로 바꿨다 — `on_conflict_do_update` 결과(최종 DB 상태)는 완전히
동일하고 왕복 횟수만 준다. 이 함수는 `_ensure_flows_cached`(온디맨드 종목
상세)와 `accumulation_screener`(전체종목 스윕) 둘 다 공유하므로 양쪽 다
같이 빨라진다.

**검증**: `venv/bin/python -m pytest -q` — 902 passed(무관 기존 실패 1건
제외, 동작 변경 없음이라 기존 테스트 그대로 통과). 벤치마크 스크립트로
개별/배치 두 구현을 실제 dev Postgres에 대해 직접 측정해 위 수치(1.557s →
0.172s) 확보. 실 컨테이너 재시작 후 `GET /api/stocks/005930/series?days=90`
호출로 실제 삼성전자 62거래일치 13개 투자자 수급이 정상 반환됨을 확인(응답
자체는 키움 API 호출 포함 5.4초 — 이건 네트워크 호출 비용이지 upsert
비용이 아님). 킬스위치/포지션(idle) 유지 확인. `stock-worker-1`도 재시작.
**다음 평일 20:00 KST 정규 실행에서 실제 단축 폭을 로그로 재확인할
예정**(이번 조치 후 첫 풀스윕이라 사전 추정이 아니라 실측으로 검증).

### Phase 5.61 — 종목별 프로그램매매 추세 요약(연속일수 + 누적 순매수) (2026-08-10)

**계기**: 사용자가 알테오젠(196170) 종목상세에서 프로그램매매 순매수 추이를
보다가 "프로그램이 매수로 보이면 외인이 산 걸로 나온다"는 패턴을 지적했다.
실 데이터로 직접 대조해 확인(11거래일 중 8~9일 프로그램=외국인 방향·규모
거의 일치, `총 순매수 프로그램=키움 ka90013` 재사용) — 한국 시장에서
프로그램매매(알고리즘/DMA)가 외국인 창구 비중이 높다는 시장 구조상 자연스러운
결과다. 이 관계를 매일 확인하는 대신 "추세 지표"(누적 순매수 + 연속
순매수/순매도 일수)로 한눈에 보고 싶다고 확인.

**구현**: `routers/stocks.py::_compute_program_trade_summary`(순수 함수) —
연속일수는 `quant/regime_backtest.py::next_streak`(시장 전체 수급 스트릭에
이미 쓰는 것과 동일한 알고리즘)을 그대로 재사용, 새 판단 로직 없음. 최근
5/10거래일 누적 순매수는 단순 합산(None 제외). `GET /api/stocks/{code}/series`
응답에 `program_trade_summary` 필드로 추가, 프론트
`StockDetailModal.jsx`의 `ProgramTradeSection`에 요약 한 줄로 표시(빨강=순매수/
파랑=순매도, house rule대로 판단·추천 문구 없이 관측값만).

**부수 정리**: 같은 파일의 `_upsert_program_trade_rows`/`_upsert_short_selling_rows`도
§5.60에서 고친 `_upsert_flow_rows`와 동일한 N+1 개별 execute 패턴이라 같이
배치화(동작 동일, 왕복만 감소) — 이 둘은 온디맨드 단일 종목 조회에만 쓰여
지금까지는 체감 영향이 작았지만, 향후 다른 배치가 재사용할 경우를 대비해
미리 정리.

**검증**: `venv/bin/python -m pytest -q` — 911 passed(무관 기존 실패 1건
제외, 신규 단위테스트 9개 포함). 프론트 `npm run build` 정상. 실 컨테이너
재시작 후 `GET /api/stocks/196170/series` 직접 호출로 `program_trade_summary`
필드가 실제 값으로 반환됨을 확인.

### Phase 5.62 — 장마감 후 breadth/flow/fx live가 최대 3일 지난 데이터를 보여주던 버그 수정 (2026-08-10)

**계기**: "장마감 했는데 대시보드 지수를 보면 7일(이전) 정보임 — 상세 정보는
오늘꺼고. 다른곳까지 확인 바람." 실 서버(월요일 15:30 정규장 마감 후 16:47
KST)에 직접 curl로 대조한 결과:

| 엔드포인트 | date | 상태 |
|---|---|---|
| `breadth/live` | 2026-08-07(금, 3일 전) | ❌ |
| `flow/live` | 2026-08-07 | ❌ |
| `fx/live` | 2026-08-07 | ❌ |
| `index-tiles/live` | 2026-08-10(오늘) | ✅ |
| `basis/live` | 2026-08-10 | ✅ |
| `futures-flow/live` | 2026-08-10 | ✅ |

**원인**: `_warm_breadth_live`/`_warm_flow_live`/`_warm_fx_live`(모두
`routers/markets.py`)는 "장 마감이면 무조건 DB 확정치로 응답, 라이브는
아예 재시도 안 함"이었다. DB 확정치는 그날 18:00 KST 배치가 돌아야 채워지므로,
정규장 마감(15:30)~배치(18:00) 사이 + 주말이 낀 월요일에는 최대 사흘 지난
값이 계속 나온다. **`_warm_index_tiles_live`는 이미 2026-08-03에 사용자가
똑같은 패턴("장마감 이후인데 며칠 전 정보가 나온다")을 지적해 고쳐진 전례가
있었는데**(라이브 소스는 정규장 마감 뒤에도 그날 마지막 값을 그대로 유지함을
실측 확인, "DB 확정치 date가 아직 오늘이 아니면 라이브를 한 번 더 시도"),
그 수정이 breadth/flow/fx 세 곳에는 전파되지 않았다.

**조치**: 검증된 동일 패턴을 세 함수에 그대로 적용 — 장 마감이어도 DB
확정치의 `date`가 오늘이 아닌 시장(코스피/코스닥 독립 판단)에 한해 라이브
소스를 한 번 더 시도하고, 실패하면 옛 확정치로 최종 폴백(502 아님, 기존
정책 유지). `_warm_flow_live`는 재시도가 필요한 시장이 있을 때만
`KiwoomClient` 인스턴스 하나를 열어 재사용(이 파일 기존 관례). `fx/live`는
자체 docstring에 이미 "KRX 정규장과 무관한 시장이라 저녁 시간대 과도하게
막힘"이라고 적혀 있던 알려진 한계였는데 같은 조치로 해결됨.

**검증**: `venv/pytest -q` — 920 passed(무관 기존 실패 1건 제외, 세 라우터
테스트 파일에 "확정치가 오늘이면 라이브 재시도 안 함"/"확정치가 옛날이면
재시도"/"재시도도 실패하면 폴백, 502 아님"/"장중 회귀 없음" 케이스 추가).
실 컨테이너 재시작 후 세 엔드포인트 직접 curl 재확인 — `flow/live`/`fx/live`
`date: "2026-08-10"`(오늘)로 정상화, `breadth/live`는 라이브 응답 자체가
원래 date 필드가 없는 스키마(기존 장중 라이브 형태와 동일, 이번 수정과
무관)라 실제 값(코스피 상승697/하락183 등)이 갱신됐는지로 확인 — 정상.

### Phase 5.63 — 수동 매수/매도 버튼 + 테스트가 실계좌에 매도 시도를 낸 사고 발견·수정 (2026-08-11)

**계기**: "자동매매가 지금 급반등을 감지 못하고 있다 — 내가 직접 할 수 있는
버튼을 만들어줘." 이어서 "내가 개입하지만, 자동매매가 감지해서 알아서
매수·매도 하는 것도 유지해야 해"로 요구사항 확정 — 자동 감지 엔진
(`run_auto_trade`/`watch_stop_loss`)은 그대로 두고 그 위에 수동 매수/매도
버튼을 얹는다.

**구현**(`routers/auto_trade.py`에 추가, 자동 엔진 자체는 무변경):
- `POST /api/auto-trade/manual-buy` — 자동 진입(`_handle_idle`)과 동일한
  안전장치를 전부 통과해야 함: ①킬스위치 꺼짐→409(**수동이라고 우회하지
  않음** — 꺼진 채로 진입하면 이후 손절 감시도 같이 꺼진 상태라 무방비
  포지션이 되기 때문), ②이미 보유 중→409, ③예산 가드(`check_entry_budget`,
  누적 25,000원+주문 1건 5만원) 실패→400, ④키움 주문 실패→502(상태 불변,
  `event_type=error` 로그만). 통과 시 `_best_fill_price`/state 전이/
  `entry_foreign_flow_sign` 기록까지 `_handle_idle`과 동일하게 수행,
  `AutoTradeLog`에 `event_type=manual_entry`로 기록.
- `POST /api/auto-trade/manual-sell` — **킬스위치 상태를 확인하지 않는다**
  (매도는 항상 최우선 허용 원칙). status가 holding/trailing 아니면 409.
  통과 시 손절/트레일청산/강제청산이 전부 공유하는 `_execute_sell`을
  재구현 없이 그대로 호출, `event_type=exit_manual`.
- 둘 다 자동 엔진과 동일한 `_position_lock`을 잡아 동시 자동 매매와 경합
  방지. 수동 매수 성공 후에는 그 포지션을 자동 손절/트레일링/EOD강제청산이
  그대로 이어서 관리한다(자동 감지 유지 요구사항 충족).
- 프론트(`AutoTradePage.jsx`): 수동 매수/매도 버튼, 각각 조건 미충족 시
  비활성화+이유 표시, 실거래 확인 대화상자, 색상은 한국 증시 관행(매수=빨강/
  매도=파랑).

**독립 검증 중 발견한 심각한 사고**: 서브에이전트 보고에 "테스트 중 실제
키움 API가 호출된 사고를 발견·수정했다"는 내용이 있어 직접 재현 검증하다가,
**그 수정 이후에도 여전히 남아있던 별개의 실사고 경로**를 발견했다.
`tests/test_auto_trade_router.py`만 단독 실행했더니 `test_get_log_returns_
rows_newest_first`가 실패했는데, 로그 내용이 `"매도 주문 시도 실패
(exit_stop_loss): ... 매도가능수량이 부족합니다"` — **실제 키움 서버에
진짜 매도 주문이 나갔다가 거부된 흔적**이었다.

원인: `test_toggle_on_preserves_existing_holding_position`(이번 작업과 무관한
기존 테스트, PLAN.md §5.54 시절부터 있던 것)이 "킬스위치를 켜도 보유
포지션이 보존되는지" 검증하려고 **실제** `POST /toggle {"enabled": true}`를
호출해 실 배포 엔진이 참조하는 바로 그 싱글턴 `AutoTradeState` 행에
`enabled=True` + `status="holding"` + `entry_price=16000`을 실제로 커밋한다.
이 저장소의 테스트는 전부 실 dev Postgres를 그대로 쓰는데(모킹 없음), 이
DB는 **실 배포된 `stock-backend-1` 컨테이너가 60초/30초 간격으로 계속
폴링하는 바로 그 DB**다 — 그 커밋이 살아있는 찰나의 순간에 운영 중인
`watch_stop_loss`(30초 잡)가 이를 진짜 포지션(당시 실시간가 15,385원, 진입가
16,000원 대비 -3.9%, 절체절명 손절 구간)으로 읽고 **실제 매도 주문을
시도했다.** 다행히 그 순간 실계좌가 0167A0을 0주 보유하고 있어서 키움이
"매도가능수량 부족"으로 거부해 실제 체결/손실은 없었다(실계좌
`get_unfilled_orders()`로 직접 재확인, 미체결 없음). **하지만 계좌가 실제로
포지션을 들고 있었다면 이 테스트를 돌리는 것만으로 진짜 손절 주문이
나갔을 것이다.**

**조치**: 근본적인 아키텍처 수정(테스트 전용 DB 분리 등)은 이번 범위 밖이라,
`tests/test_auto_trade_router.py`/`tests/test_auto_trader_collector.py`의
공통 autouse 픽스처(`_snapshot_and_restore_state`)에 **가드**를 추가했다 —
테스트 시작 전 실제 싱글턴 행의 `enabled`이 `True`면(즉 킬스위치가 실제로
켜져 있으면) `pytest.fail()`로 그 자리에서 크게 실패시키고 테스트 진행 자체를
막는다("조용히 안전하게 처리"가 아니라 "사람이 먼저 눈치채고 끄게 만든다"
쪽을 택함 — 조용히 넘어가면 다음에 또 같은 사고가 날 수 있다). 실제로
킬스위치를 켠 채로 단일 테스트를 실행해 가드가 기대대로 즉시(테스트 본문
실행 전, fixture 단계에서) 실패하는 것을 확인했다.

**향후 규칙(반드시 지킬 것)**: `test_auto_trade_router.py`/
`test_auto_trader_collector.py`를 실행하기 전에는 항상 먼저
`GET /api/auto-trade/state`로 실제 킬스위치가 꺼져 있는지 확인할 것 — 위
가드가 있어도 "몰라서 계속 켜둔 채로 시도하다 매번 실패만 반복"하는 것보다
애초에 끄고 시작하는 게 낫다.

**검증**: 킬스위치를 껐다가(`POST /toggle {"enabled": false}`) `venv/bin/
python -m pytest -q` 전체 실행 — 931 passed(무관 기존 실패 1건 제외, 신규
manual-buy/manual-sell 테스트 19개 포함). 같은 명령을 연속 2회 실행해도
동일 결과(재현성 확인). 프론트 `npm run build`/`npx oxlint` 정상. 실 계좌
`get_unfilled_orders()` 재확인 — 미체결 없음. `auto_trade_log`에서 사고
당시 생성된 잔여 행(id=1698) 직접 삭제로 정리. 최종 확인 시점 실제 상태:
...
### Phase 5.65 — 관심 테마 · 대장 종목 미니 트리맵 그리드 추가 (2026-08-13)

**계기**: "tree chart 혹은 heat chart 부분에... 2차전지, 엔터, 방산 이런거처럼
테마 별로 상위 종목만 따로 가지고 있는 차트 하나 더 추가하자, 섹터별로 10개
정도." 기존 업종·테마 트리맵(`GroupTreemap`)은 테마 자체(266개)를 박스로
접어서 보여줄 뿐 "테마 안의 개별 종목"은 보여주지 않는다 — 클릭하면 뜨는
"대장 종목 TOP10" 모달이 유일한 개별 종목 뷰였다. AskUserQuestion으로 확인:
테마 8개 고정(2차전지/반도체/엔터/방산/바이오/AI 등, 추천안 채택) + 레이아웃은
테마별 작은 트리맵을 그리드로 배치(추천안 채택, 기존 컴포넌트 재사용이 쉬움).

**8개 고정 테마**(`group_snapshot` 실데이터에서 정확한 문자열 확인 후 채택):
2차전지 / 반도체 대표주(생산) / 엔터테인먼트 / 방위산업/전쟁 및 테러 /
바이오시밀러(복제 바이오의약품) / 지능형로봇/인공지능(AI) / 조선 /
자동차 대표주.

**구현**:
- `routers/groups.py`에 `GET /api/groups/theme-picks?limit=10` 추가 — 이미
  있던 "트리맵 박스 클릭 → 대장 종목 TOP10" 기능(`_warm_group_name_to_no`
  1일 TTL, `_warm_top_stocks` 5분 TTL)을 재구현 없이 그대로 재사용해 8개
  테마를 한 번에 모아 반환. 테마 하나가 실패해도(이름 매핑 없음/네이버 조회
  실패) 그 테마만 `rows: []`로 두고 나머지 7개는 정상 반환(부분 실패 허용,
  전체 502 없음).
- `GroupTreemap.jsx`에 `fold`(기본 `true`) prop 추가 — 기존 업종/테마
  메인 트리맵은 그대로(회귀 없음), 새 미니 트리맵만 `fold={false}`로 "보합권
  캡션 접기·상승/하락 각 TOP10 박스 제한" 로직을 건너뛰어 이미 10개로 추려진
  종목이 캡션으로 밀려나지 않고 전부 박스로 그려지게 함.
- `DashboardPage.jsx`에 "관심 테마 · 대장 종목" 섹션 추가(업종·테마 트리맵
  바로 아래, 2열 그리드) — 기존 7분 폴링 티어에 얹어 새 setInterval 없이
  재사용, 박스 클릭 시 기존 `openStockModal` 헬퍼로 종목 상세 모달 오픈.

**검증**: 실제 계좌에 보유 포지션이 열려 있는 상태(진입 17,795원)라 이번엔
전체 테스트 스위트를 돌리지 않고 이 기능과 무관한 groups 관련 테스트 4개
파일만 직접 실행 — 25 passed(신규 `test_groups_theme_picks_router.py` 6개
포함, 캐시 공유 검증 포함). `npm run build`/`npx oxlint` 클린(기존 무관
warning 5건 제외). 실 컨테이너에서 `curl .../groups/theme-picks` 직접 확인
— 8개 테마 전부 실데이터 반환(일부 테마는 실제 구성 종목이 10개 미만이라
...
### Phase 5.70 — 매매일지 노이즈 근본 수정 + 페이지네이션/날짜 필터 (2026-08-18)

**계기**: "자동매매 > 매매일지가 이상해.. 너무 방대하게 정보가 나와.. paging
처리를 하던.. 날짜 조회를 하던 data grid 를 쓰던 해야 할 것 같아." 실 데이터
확인 결과 "이상해 보이는" 진짜 원인은 UI가 아니라 백엔드였다 — 최근 200행
전부가 `event_type: "error"`. 추적: 2026-08-17(광복절 대체공휴일)은 평일이라
`market_hours.is_nxt_closed()`(요일+시각만 보고 공휴일은 모름, 기존 문서화된
한계)가 "열림"으로 판정 → `run_auto_trade`(60초)가 하루 종일 진입 신호를
정상 평가 → 조건 충족마다 실제 매수 주문 제출 → 키움이 "장이 열리지않는
날입니다"로 매번 거부 → 그 실패가 매번 `AutoTradeLog`에 남아 **하루 288건**.
정상 이벤트 몇 건을 노이즈가 통째로 덮어써서 "이상하다"는 인상을 준 것.

**근본 수정**(`collectors/auto_trader.py::run_auto_trade`): 새 홀리데이
달력 없이, 이미 매 폴링 가져오는 1분봉(`_warm_stock_intraday`)의 마지막
봉 날짜를 오늘과 대조하는 가드 추가 — `ka10080`은 공휴일에 조회해도
`parse_minute_chart_rows`(`clients/kiwoom.py`)가 "가장 최근 날짜"만 골라
반환하므로 마지막 실제 거래일 데이터가 그대로 온다, 그 날짜가 오늘과 다르면
킬스위치 꺼짐과 동일하게 신호 평가/주문 시도 자체를 하지 않고 로그도
안 남긴다(`action="stale_data"`, 노이즈 방지 원칙 그대로). `now_kst`와
나란히 `today_str`(선택, 기본 None=실제 오늘)을 주입 가능하게 추가(house
rule). `watch_stop_loss`(손절 감시, 실시간 호가 기반, 포지션 있을 때만
동작)는 범위 밖으로 뒀다 — 당시 실계좌에 포지션이 없었고(재확인 완료),
포지션이 걸린 경로는 "일단 시도, 브로커가 검증"이 더 안전한 기본값이라
스킵 로직을 추가하지 않는 게 낫다는 판단.

**페이지네이션/날짜 필터**: `routers/auto_trade.py`의 `GET /log`에
`offset`/`date`(KST 기준 하루) 추가 + 응답에 `total` 추가, 기본 `limit`을
200→30으로 낮춤. `AutoTradePage.jsx`의 `LogPanel`에 날짜 필터
(`<input type="date">`) + 이전/다음 페이지 버튼 + "N–M / 총 T건" 표시
추가(새 데이터그리드 라이브러리 도입 안 함 — 이 코드베이스 전체가 순수
`<table>` 관례). 15초 자동 새로고침은 유지하되 현재 필터/페이지를 그대로
다시 불러오도록 수정(과거 페이지 보는 중에 최신 페이지로 안 튕기게).

**검증**: 킬스위치 확인 후(`enabled: true, status: idle`) 임시로 꺼서
`test_auto_trader_collector.py`/`test_auto_trade_router.py` 포함 전체
실행 — 63 passed(신규 가드 테스트 2개 + `/log` 날짜/페이지네이션 테스트
2개 포함), 직후 `enabled: true`로 정확히 복원 확인(§5.63/§5.64 절차).
기존 `_patch_signals` 테스트 헬퍼가 봉에 `date` 필드가 없어 새 가드가
모든 기존 테스트를 오탐(stale_data)시킬 뻔했던 것을 미리 발견 —
`bars_date` 기본값을 "실제 오늘"(가드의 기본 동작과 동일하게 계산)로
채워 35개 기존 호출부를 하나도 안 건드리고 해결. 나머지 파일 제외
`pytest -q` — 856 passed(§5.69와 동일, 무관 기존 실패 1건 그대로).
`npm run build`/`npx oxlint` 클린(신규 warning은 다른 페이지들에 이미
있던 것과 동일한 종류의 exhaustive-deps 경고 1건뿐). 실 컨테이너 curl로
`/api/auto-trade/log?limit=5&offset=0`, `?date=2026-08-17` 각각 정상
응답(`total` 필드 포함) 확인, `docker logs stock-backend-1`에 신규
에러 없음.

**후속 발견(이번 수정 범위 밖, 사용자에게 별도 보고)**: 검증 중 오늘
(2026-08-18, 정상 거래일) 실시간으로 `event_type: "error"` 행이 계속
쌓이고 있는 걸 확인 — 사유가 이번에 고친 홀리데이 케이스와 다르다
("매수증거금이 부족합니다. 0주 매수가능", 키움 반환코드 855056). 오늘은
실제 거래일이라 새 가드가 막지 않는 게 정상이고, 이건 계좌 잔고/증거금
문제로 보여 코드 범위 밖이다 — PLAN.md에 남기고 사용자에게 보고만 함.

### Phase 5.71 — 자동매매 안전장치: 미체결 주문 확인 + 매매일지 반복 노이즈 억제 (2026-08-19)

**계기(실사고)**: 사용자가 매매일지를 보고 "900건 이상인데 의미 있는
정보가 맞을까?"(§5.70 후속) 지적하는 도중, "지금 내가 직접 매수 했는데
실거래가 안되고 있어"라는 별도 문제 제기가 들어와 조사하다가 훨씬 심각한
사고를 발견했다: 8/14 15:20 KST EOD 강제청산이 매도 지정가 주문을 제출해
접수(`return_code=0`)까지는 됐지만, 그 사이 가격이 움직여 장마감(15:30)
까지 실제로 체결되지 않은 채 자동 취소됐다. 그런데 `_execute_sell`은
**주문 제출 성공 = 체결 성공**으로 간주해 그 즉시 `AutoTradeState`를
idle로 갱신해버렸다 — 실제로는 그대로 보유 중이던 포지션(1주, 매입가
18,015원)에 **5일간(8/14~8/19) 손절/트레일링 감시가 전혀 작동하지
않았다**. 실계좌를 직접 조회(kt00004/kt00018, 이번에 처음 실호출 확정)해
발견, 상태를 수동으로 `holding`으로 정정하자마자 30초 손절 감시가 즉시
작동해 -5.52% 구간에서 실제로 청산해 실피해를 막았다(체결 확인 완료,
계좌 실측 총자산 25,000원→약 23,042원 — 걱정했던 것만큼 크게 잃은 건
아니었고 대부분 "잃은 게 아니라 5일간 안 팔리고 묶여있던 평가액"이었음).

이 모듈의 기존 docstring이 애초에 "이 전제(주문 접수=체결)가 깨지는
경우에 대한 대응은 이번 범위 밖의 후속 작업"이라고 명시해뒀던 것 — 오늘이
바로 그 후속 작업이 필요해진 시점.

**구현 1 — 미체결 주문 확인**: 새 헬퍼 2개(`_order_still_unfilled`,
`_cancel_unfilled_order_silently`, `collectors/auto_trader.py`) —
`get_unfilled_orders`(ka10075)/`cancel_order`(kt10003) 둘 다 이미 실호출
확정된 조회/취소 TR을 재사용(새 TR 없음). 매수/매도 주문 제출 직후마다
호출해 미체결이면 취소를 시도하고 **상태 전이를 하지 않은 채**(idle↔holding
어느 방향으로도) `buy_unconfirmed`/`sell_unconfirmed` 이벤트로만 로그.
적용 지점 4곳(전부 동일 패턴): `_handle_idle`의 매수 블록(자동 진입),
`_execute_sell`(손절/트레일청산/EOD강제청산/수급반전청산/수동매도 공유
함수), `watch_stop_loss`의 인라인 매도 블록(`_execute_sell`을 안 쓰고
자체 구현이라 별도 적용 필요했음 — 30초 고빈도 손절 감시), `routers/
auto_trade.py`의 `manual_buy_auto_trade`(마찬가지로 자체 구현, 502 +
"체결 미확인" 메시지). 조회 자체가 실패하거나 `ord_no`가 없으면 보수적으로
"미체결"로 취급(확인 안 되면 이전 상태 유지가 항상 더 안전).

**구현 2 — 매매일지 반복 노이즈 억제**: `_log()` 앞단에 in-process 모듈
전역 dict(`_last_log_by_code`, `_intraday_cache` 등 기존 캐시와 동일한
패턴) — 같은 code에 대해 직전 기록과 `(event_type, reason)`이 바이트
단위로 동일하고 5분(`_LOG_DEDUP_WINDOW`) 이내면 새 행을 안 만든다. DB
쿼리 기반 dedup 대신 in-process dict를 쓴 이유: 이 프로젝트 테스트가 전부
실 dev Postgres를 공유해서 DB 쿼리로 하면 다른 테스트/실운영 행과 우연히
겹쳐 오탐 억제될 위험이 있음.

**검증(실계좌 안전 절차, §5.63/§5.64보다 엄격히 적용)**: 구현 도중
실제로 새 포지션이 진입돼(수동 매수, 매입가 17,020원) 보유 중인 상태가
됐다 — `test_auto_trader_collector.py`/`test_auto_trade_router.py`는
`status: holding`인 동안 절대 실행 금지(킬스위치를 끄면 실시간 손절
보호가 사라짐)이므로, 그 두 파일을 제외한 나머지 전체(`pytest -q`)만
먼저 실행해 856 passed 확인, 이후 포지션이 자연 청산될 때까지 대기 —
실제로 30초 후 손절 감시(-0.88%, 리스크 경보로 임시 -0.8% 적용)가
발동해 청산됐고, **이번에 새로 추가한 체결 확인 로직이 정상적으로
"체결 확인됨"(exit_stop_loss, sell_unconfirmed 아님)으로 판정**해
idle로 정확히 복귀함을 실계좌 재조회(kt00018, 보유 0건)로 확인 — 새
안전장치가 실거래에서 첫 실전 검증을 통과함. 이후 `status: idle` 확인
후 킬스위치를 임시로 끄고 두 파일 포함 전체 실행(72 passed, 신규
`_UnfilledBuyClient`/`_UnfilledSellClient` 테스트 더블 관련 버그
1개 발견·수정 — `instances`가 베이스 클래스 `_FakeKiwoomClient`의
클래스 속성이라 서브클래스에 재할당하면 실제 append가 일어나는 리스트와
분리되는 문제, 베이스 클래스 참조로 통일해 해결), 직후 킬스위치를
정확히 원상 복구(`enabled: true`) 확인. 나머지 파일 제외 재실행도
856 passed 그대로(무관 기존 실패 1건). `npm run build`/`npx oxlint`
