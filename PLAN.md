...
| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.12-1 | 백엔드 | `fetch_group_constituents` + `GET /api/groups/top-stocks` + 이름→no 캐싱 | 실제 업종/테마 이름으로 TOP10(코드·이름·등락률·거래대금) 반환 확인 |
| 5.12-2 | 프런트 | 트리맵 박스 클릭 → TOP10 모달, 행 클릭 → 종목 상세 | 트리맵 아무 박스나 클릭 시 그 분류의 실제 대장주 상위 10개 표시 |
