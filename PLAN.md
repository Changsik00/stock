...
| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.39-1 | 소스 조사 | 키움 API(관리종목/투자유의·경고·위험 종목, 단기과열 지정 관련 TR 탐색) + data.krx.co.kr 무인증 서브포털(§5.32와 같은 방식) 실호출로 확인 | 실호출로 URL/파라미터/응답 스키마 확정 또는 정직한 dead-end 문서화 |
| 5.39-2 | (조건부) 수집/조회 구현 | 5.39-1에서 소스를 찾은 경우만 진행 — 등급 조회 로직 + `risk_alert.py` 확장(기존 "공식 임계값 + 관찰형 문구" 원칙 그대로 적용) | 실데이터로 등급 반영 확인 |
| 5.39-3 | (조건부) 프런트 반영 | 위험 배너에 새 등급 alert kind 추가 | vite build + 실데이터 확인 |
