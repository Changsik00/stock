...
**장중 갱신 확인**: 08:00 KST(개장 전) 조사 시작 시점엔 최신 거래일이 전날(07-20, 월)로 나와 "개장 전엔
어제 마지막 거래일" 가정과 일치. **09:00 개장 이후 실제로 최신 봉이 갱신되는지는 미검증 — 이번 세션은
개장 전(08:00~08:20 KST)에 종료돼 재호출로 확인하지 못했다.** 다음 세션에서 개장 중(09:00 이후) `curl
"http://localhost:8123/api/stocks/005930/intraday?interval=1"`을 몇 분 간격으로 재호출해 `bars` 마지막
원소의 `time`이 갱신되는지 확인 필요 — 캐시 TTL(1분봉 60초)이 지나면 새 호출이 나가도록 이미 구현은
돼 있으나(§5.1-1 코드), 키움 API 자체가 장중에 최신 분봉을 실시간으로 채워주는지는 실측하지 않았다.
