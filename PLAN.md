...
| 5.50-6 | "오늘의 포지셔닝 재료" 브리핑 패널(하이닉스 중심) | 새 프런트 섹션 — ①regime+쏠림 비율, ③외인/기관 현물·선물 오전 누적(`foreign-position/intraday-accumulated`), ⑤리스크 경보를 기존 엔드포인트 그대로 재사용해 하이닉스 중심 한 화면에 재배치. 신규 백엔드 없음(순수 프런트 조합) | 실 컨테이너에서 5개 섹션 모두 데이터 표시 확인, vite build 클린, 판단 문구 없음(관찰치만) |
| 5.50-7 | 하이닉스 상대강도(신규) | 000660 등락률 − 코스피 등락률(z-score 아님, 단순 초과수익률 관찰치) 계산 — 백엔드 소스는 이미 있는 stocks 등락률 + market series 재사용, 새 TR 없음 | 실데이터 확인, "강함/약함" 같은 판정어 없이 %pt 숫자만 노출 |
