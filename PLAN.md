...
| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.16-1 | 개인 "집계 중" 라벨 | 오늘 날짜 + 개인 net_value=0 + 다른 투자자군은 0 아님 → "집계 중" 표시(진짜 0과 구분) | 실제 0원인 날/종목과 구분되게 렌더 |
| 5.16-2 | 회전율 추가 | `/api/stocks/{code}/series`에 `value_rank` 최신 회전율 조인, 종목 상세 모달에 표시 | 값 있으면 %, 없으면 "데이터 없음" |
