...
| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.46-1 | 키움 토큰 8005 자동 재발급 | `clients/kiwoom.py::call_tr`에 `return_code==3`(인증 실패) 감지 시 로컬 캐시(메모리+파일) 모두 무시하고 강제 재발급 후 1회 재시도하는 로직 추가(무한루프 방지 — 기존 `max_retries` 상한 그대로 적용) | 단위테스트(모킹으로 8005 응답 → 재발급 → 성공 시나리오) |
| 5.46-2 | flow_rank 저녁 재수집 크론 | §5.22와 동일한 패턴으로 `collectors/scheduler.py`에 18:00 정기 배치와 별개로 flow_rank만 다시 도는 저녁 늦은 시각(예: 19:30 KST) 크론 추가 — 소스가 idempotent(재실행 안전)이므로 새 로직 없이 같은 collect_fn 재사용 | 실행 후 collect_log의 flow_rank message가 target_date와 일치하는지 확인 |
