...
| 5.50-2 | 종목 페어 통합 뷰 | 본주+레버리지2X+인버스2X 3열, 현재가·등락률·호가 매물대(ka10004 재사용). 삼성전자/SK하이닉스 세트 토글 | 실데이터로 세 종목 호가 동시 표시 확인, vite build 클린 |
| 5.50-3 | ETF 괴리율 표시 | 5.50-1에서 확정한 `naver_etf.fetch_etf_list()` 재사용, 레버리지/인버스 ETF 열에 NAV 대비 괴리율(%) 추가 | 실데이터 확인 |
| 5.50-5 | 나스닥선물 참고 타일 | 5.50-1에서 확정한 yfinance `NQ=F` 인트라데이로 최신값 + 갱신 시각 표시(판단 문구 없이 숫자만). SOX는 기존 EOD 타일 그대로 재사용 | 실데이터 확인 |
