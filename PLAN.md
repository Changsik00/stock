...
| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.41-1 | 부호 있는 프로파일 계산 함수 | 신규 `quant/futures_flow_profile.py` — `quant/volume_profile.py`의 binning 로직을 부호 있는 값에 맞게 재사용/변형, 양/음 각각 극값(레벨) 추출 | 단위테스트(합성 데이터로 상방/하방 구간 분리 검증) |
| 5.41-2 | API 반영 | `GET /api/markets/futures/series`에 옵트인 필드(예: `include_flow_profile`)로 외국인/기관계 각각의 프로파일 추가 — 새 외부 호출 없음(이미 있는 index_ohlcv+market_flow 조인) | curl로 실데이터 확인 |
| 5.41-3 | 프런트 반영 | `CandleChart.jsx`에 방향성 있는 레벨 오버레이 추가(§5.34 `levels` prop과는 별개 색상 로직 — 매수구간 빨강계열/매도구간 파랑계열), 관찰형 문구+근사치 명시, 선물 캔들 모달에 적용 | 코드리뷰 + vite build 클린 + 실데이터 확인 |
