...
| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.15-1 | 백테스트 함수 | `regime_backtest.py` — 스트릭·버킷 통계 계산을 재사용 가능한 함수로 | 오늘 애드혹 결과와 동일한 수치가 함수 호출로 재현됨 |
| 5.15-2 | 오늘의 판정 API | `GET /api/markets/regime` — 현재 스트릭 + 백테스트 통계 + 판정 | 코스닥 외국인 스트릭이 실시간 반영, 코스피는 "신호 약함" 명시 |
| 5.15-3 | 프런트 배너 + 필터 연결 | "지금 유입 우세" 카드 + 종목 랭킹 3종 시장 필터 버튼 | 판정된 시장 클릭 시 3개 카드가 그 시장으로 필터링 |
