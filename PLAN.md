...
| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.18-1 | 쏠림 비율 조회 함수 | `intraday_snapshot.py`에 4개 시리즈를 시간매칭해 쏠림%를 계산하는 함수 추가 | 오늘 실데이터로 정확한 쏠림% 시계열 산출 |
| 5.18-2 | API + 프런트 | `GET /api/markets/flow-concentration/intraday-accumulated`(또는 기존 라우트 확장) + 1D 추이 차트(50% 기준선, `BreadthRatioChart` 패턴 재사용) | 대시보드에서 쏠림 추이가 시간순 그래프로 보임 |
