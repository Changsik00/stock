...
| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.10-1 | 백엔드 버퍼 분리 | `intraday_snapshot.py` 코스피/코스닥 버퍼 분리, `get_flow_series` 응답 변경, `get_foreign_position_series`는 합산 유지(회귀 없음) | 유닛테스트로 분리·합산 둘 다 검증 |
| 5.10-2 | 프런트 토글 | FlowSummaryModal 3M/1D 양쪽에 합계/코스피/코스닥 토글 | 코스닥만 선택 시 코스닥 수급만 표시, 외인 양손 모달은 그대로 |
