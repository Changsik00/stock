...
|---|---|---|
| 1분 (유지) | 등락종목수·수급잠정(ka10051)·관심순위(ka00198) | 환율(네이버, FX는 상시 변동) — 값이 실제로 분단위로 바뀌면 편입 |
...
| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 4.7-1 | 장중 실측 | 조건부 대상 소스를 실제 장중(2026-07-20 월요일)에 5~10분 간격으로 재호출해 값이 바뀌는지 확인 — 바뀌면 티어 편입, 안 바뀌면(일별 확정치임이 실증되면) 1일 고정으로 재분류하고 근거를 PLAN에 기록 | 소스별 장중 갱신 여부 표 |
| 4.7-2 | 3단 스케줄러 | collectors/live_refresh.py를 확장(또는 새 인터벌 잡 추가) — 1분 잡은 기존 그대로, 5~10분 잡을 별도 IntervalTrigger로 추가(장중만). market_flow/futures_flow/basis/value_rank/flow_rank/group_snapshot의 "장중 온디맨드" 캐시 워밍 함수 필요(기존 EOD 배치와 별개 — DB에는 안 쓰고 메모리 캐시만, §3.5 원칙과 동일) | 5~10분 캐시가 장중에 갱신됨 |
| 4.7-3 | 프런트 반영 | 대시보드의 해당 섹션에 폴링 주기 배지(예: "5분 갱신") 표시, setInterval 주기 조정 | 화면에서 주기별 갱신 확인 |
