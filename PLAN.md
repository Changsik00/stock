...
| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.1-1 | 종목·지수 분봉 API | ka10080/ka20005 실호출 확정(tic_scope 허용값, 응답 필드, 하루 범위) → `GET /api/stocks/{code}/intraday?interval=` , `GET /api/markets/{market}/intraday?interval=` (온디맨드+짧은 캐시, DB 미저장) | 1/3/5/10/60분봉 실데이터 응답 |
| 5.1-2 | 선물 분봉 소스 탐색 | 네이버 등에서 K200 선물 인트라데이 분봉 소스 실측 → 되면 API 추가, 안 되면 PLAN에 한계 기록 | 판정 결과 문서화 |
| 5.1-3 | 차트 UI | CandleChart에 "일봉 / 1·3·5·10·60분" 토글 — 시장 탭 + 종목 모달 | 분봉 캔들 렌더 |
