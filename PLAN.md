...
### Phase 5.54 — 자동매매 엔진: 트레일링 스탑(KODEX SK하이닉스레버리지) + 전용 탭 (2026-08-04)
...
**대상**: SK하이닉스 본주(1주 159만원대, 예산으로 살 수 없음)나 삼성전자(24만원대)가 아니라
**KODEX SK하이닉스레버리지(0193T0, §5.50에서 이미 확보한 인프라 — `stock_quote`/
`parse_quote_levels`/`naver_etf` NAV 재사용 가능)** 단독. 국내 상장 ETF는 매도 시
증권거래세가 면제돼(개별 주식은 0.15~0.18%) 반복 매매 구조에 유리하다는 점도 사용자가
직접 근거로 제시했다. 사용자가 "2X 롱/-2X 인버스 둘 다 쓸까"를 물었을 때 **"지금은 2X 롱만"**
으로 확정 — 인버스(0197X0)는 이번 범위 밖.
...
| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.54-0 | 주문 TR 실호출 검증(선행) | 0193T0 1주, 체결 불가 가격으로 매수 → 미체결 조회 → 취소. 실제 응답 필드 확인해 kt10000/kt10001 docstring "미확정" 표기 갱신 | 실호출 3단계(주문/조회/취소) 전부 성공, 실제 돈 이동 없음(체결 안 됨) 확인 |
