...
**신규 데이터소스 조사 필요(이번 Phase의 첫 작업, 5.50-1)**:
- **ETF NAV 실시간 괴리율**: §1에 이미 `ka40001`~`ka40010`, 실시간 `0G`(NAV) 후보가 적혀
  있었으나 괴리율(현재가 대비 NAV 편차) 필드가 실제로 있는지는 미확인. `ka40004`(ETF 목록,
  이미 EOD로 수집 중인 `etf_stats`의 소스)에 괴리율 필드가 있는지부터 확인.
- **개별주식선물(주식선물) 시세**: 위에서 확인한 대로 상품 자체는 존재가 확인됐으나, 키움
  TR로 시세를 조회하는 구체적 방법(TR id/파라미터)은 아직 미확인.
- **나스닥 선물(NQ=F 등) 준실시간 소스**: 기존 `clients/us_indices.py`는 EOD 종가만
  제공한다(yfinance `^IXIC`, 전일 값). 장중(KST 기준) 시점의 선물가는 별도 조사 필요
  (yfinance `NQ=F` 인트라데이 조회 가능 여부 등).

| # | 작업 | 내용 | 완료 기준 |
|---|---|---|---|
| 5.50-1 | 데이터소스 조사 | ETF 괴리율 필드(ka40001~10), 개별주식선물 시세 TR/소스, 나스닥선물·SOX 준실시간 소스 3가지를 웹서치+실호출 probe로 확정. 코드 변경 없음, 조사 결과만 이 섹션에 추가 기록 | 3개 소스 전부 "확정" 또는 "대안 확정"으로 이 섹션에 기록 |
| 5.50-2 | 종목 페어 통합 뷰 | 새 프런트 화면/모달 — 본주+레버리지2X+인버스2X 3열, 각 열에 현재가·등락률·호가 매물대(ka10004 재사용) 표시. 삼성전자 세트/SK하이닉스 세트 토글 | 실데이터로 세 종목 호가 동시 표시 확인, vite build 클린 |
| 5.50-3 | ETF 괴리율 표시 | 5.50-1 확정 소스로 레버리지/인버스 ETF 열에 NAV 대비 괴리율(%) 추가 | 실데이터 확인 |
| 5.50-4 | 개별주식선물 표시 | 5.50-1 확정 소스로 참고 열 추가(가격·베이시스) | 실데이터 확인 |
| 5.50-5 | SOX/나스닥선물 참고 타일 | 통합 뷰 상단에 두 지수 최신값 + 갱신 시각 표시(판단 문구 없이 숫자만) | 실데이터 확인 |
