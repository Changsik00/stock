...
// GET /api/stocks/{code}/series?days=N[&include_volume_profile=true][&include_flow_percentile=true]
// -> {code, name, market, is_etf, days, prices, flows, meta, volume_profile?, flow_percentile?} —
// 마찬가지로 온디맨드 전용(정적 스냅샷 없음).
...
// includeVolumeProfile(PLAN.md §5.34)/includeFlowPercentile(PLAN.md §5.38)은
// 둘 다 옵트인 — 백엔드가 기본값 false로 두어 이 계산을 매번 하지 않는 정책
// (routers/stocks.py::stock_series docstring 참고)과 대칭을 맞춘다.
export async function fetchStockSeries(
  code,
  days = 180,
  hints = {},
  includeVolumeProfile = false,
  includeFlowPercentile = false
) {
  const params = new URLSearchParams({ days: String(days) })
  ...
  if (includeVolumeProfile) params.set('include_volume_profile', 'true')
  if (includeFlowPercentile) params.set('include_flow_percentile', 'true')
