...
PLAN.md §6 Phase 1-4. collectors/market_flow.py(pykrx)를 하루씩 순회 호출한다.
...
- 특정 일자 수집이 실패해도(재시도 3회 모두 실패) 그 날짜만 건너뛰고 계속 진행한다
  ...
  이를 재구현하지 않는다).
- 재실행 시 이미 market_flow에 해당 (market, date) 행이 있는 날짜는 pykrx를 다시
  호출하지 않고 건너뛴다(휴장일 판별이 안 되므로 순수 날짜 존재 여부로 skip 판단).

**중요**: 2026-02 KRX 데이터 포털 개편으로 pykrx가 동작하려면 .env에 KRX_ID/KRX_PW
(무료 data.krx.co.kr 회원 로그인)가 설정돼 있어야 한다. 없으면 모든 날짜가
"데이터 없음(0행)"으로 표시되며 실제로는 아무것도 적재되지 않는다 — 자세한 배경은
app/clients/pykrx_client.py 모듈 docstring 참고.
