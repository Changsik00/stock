...
- basis.json: GET /api/markets/basis?days=1095 와 동일 ({"days", "series", "latest",
  "expiry"}) — K200 선물-현물 베이시스 + 다음 만기(PLAN.md §4.5-3/-5). basis.py는
  main.py에 등록된 라우터라(routers/markets.py 등과 동일하게) 함수를 직접 import해서
  세션만 넘기면 된다 — 다른 미등록 라우터를 위한 특별 처리 불필요.
- derivative-flow.json: GET /api/etf/derivative-flow?days=365 와 동일 ({"days",
  "universe", "latest", "series"}) — 파생형(레버리지/인버스) ETF 방향성 게이지
  (§4.5-1/-5). 라우터 상한이 le=365라(routers/etf.py) DEFAULT_DAYS(1095)를 그대로
  쓸 수 없어 DERIVATIVE_FLOW_DAYS(365, 라우터 상한과 동일)를 쓴다. net_inflow가
  2026-07-15부터 적재를 시작한 누적형 시리즈라(모듈 docstring 참고) 당분간 series
  길이는 실제 적재된 날짜 수만큼만 나온다 — 프런트(api.js fetchDerivativeFlow)가
  필요한 만큼만 다시 자른다.
...
from app.routers.basis import basis_series  # noqa: E402
from app.routers.etf import derivative_flow  # noqa: E402
...
MACRO_IDS = (
    ...
    "prog_arb_kospi,prog_nonarb_kospi,prog_arb_kosdaq,prog_nonarb_kosdaq"
)
DEFAULT_DAYS = 1095
FLOW_RANK_DAYS = 30
FLOW_PATH_LIMIT = 50
DERIVATIVE_FLOW_DAYS = 365  # routers/etf.py derivative_flow의 상한(le=365)과 동일
...
async def main(out_dir: str, days: int = DEFAULT_DAYS) -> None:
    ...
    async with async_session_factory() as session:
        ...
        basis_result = await basis_series(days=days, session=session)
        basis_path = out_path / "basis.json"
        size = _write_json(basis_path, basis_result)
        logger.info(
            "basis: %d개 날짜, latest=%s -> %s (%d bytes)",
            len(basis_result["series"]),
            basis_result["latest"],
            basis_path,
            size,
        )

        derivative_flow_result = await derivative_flow(days=DERIVATIVE_FLOW_DAYS, session=session)
        derivative_flow_path = out_path / "derivative-flow.json"
        size = _write_json(derivative_flow_path, derivative_flow_result)
        logger.info(
            "derivative-flow: %d개 날짜, universe=%s -> %s (%d bytes)",
            len(derivative_flow_result["series"]),
            derivative_flow_result["universe"],
            derivative_flow_path,
            size,
        )
