...
- flow-path.json: GET /api/markets/flow-path?days=30&limit=50 과 동일
  ({"date", "days", "rows"}) — flow_path도 flow_rank와 같은 이유로 날짜 누적이
  느리므로 FLOW_RANK_DAYS(30)를 그대로 재사용한다. 프런트(api.js fetchFlowPath)는
  정적 모드에서 이 파일을 그대로 반환한다(서버처럼 "최신 날짜 1개"만 담겨 있으므로
  추가 슬라이싱이 필요 없음).
...
async def main(out_dir: str, days: int = DEFAULT_DAYS) -> None:
    ...
    async with async_session_factory() as session:
        ...
        flow_path_result = await flow_path_top(days=FLOW_RANK_DAYS, limit=FLOW_PATH_LIMIT, session=session)
        flow_path_path = out_path / "flow-path.json"
        size = _write_json(flow_path_path, flow_path_result)
        logger.info(
            "flow-path: date=%s, %d개 종목 -> %s (%d bytes)",
