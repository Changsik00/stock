...
1. ``live_refresh``(60초, 기존): routers/markets.py의 breadth/live, flow/live,
   ...
   갱신을 확인해 편입) 5개 라이브 캐시를 요청 없이도 선제적으로 채운다.
...
kospi/kosdaq + index-tiles 선물 fchart + fx 환율 1콜) — KiwoomClient 자체
리미터(1req/s)가 있어 문제없다. 7분 잡은 매 호출마다 네이버 ~50콜(value-rank
전량 순회 44콜 내외 + basis 2콜 + groups 2콜 + futures-flow 1콜) — 7분 창 안에
15~30초 소요라 여유 있다.
"""
